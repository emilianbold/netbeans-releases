...
class ProjectPackages extends SelectorChildren<ContainerNode> {
    ...
    public ProjectPackages(final SourcePackageInfo.Scope type, final boolean includeSubprojects) {
        this.pType = type;
    ...
    protected List<SelectorNode> prepareChildren(ContainerNode parent) {        
        ...
        for(SourcePackageInfo pkg : ProfilerTypeUtils.getPackages(subprojects, pType, parent)) {
            nodes.add(new PackageNode(pkg, parent));
        }
        Collections.sort(nodes, PackageNode.COMPARATOR);
