...
public class PanelUninstallFeatureConsoleImpl {
    ...
    private static final String DESCRIPTION =
        "$L(com.installshield.product.i18n.ProductResources,UninstallFeaturePanel.description,$P(displayName))";

    public void consoleInteractionUninstallFeature(ISDialogContext context) {
        ...
        while (true) {
            ...
            if (response >= 1) {
                ...
                if (beanHasChildren(pService, selected)) {
    ...
    private String[] getRequiredFeatureProperties() {
        return new String[] { "activeForUninstall", "displayName", "enabled" };
    }
    ...
    private ProductBean[] printFeatures(
        ...
        for (ProductBean child = iter.getNext(iter.begin());
            ...
            if (canDisplayBean(service, child)) {
                ...
                if (beanHasChildren(service, child)) {
                    line.append(" +");
                }
                else {
                    line.append("  ");
    ...
    private String getSelectOption(String beanDisplayName) {
        return LocalizedStringResolver.resolve(
            "com.installshield.product.i18n.ProductResources",
            "UninstallFeaturePanel.selectFeature",
            new String[] { beanDisplayName });
    }

    private boolean beanHasChildren(ProductService service, ProductBean bean) {
    ...
    private boolean canDisplayBean(ProductService service, ProductBean bean) {
        return (bean instanceof ProductFeature || bean instanceof Product)
            && isBeanVisible(service, bean.getBeanId());
    }
    ...
    private boolean isBeanActive(ProductTree tree, String beanId) {
        ...
        return bean instanceof SoftwareObject
            ? ((SoftwareObject)bean).isActiveForUninstall()
            : false;
    }
    ...
    private boolean isBeanVisible(ProductService service, String beanId) {
        return isBeanInstalled(service, beanId);
    }
    ...
    private String getNodeCaption(
        ...
        if (disabled) {
            buf.append(" ");
            buf.append(LocalizedStringResolver.resolve(
                    "com.installshield.product.i18n.ProductResources",
    ...
    private boolean isBeanInstalled(String beanId, WizardServices wServices) {
