...
public class SetSystemPropertiesAction extends WizardAction {
    ...
    private void resolveProductBeanProperties () {
        try {
            ...
            // ------------------ Core IDE -----------------------------
            keyObject = (SoftwareObjectKey) service.getProductBeanProperty
            (ProductService.DEFAULT_PRODUCT_SOURCE, "beanMobilityCluster", "key");
            key = resolveString("$L(org.netbeans.installer.cluster.Bundle,MobilityCluster.UID)");
            logEvent(this, Log.DBG,"Mobility Cluster UID: " + key);
            keyObject.setUID(key);
            ...
            service.setRetainedProductBeanProperty
            (ProductService.DEFAULT_PRODUCT_SOURCE, "beanMobilityCluster", "key", keyObject);
