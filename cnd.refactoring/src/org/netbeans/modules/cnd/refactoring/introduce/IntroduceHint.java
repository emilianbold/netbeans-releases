...
/**
 * based on org.netbeans.modules.java.hints.introduce.IntroduceHint
 * @author Jan Lahoda
...
//    public void run(CsmContext info) {
//        cancel.set(false);
//
//        FileObject file = info.getFileObject();
//        int[] selection = SelectionAwareCsmFileTaskFactory.getLastSelection(file);
//
//        if (selection == null) {
//            //nothing to do....
...
//            Fix variable = isVariable ? new IntroduceFix(h, info.getCsmFile(), guessedName, duplicatesForVariable.size() + 1, IntroduceKind.CREATE_VARIABLE) : null;
//            Fix constant = isConstant ? new IntroduceFix(h, info.getCsmFile(), guessedName, duplicatesForConstant.size() + 1, IntroduceKind.CREATE_CONSTANT) : null;
            Fix variable = new IntroduceFix(info, guessedName, 1, IntroduceKind.CREATE_VARIABLE);
            Fix constant = new IntroduceFix(info, guessedName, 1, IntroduceKind.CREATE_CONSTANT);
            Fix field = null;
            Fix methodFix = null;
//
...
//                }
//
//                field = new IntroduceFieldFix(h, info.getCsmFile(), guessedName, duplicatesForConstant.size() + 1, initilizeIn, statik, allowFinalInCurrentMethod);
//
...
//
                List<CsmType> paramTypes = new LinkedList<CsmType>();
                List<String> paramNames = new LinkedList<String>();
//
//                for (VariableElement ve : scanner.usedLocalVariables) {
//                    paramTypes.add(CsmType.create(ve.asType()));
...
//
                Set<CsmType> exceptionHandles = new HashSet<CsmType>();
//
//                for (TypeMirror tm : exceptions) {
//                    exceptionHandles.add(CsmType.create(tm));
//                }
//
...
//
//        if (!fixes.isEmpty()) {
//            int pos = CaretAwareCsmFileTaskFactory.getLastPosition(info.getFileObject());
...
//    static Fix computeIntroduceMethod(CsmContext info, int start, int end, Map<IntroduceKind, Fix> fixesMap, Map<IntroduceKind, String> errorMessage, AtomicBoolean cancel) {
...
//
//        List<CsmType> additionaLocalTypes = new LinkedList<CsmType>();
//        List<String> additionaLocalNames = new LinkedList<String>();
//
//        for (VariableElement ve : additionalLocalVariables) {
//            additionaLocalTypes.add(CsmType.create(ve.asType()));
...
//        }
//
//        return new IntroduceMethodFix(info.getCsmFile(), h, paramTypes, paramNames, additionaLocalTypes, additionaLocalNames, CsmType.create(returnType), returnName, declareVariableForReturnValue, exceptionHandles, exits, exitsFromAllBranches, statements[0], statements[1]);
//    }
...
//
//            return checkConstantExpression(info, new TreePath(path, bt.getLeftOperand())) && checkConstantExpression(info, new TreePath(path, bt.getRightOperand()));
//        }
//
...
//        while (statementPath != null && (!StatementTree.class.isAssignableFrom(statementPath.getLeaf().getKind().asInterface()) || (statementPath.getParentPath() != null && statementPath.getParentPath().getLeaf().getKind() != Kind.BLOCK))) {
...
//            }
//
//            if (path.getLeaf().getKind() == Kind.BLOCK && path.getParentPath() != null && path.getParentPath().getLeaf().getKind() == Kind.CLASS) {
//                //initializer:
//                return path;
//            }
//
...
//    }
//
//    private static List<VariableTree> createVariables(WorkingCopy copy, List<CsmType> parameterTypes, List<String> parameterNames) {
//        final TreeMaker make = copy.getTreeMaker();
//        List<VariableTree> formalArguments = new LinkedList<VariableTree>();
//        Iterator<CsmType> argType = parameterTypes.iterator();
...
//            CsmVisibility formalArgMods = EnumSet.noneOf(Modifier.class);
//
...
//    }
//
//    private static List<ExpressionTree> typeHandleToTree(WorkingCopy copy, Set<CsmType> thrownTypes) {
//        final TreeMaker make = copy.getTreeMaker();
//        List<ExpressionTree> thrown = new LinkedList<ExpressionTree>();
//
//        for (CsmType h : thrownTypes) {
...
//            final CsmVisibility access = kind == IntroduceKind.CREATE_CONSTANT ? panel.getAccess() : null;
//            csmFile.runModificationTask(new Task<WorkingCopy>() {
//
...
//                            }
//
//                            CsmVisibility localAccess = EnumSet.of(Modifier.FINAL, Modifier.STATIC);
//
...
//
//                    CsmVisibility mods = declareFinal ? EnumSet.of(Modifier.FINAL) : EnumSet.noneOf(Modifier.class);
//
//                    if (statik) {
...
//
//                    Iterator<CsmType> additionalType = additionalLocalTypes.iterator();
...
//
//                    CsmVisibility modifiers = EnumSet.noneOf(Modifier.class);
//
//                    if (isStatic) {
