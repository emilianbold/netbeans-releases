...
final class ChooseDependencyKindWizardPanel implements WizardDescriptor.Panel<Map<String,Object>>, ChangeListener {
    ...
    public void readSettings(Map<String, Object> settings) {
        getComponent();
        WizardDepKind kind = (WizardDepKind) settings.get(PROP_DEP_KIND);
        component.setDependencyKind(kind);
        JCProject p = (JCProject) settings.get(AddDependencyWizardIterator.PROP_TARGET_PROJECT);
        ProjectKind pkind = p.kind();
        component.setJavacard3ArtifactsVisible (!pkind.isClassic());
    }
