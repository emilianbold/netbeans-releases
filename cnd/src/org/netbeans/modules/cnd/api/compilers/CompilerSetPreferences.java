...
final class CompilerSetPreferences {
    ...
    static void saveToDisk(CompilerSetManager manager) {
        if (!manager.getCompilerSets().isEmpty() && manager.getPlatform() != PlatformTypes.PLATFORM_NONE) {
    ...
    static CompilerSetManager restoreFromDisk(ExecutionEnvironment env) {
    ...
    private static CompilerSetManager restoreFromDisk10() {
        ...
        for (int setCount = 0; setCount < noSets; setCount++) {
            ...
            CompilerSetManager.completeCompilerSet(ExecutionEnvironmentFactory.getLocal(), cs, css);
            css.add(cs);
        }
        CompilerSetManager csm = new CompilerSetManager(ExecutionEnvironmentFactory.getLocal(),  css, CompilerSetUtils.computeLocalPlatform());
        return csm;
    }
