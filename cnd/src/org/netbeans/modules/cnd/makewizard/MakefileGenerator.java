...
public class MakefileGenerator {
	...
	if (initialize()) {
		...
		for (int i = 0; i < tlist.size(); i++) {
		    generateTarget((TargetData) tlist.get(i));
		}
	...
	if (out != null) {
	    ArrayList tlist = md.getTargetList();
	    String cwd = md.getBaseDirectory(MakefileData.EXPAND);
	    HashSet mkhash = new HashSet(tlist.size(), 1.0F);
	    String dir;
	...
	*/
	...
	while (iter.hasNext()) {
		...
		if (entry.charAt(0) == 'U') {
		    String dep = (String) moduleList.get(entry.substring(1));
	...
	return (String[]) neu.toArray(new String[neu.size()]);
    }
	...
	public final void addDependsOn(String dep) {
		...
		if (dep.equals(dependsOn.get(i).toString())) {
	...
	public final void setDependsOn(ArrayList dependsOn) {
