...
public class NavigationView extends ChoiceView {
    ...
    private void checkCursor() {
        ...
        if (jEditorPane != null) {
            ...
            if (caret.getDot() != lastCursorPos && caret.getDot() == lastCursorPosWhenChecked) {
                ...
                int index = Collections.<IndexLineNumber>binarySearch(indexLineNumber, new IndexLineNumber(0, caretLineNo), new IndexLineNumberComparator());
                if (index < 0) {
                    // exact line not found, but insersion index (-1) returned instead
                    index = -index - 2;
                }

                bestIndex = (indexLineNumber.get(index)).getIndex();
    ...
    class MyCtagsTokenListener implements CtagsTokenListener {
        ...
        MyCtagsTokenListener() {
            nodes = new ArrayList<ViewNode>(5);
        ...
        ViewNode[] getNodes() {
            Collections.<ViewNode>sort(nodes, new NodesComparator());
            return nodes.toArray(new ViewNode[nodes.size()]);
        }

        private ArrayList<IndexLineNumber> getIndexLineNumbers() {
            lineNumberIndex = new ArrayList<IndexLineNumber>(5);
            for (int i = 0; i < nodes.size(); i++) {
                lineNumberIndex.add(new IndexLineNumber(i, (nodes.get(i)).getLineNo()));
            }
            Collections.<IndexLineNumber>sort(lineNumberIndex, new IndexLineNumberComparator());
