...
class ConfigurationXMLCodec extends CommonConfigurationXMLCodec {
	...
	if (element.equals(CONF_ELEMENT)) {
	    ...
	    ((MakeConfiguration)currentConf).getMakefileConfiguration().getBuildCommandWorkingDir().setValue(currentText);
	}
	    ...
	    ((MakeConfiguration)currentConf).getMakefileConfiguration().getOutput().setValue(currentText);
	}
		...
		currentIncludeDirectories.add(currentText);
	}
