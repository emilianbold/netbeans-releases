 ...
 * @author treyspiva
 */
public class AssociationClassConnectProvider extends SceneConnectProvider
{
    
    public AssociationClassConnectProvider(String defaultType)
    {
        super(defaultType, "");
    }
    ...
    public ConnectorState isTargetWidget(Widget sourceWidget, Widget targetWidget)
    {
        ...
        if(sourceWidget.getScene() instanceof ObjectScene)
        {
            ...
            if((source != null) && (target != null))
            {
                ...
                if(sourceElement instanceof IClassifier)
                {
                    if(targetElement instanceof IClassifier)
                    {
    ...
    public void createConnection(Widget sourceWidget, Widget targetWidget)
    {
        if(sourceWidget.getScene() instanceof GraphScene)
        {
            ...
            if((source != null) && (target != null))
            {
                ...
                IClassifier soruceClassifier = null;
                IClassifier targetClassifier = null;
                
                if(sourceElement instanceof IClassifier)
                {
                    soruceClassifier = (IClassifier)sourceElement;
                    if(targetElement instanceof IClassifier)
                    {
                       targetClassifier = (IClassifier)targetElement;
                    }
                }
                
                if((soruceClassifier != null) && (targetClassifier != null))
                {
                    IRelationFactory factory = new org.netbeans.modules.uml.core.metamodel.infrastructure.RelationFactory();
                    IAssociationClass assocClass = 
                            factory.createAssociationClass(soruceClassifier, 
                                                           targetClassifier, 
                                                           AssociationKindEnum.AK_ASSOCIATION_CLASS, 
                                                           false, 
                                                           false, 
                                                           soruceClassifier.getOwningPackage());
                    
                    IPresentationElement edge = createNodePresentationElement(assocClass);
