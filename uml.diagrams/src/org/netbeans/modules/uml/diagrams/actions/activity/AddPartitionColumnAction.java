 ...
 * @author Thuy
 */
public class AddPartitionColumnAction extends AbstractAction implements IPartitionAction
{

    protected ActivityPartitionWidget mainWidget;

    public AddPartitionColumnAction(ActivityPartitionWidget widget)
    {
        super();
        mainWidget = widget;
    }
    ...
    {
        return NbBundle.getMessage(AddPartitionColumnAction.class,
                                   "CTL_AddPartitionColumn");
    }
    ...
    {
        System.out.println("handle add partition");
        int count = 1;
        if (mainWidget != null)
        {
            updateOrientation();
            IActivityPartition parentPartition = mainWidget.getParentPartition();
            ETList<IActivityPartition> subPartitions = parentPartition.getSubPartitions();
            if (subPartitions == null || subPartitions.size() == 0)
            {
                count = 2;
                mainWidget.removeSubPartitionWidgets();
            }
            while (count-- > 0)
            {
                ...
                if (subPartition != null)
                {
                    ...
                    mainWidget.addSubPartition(subPartition);
                }
            }
        }
    }
