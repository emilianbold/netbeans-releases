...
//        getScene().validate();
    ...
    public INamedElement processDrop(INamedElement elementToDrop) {
        ...
        if(ret.getNamespace()==null && ret.getOwner()==null && ns!=null)ns.addOwnedElement(ret);//need to add here as a fix for combined fragment initialization problem below caused by not set of namespace in accept on containers before call to processing, set default ns to diagram ns
