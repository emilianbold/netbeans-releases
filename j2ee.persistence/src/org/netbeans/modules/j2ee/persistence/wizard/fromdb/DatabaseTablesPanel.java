...
public class DatabaseTablesPanel extends javax.swing.JPanel {
    ...
    public void initialize(Project project, DBSchemaFileList dbschemaFileList, PersistenceGenerator persistenceGen, TableSource tableSource, FileObject targetFolder) {
        ...
        if (enabled) {
            ...
            if (withDatasources) {
                ...
                initializeWithDatasources(provider);
            } else {
            ...
            selectDefaultTableSource(tableSource, provider, withDatasources, project, targetFolder);
        } else {
    ...
    private void initializeWithDatasources(J2eeModuleProvider provider) {
     ...
     */
    private void selectDefaultTableSource(TableSource tableSource, J2eeModuleProvider provider, boolean withDatasources, Project project, FileObject targetFolder) {
        if (tableSource == null) {
            ...
            switch (tableSource.getType()) {
                case DATA_SOURCE:
                    ...
                    if (withDatasources) {
                        if (selectDatasource(provider, tableSourceName)) {
                            return;
                        }
                    }
                    break;
     ...
     */
    private boolean selectDatasource(J2eeModuleProvider provider, String jndiName) {
        Datasource datasource = DatasourceHelper.findDatasource(provider, jndiName);
        if (datasource == null) {
            return false;
        }
    ...
    private void updateSourceSchema() {
        ...
        if (datasourceRadioButton.isSelected()) {
            ...
            if (item instanceof Datasource) {
                ...
                List<DatabaseConnection> dbconns = DatasourceHelper.findDatabaseConnections(ds);
                if (dbconns.size() > 0) {
