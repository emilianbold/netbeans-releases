...
import org.netbeans.modules.dlight.terminal.api.IOResizable.Listener;
...
public final class TerminalInputOutput implements InputOutput, Lookup.Provider {
    ...
    private final Map<OutputType, Color> typeColorMap = new EnumMap<OutputType, Color>(OutputType.class);
    ...
    public TerminalInputOutput(String name, IOContainer ioContainer) {
        this.ioContainer = ioContainer;
        terminal = TerminalProvider.getDefault().createTerminal(name, ioContainer);
        term = terminal.term();

        lookup = Lookups.fixed(new MyIOColorLines(),
                new MyIOColors(),
                new MyIOPosition(),
                new MyIOResizable(),
                new MyIOEmulation(),
