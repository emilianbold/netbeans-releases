...
public final class EngineDescriptor {
    ...
    public EngineDescriptor(EngineType et) {
        assert et != null;
        this.et = et;
    }

    public boolean hasCapability(EngineCapability capability) {
        Collection<? extends EngineCapabilityProvider> services = Lookup.getDefault().lookupAll(EngineCapabilityProvider.class);
        for (EngineCapabilityProvider engineCapabilityProvider : services) {
            if (engineCapabilityProvider.hasCapability(et, capability)) {
                return true;
            }
        }
        return false;
