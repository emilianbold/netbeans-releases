...
public final class Host implements Record, OptionSetOwner {
	...
	if (hostName.equals(localhost))		// NOI18N
	    return false;
        if (hostName.equals("127.0.0.1")) // NOI18N
            return false;
	if (hostName.equals(getLocalHost()))	// NOI18N
	    return false;
	return true;
    }

    public static boolean isRemote(Host host) {
	if (host == null)
	    return false;
	if (IpeUtils.isEmpty(host.getHostName()))
	    return false;
	if (host.getHostName().equals(localhost))		// NOI18N
	    return false;
        //FIXME - in NB6.8 remote we have 127.0.0.1 as a localhost
        if (host.getHostName().equals("127.0.0.1")) // NOI18N
            return false;
	if (host.getHostName().equals(getLocalHost()))	// NOI18N
	    return false;
	return true;
        }

    public Host(OptionSet options) {
	...
	Executor executor = Executor.getDefault("platform", this, 0); // NOI18N
	return executor.svc_is64();
    }
    ...
    // interface Record
    public Host cloneRecord() {
	return new Host(options.makeCopy());
    }
