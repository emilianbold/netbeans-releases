...
public class GruntCustomizerPanel extends JPanel {

    private final ProjectCustomizer.Category category;
    private final GruntPreferences preferences;
    private final List<GruntTask> gruntTasks = new CopyOnWriteArrayList<>();


    GruntCustomizerPanel(ProjectCustomizer.Category category, Project project) {
        assert EventQueue.isDispatchThread();
        assert category != null;
        assert project != null;

        this.category = category;
        preferences = GruntBuildTool.forProject(project).getGruntPreferences();
    ...
    private void init() {
        gruntTasks.add(new GruntTask(ActionProvider.COMMAND_BUILD, "build", buildCheckBox, buildTextField)); // NOI18N
        gruntTasks.add(new GruntTask(ActionProvider.COMMAND_CLEAN, "clean", cleanCheckBox, cleanTextField)); // NOI18N
    ...
    @NbBundle.Messages("GruntCustomizerPanel.error.task.empty=Grunt task cannot be empty")
    void validateData() {
        assert EventQueue.isDispatchThread();
        for (GruntTask gruntTask : gruntTasks) {
            String text = gruntTask.getText();
            if (text != null
                    && text.isEmpty()) {
                category.setErrorMessage(Bundle.GruntCustomizerPanel_error_task_empty());
    ...
    private void initComponents() {
        ...
        Mnemonics.setLocalizedText(assignLabel, NbBundle.getMessage(GruntCustomizerPanel.class, "GruntCustomizerPanel.assignLabel.text")); // NOI18N

        Mnemonics.setLocalizedText(buildCheckBox, NbBundle.getMessage(GruntCustomizerPanel.class, "GruntCustomizerPanel.buildCheckBox.text")); // NOI18N

        Mnemonics.setLocalizedText(cleanCheckBox, NbBundle.getMessage(GruntCustomizerPanel.class, "GruntCustomizerPanel.cleanCheckBox.text")); // NOI18N

        Mnemonics.setLocalizedText(rebuildCheckBox, NbBundle.getMessage(GruntCustomizerPanel.class, "GruntCustomizerPanel.rebuildCheckBox.text")); // NOI18N
    ...
    private final class GruntTask {
        ...
        GruntTask(String commandId, String defaultValue, JCheckBox checkBox, JTextField textField) {
