...
public class PhpProjectUtils {
    ...
    public static String getRelativeSrcPath(Project phpProject, 
            FileObject fileObject) 
    {
        if ( fileObject.equals(phpProject.getProjectDirectory() )) {
            return "";
        }
        FileObject[] sources = getSourceObjects(phpProject);
        for (FileObject source : sources) {
            if (FileUtil.isParentOf(source, fileObject)) {
                return FileUtil.getRelativePath(source, fileObject);
            }
            else if( source.equals( fileObject )) {
                return "";
            }
        }
        return null;
