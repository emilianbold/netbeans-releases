 ...
 * @author avk
 */
public class WebConfigNode extends LocalFileNode{

    public WebConfigNode(LocalHostImpl host) {
        super(host, getConfigFile(host));
        
        setDisplayName( getFile().getName() );
        setName(getDisplayName());
        setShortDescription( getFile().getPath() );
        setIconBaseWithExtension(ResourceMarker.getLocation()
                + ResourceMarker.CONFIG_ICON);
    }

    
    private static File getConfigFile(LocalHostImpl host){
        assert host != null;
