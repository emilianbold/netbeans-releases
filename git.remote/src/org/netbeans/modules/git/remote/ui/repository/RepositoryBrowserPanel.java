...
import org.netbeans.modules.git.remote.ui.branch.DeleteBranchAction;
...
import org.netbeans.modules.git.remote.ui.history.SearchIncomingAction;
import org.netbeans.modules.git.remote.ui.history.SearchOutgoingAction;
import org.netbeans.modules.git.remote.ui.merge.MergeRevisionAction;
import org.netbeans.modules.git.remote.ui.push.PushAction;
import org.netbeans.modules.git.remote.ui.push.PushMapping;
import org.netbeans.modules.git.remote.ui.push.PushToUpstreamAction;
import org.netbeans.modules.git.remote.ui.repository.remote.RemoveRemoteConfig;
import org.netbeans.modules.git.remote.ui.stash.ApplyStashAction;
...
import org.netbeans.modules.git.remote.ui.tag.ManageTagsAction;
...
public class RepositoryBrowserPanel extends JPanel implements Provider, PropertyChangeListener, ListSelectionListener,
    ...
    public RepositoryBrowserPanel () {
        this(EnumSet.complementOf(EnumSet.of(Option.DISPLAY_REVISIONS, Option.EXPAND_BRANCHES, Option.EXPAND_TAGS)),
                null, new VCSFileProxy[0], null);
    }

    public RepositoryBrowserPanel (final EnumSet<Option> options, VCSFileProxy repository, VCSFileProxy[] roots, RepositoryInfo info) {
    ...
    private VCSFileProxy lookupRepository (Node selectedNode) {
    ...
    public void selectRepository (VCSFileProxy repository) {
        Node[] nodes = root.getChildren().getNodes();
        for (Node node : nodes) {
            if (node instanceof RepositoryNode && repository.equals(node.getLookup().lookup(VCSFileProxy.class))) {
    ...
    private abstract class RepositoryBrowserNode extends AbstractNode {
        ...
        protected RepositoryBrowserNode (Children children, VCSFileProxy repository) {
            this(children, repository, null);
        }

        protected RepositoryBrowserNode (Children children, VCSFileProxy repository, Lookup lookup) {
    ...
    private class RepositoriesChildren extends Children.SortedMap<VCSFileProxy> {
        ...
        public RepositoriesChildren () {
            setComparator(new Comparator<Node>() {
                ...
                public int compare (Node o1, Node o2) {
                    ...
                    if (o1 instanceof RepositoryNode && o2 instanceof RepositoryNode) {
                        VCSFileProxy repo1 = ((RepositoryNode) o1).getRepository();
                        VCSFileProxy repo2 = ((RepositoryNode) o2).getRepository();
                        result = repo1.getName().compareTo(repo2.getName());
                        if (result == 0 && !repo1.equals(repo2)) {
                            result = repo1.getPath().compareTo(repo2.getPath());
                        }
                    } else {
            ...
            GitRepositories.getInstance().addPropertyChangeListener(WeakListeners.propertyChange(list = new PropertyChangeListener() {
                ...
                public void propertyChange (PropertyChangeEvent evt) {
                    final Set<VCSFileProxy> oldValues = (Set<VCSFileProxy>) evt.getOldValue();
                    final Set<VCSFileProxy> newValues = (Set<VCSFileProxy>) evt.getNewValue();
        ...
        protected void addNotify() {
            ...
            if (!initialized) {
                // initialize keys
                initialized = true;
                list.propertyChange(new PropertyChangeEvent(GitRepositories.getInstance(), GitRepositories.PROP_REPOSITORIES, Collections.<VCSFileProxy>emptySet(), GitRepositories.getInstance().getKnownRepositories()));
            }
        }
    }
    ...
    private class RepositoryNode extends RepositoryBrowserNode implements PropertyChangeListener {
        ...
        public RepositoryNode (final VCSFileProxy repository, RepositoryInfo info) {
            super(new RepositoryChildren(), repository);
            this.repository = repository;
            setIconBaseWithExtension("org/netbeans/modules/git/remote/resources/icons/repository.png"); //NOI18N
            if (info == null) {
    ...
    private class BranchesTopNode extends RepositoryBrowserNode {
        ...
        public BranchesTopNode (VCSFileProxy repository) {
            super(new BranchesTopChildren(repository), repository);
            setIconBaseWithExtension("org/netbeans/modules/git/remote/resources/icons/branches.png"); //NOI18N
        }
    ...
    private class BranchesNode extends RepositoryBrowserNode {
        ...
        private BranchesNode (VCSFileProxy repository, BranchNodeType type, Map<String, GitBranchInfo> branches) {
    ...
    private class BranchesChildren extends Children.Keys<GitBranchInfo> {
        ...
        protected Node[] createNodes (GitBranchInfo key) {
            ...
            VCSFileProxy repository = ((RepositoryNode) node).getRepository();
    ...
    private class BranchNode extends RepositoryBrowserNode {
        ...
        public BranchNode (VCSFileProxy repository, GitBranchInfo branchInfo) {
            ...
            setIconBaseWithExtension("org/netbeans/modules/git/remote/resources/icons/branch.png"); //NOI18N
            RepositoryInfo info = RepositoryInfo.getInstance(repository);
            if (info == null) {
        ...
        private void refreshActiveBranch (GitBranch activeBranch) {
            ...
            if (active != oldActive) {
                setIconBaseWithExtension("org/netbeans/modules/git/remote/resources/icons/" + (active ? "active_branch" : "branch") + ".png"); //NOI18N
            }
                ...
                actions.add(new AbstractAction(NbBundle.getMessage(SearchHistoryAction.class, "LBL_SearchHistoryAction_PopupName")) { //NOI18N
                    ...
                    public void actionPerformed (ActionEvent e) {
                        EventQueue.invokeLater(new Runnable() {
                            @Override
                            public void run () {
                                SearchHistoryAction.openSearch(repo, new VCSFileProxy[] { repo }, branch,
                                        VCSFileProxySupport.getContextDisplayName(GitUtils.getContextForFile(repo)));
                            }
                        });
                    }
                });
                    ...
                    actions.add(new AbstractAction(NbBundle.getMessage(SearchIncomingAction.class, "LBL_SearchIncomingAction_PopupName")) { //NOI18N
                        ...
                        public void actionPerformed (ActionEvent e) {
                            Utils.post(new Runnable () {
                                ...
                                @Override
                                public void run () {
                                    SystemAction.get(SearchIncomingAction.class).openSearch(repo, new VCSFileProxy[] { repo },
                        ...
                        actions.add(new AbstractAction(NbBundle.getMessage(SearchOutgoingAction.class, "LBL_SearchOutgoingAction_PopupName")) { //NOI18N
                            ...
                            public void actionPerformed (ActionEvent e) {
                                Utils.post(new Runnable () {
                                    ...
                                    @Override
                                    public void run () {
                                        SystemAction.get(SearchOutgoingAction.class).openSearch(repo, new VCSFileProxy[] { repo },
                                                branch, getContextDisplayName(repo));
                                    }
        ...
        private void refreshTracking (final GitBranch trackedBranch, final VCSFileProxy repository) {
    ...
    private class TagsNode extends RepositoryBrowserNode {
        ...
        public TagsNode (VCSFileProxy repository) {
            super(new TagChildren(repository), repository);
            assert repository != null;
            setIconBaseWithExtension("org/netbeans/modules/git/remote/resources/icons/tags.png"); //NOI18N
        }
    ...
    private class TagNode extends RepositoryBrowserNode {
        ...
        public TagNode (VCSFileProxy repository, GitTag tag) {
            ...
            setIconBaseWithExtension("org/netbeans/modules/git/remote/resources/icons/tag.png"); //NOI18N
            RepositoryInfo info = RepositoryInfo.getInstance(repository);
            if (info == null) {
    ...
    private class StashesNode extends RepositoryBrowserNode {
        ...
        public StashesNode (VCSFileProxy repository) {
            super(new StashesChildren(repository), repository);
            assert repository != null;
            setIconBaseWithExtension("org/netbeans/modules/git/remote/resources/icons/stashes.png"); //NOI18N
        }
    ...
    private class StashesChildren extends Children.Keys<Stash> implements PropertyChangeListener {
        ...
        private StashesChildren (VCSFileProxy repository) {
            this.repository = repository;
            RepositoryInfo info = RepositoryInfo.getInstance(repository);
            if (info == null) {
    ...
    private class StashNode extends RepositoryBrowserNode {
        ...
        public StashNode (VCSFileProxy repository, Stash item) {
            super(Children.LEAF, repository);
            this.item = item;
            setIconBaseWithExtension("org/netbeans/modules/git/remote/resources/icons/stash.png"); //NOI18N
        }
...
//                });
    ...
    private class RemotesNode extends RepositoryBrowserNode {
        ...
        public RemotesNode (VCSFileProxy repository) {
    ...
    private class AllRemotesChildren extends Children.Keys<GitRemoteConfig> implements PropertyChangeListener {
        ...
        private AllRemotesChildren (VCSFileProxy repository) {
            this.repository = repository;
            RepositoryInfo info = RepositoryInfo.getInstance(repository);
            if (info == null) {
    ...
    private class RemoteNode extends RepositoryBrowserNode {
        ...
        public RemoteNode (VCSFileProxy repository, GitRemoteConfig remote) {
            super(new RemoteChildren(remote), repository, Lookups.fixed(remote));
            this.repository = repository;
            this.remoteName = remote.getRemoteName();
            setIconBaseWithExtension("org/netbeans/modules/git/remote/resources/icons/remote.png"); //NOI18N
        }
    ...
    private class RemoteUriNode extends RepositoryBrowserNode {
        ...
        public RemoteUriNode (VCSFileProxy repository, RemoteUri uri, GitRemoteConfig remote) {
            super(Children.LEAF, repository);
            this.uri = uri;
            this.remote = remote;
            setIconBaseWithExtension("org/netbeans/modules/git/remote/resources/icons/remote_" + (uri.push ? "push" : "fetch") + ".png"); //NOI18N
        }
    ...
    private static String getContextDisplayName (final VCSFileProxy repo) {
        return VCSFileProxySupport.getContextDisplayName(GitUtils.getContextForFile(repo));
    }
    ...
    final org.netbeans.modules.git.remote.ui.repository.RevisionListPanel revisionsPanel1 = new org.netbeans.modules.git.remote.ui.repository.RevisionListPanel();
    private final org.netbeans.modules.git.remote.ui.repository.ControlToolbar toolbar = new org.netbeans.modules.git.remote.ui.repository.ControlToolbar();
