...
//        RSYNC("rsync", NbBundle.getMessage(RemoteRepository.class, "Scheme.RSYNC")), //NOI18N
    ...
    public RemoteRepository(FileSystem fs, String forPath) {
        this(fs, null, forPath);
    }

    public RemoteRepository(FileSystem fs, PasswordAuthentication pa, String forPath) {
        this(fs, pa, forPath, false);
    }
    
    private RemoteRepository(FileSystem fs, PasswordAuthentication pa, String forPath, boolean fixedUrl) {
        assert fs != null;
    ...
    public static boolean updateFor (String url, VCSFileProxy root) {
        boolean retval = false;
        final RemoteRepository repository = new RemoteRepository(VCSFileProxySupport.getFileSystem(root), null, url, true);
    ...
    private void onBrowse() {
        ...
        try {
            ...
            if (uri.isAbsolute() && "file".equalsIgnoreCase(uri.getScheme())) { //NOI18N
                ...
                file = VCSFileProxySupport.getResource(fileSystem, comboEditor.getText());
            }
        } catch (URISyntaxException ex) {
            //
        }
    ...
    private final class SSHConnectionSettingsType extends ConnectionSettingsType implements ActionListener {
        ...
        private void onBrowse() {
            ...
            VCSFileProxy file = VCSFileProxySupport.getResource(fileSystem, path);
            JFileChooser fileChooser = VCSFileProxySupport.createFileChooser(path.isEmpty() ? file : file.getParentFile());
            ...
            if (JFileChooser.APPROVE_OPTION == fileChooser.showDialog(panel, null)) {
                VCSFileProxy f = VCSFileProxySupport.getSelectedFile(fileChooser);
                settingsPanel.txtIdentityFile.setText(f.getPath());
            }
        }
        ...
        private String getDefaultIdentityFilePath () {
            String identityFile = VCSFileProxySupport.getHome(VCSFileProxy.createFileProxy(fileSystem.getRoot())).getPath() + "/" + ".ssh" + "/" + "id_dsa"; //NOI18N
            return identityFile;
        }
