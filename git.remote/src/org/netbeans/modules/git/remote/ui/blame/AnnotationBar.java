...
import org.netbeans.modules.git.remote.ui.checkout.CheckoutPathsAction;
...
final class AnnotationBar extends JComponent implements Accessible, PropertyChangeListener, DocumentListener, ChangeListener, ActionListener, Runnable, ComponentListener {
     ...
     * Holdes kenai users
     */
    private final Map<String, KenaiUser> kenaiUsersMap = null;
...
//        }
    ...
    private JPopupMenu createPopup(MouseEvent e) {
        ...
        final VCSFileProxy originalFile = al == null ? null : al.getFile();
    ...
    private String getKeyFor (VCSFileProxy file, String revision) {
        return file.getPath() + "#" + revision; //NOI18N
    }
    ...
    private class PreviousRevisionInvoker extends GitProgressSupport.NoOutputLogging {
        ...
        private PreviousRevisionInvoker (VCSFileProxy originalFile, GitRevisionInfo revisionPerLine) {
        ...
        private String getParentRevision (VCSFileProxy file, GitRevisionInfo revision) {
    ...
    private void checkout (final VCSFileProxy file, String revision) {
        final VCSFileProxy root = Git.getInstance().getRepositoryRoot(file);
        if(root == null) {
            return;
        }
        
        VCSFileProxy[] files = new VCSFileProxy [1];
        files[0] = file; 
        SystemAction.get(CheckoutPathsAction.class).checkoutFiles(repositoryRoot, new VCSFileProxy[] { file }, revision);
    }
