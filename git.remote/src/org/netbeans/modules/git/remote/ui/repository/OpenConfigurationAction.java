 ...
 */
@ActionID(id = "org.netbeans.modules.git.remote.ui.repository.OpenConfigurationAction", category = "GitRemote")
...
public class OpenConfigurationAction extends MultipleRepositoryAction {
    ...
    protected RequestProcessor.Task performAction (VCSFileProxy repository, VCSFileProxy[] roots, VCSContext context) {
        VCSFileProxy config = VCSFileProxy.createFileProxy(GitUtils.getGitFolderForRoot(repository), "config"); //NOI18N
        if (VCSFileProxySupport.canRead(config)) {
            VCSFileProxySupport.openFile(config);
        }
        return null;
    }
