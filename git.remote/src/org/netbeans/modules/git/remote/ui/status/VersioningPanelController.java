...
import org.netbeans.modules.git.remote.ui.commit.IncludeInCommitAction;
...
class VersioningPanelController implements ActionListener, PropertyChangeListener, PreferenceChangeListener {
    ...
    private static final RequestProcessor RP = new RequestProcessor("GitRemoteVersioningWindow", 1, true); //NOI18N
    ...
    JPopupMenu getPopupFor (final Node[] nodes) {
        ...
        VCSFileProxy[] files = toFiles(nodes);
        JMenuItem item;
    ...
    private boolean affectsView (FileStatusCache.ChangedEvent changedEvent) {
        ...
        if (oldInfo == null) {
            ...
            if (!oldInfo.containsStatus(displayStatuses) && !newInfo.containsStatus(displayStatuses)) {
                return false;
            }
        }
    ...
    private VCSFileProxy[] toFiles (Node[] nodes) {
        List<VCSFileProxy> files = new ArrayList<>(nodes.length);
        for (Node n : nodes) {
            VCSFileProxy f = n.getLookup().lookup(VCSFileProxy.class);
            if (f != null) {
                files.add(f);
            }
        }
        ...
        public void run() {
            ...
            for (VCSFileProxy f : interestingFiles) {
                ...
                if (root != null) {
                    if (f.equals(root)) {
                        // gitlink???
                        VCSFileProxy parentFile = f.getParentFile();
                        VCSFileProxy parentRepository = parentFile == null ? null : git.getRepositoryRoot(parentFile);
    ...
    private class ApplyChangesTask implements Runnable {
        ...
        public void run() {
            ...
            Git git = Git.getInstance();
            final Map<VCSFileProxy, GitStatusNodeImpl> nodes = Mutex.EVENT.readAccess(new Mutex.Action<Map<VCSFileProxy, GitStatusNodeImpl>>() {
                @Override
                public Map<VCSFileProxy, GitStatusNodeImpl> run() {
            ...
            for (FileStatusCache.ChangedEvent evt : events) {
                ...
                if (newInfo.containsStatus(displayStatuses)) {
                    if (node != null) {
                        ...
                        VCSFileProxy root = git.getRepositoryRoot(evt.getFile());
                        if (root != null) {
                            // gitlink???
                            VCSFileProxy parentFile = evt.getFile().getParentFile();
                            VCSFileProxy parentRepository = parentFile == null ? null : git.getRepositoryRoot(parentFile);
                            if (parentRepository != null) {
                                root = parentRepository;
                            }
