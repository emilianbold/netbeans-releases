...
package org.netbeans.modules.visualweb.test.components.output.message;
...
public class AcceptanceTest extends RaveTestCase {
    ...
    public String _projectName = "MessageComponentTests";
	...
	log(">> Add textField");
        ...
        palette.addComponent("Text Field", designer, new Point(20, 40));
        try { Thread.sleep(2000); } catch(Exception e) {}

	log(">> Set text id");
        DocumentOutlineOperator doo = new DocumentOutlineOperator(Util.getMainWindow());
        Util.wait(2000);
        String path = _outlineForm1 + "textField1";
        doo.verify();
        doo.selectPath(path);
        Util.wait(5000); 
        sheet = new SheetTableOperator();
		sheet.setTextValue("id", "name");
/*        int row = sheet.findCellRow("id");
        sheet.clickForEdit(row, 1);
        sheet.clickForEdit(row, 1);
        new JTextComponentOperator(sheet).enterText("name");
        try { Thread.sleep(2000); } catch(Exception e) {}
*/
        
	log(">> Add message");
        palette.addComponent("Message", designer, new Point(250, 40));
        path = _outlineForm1 + "message1";
        doo.verify();
        doo.selectPath(path);

	log(">> set message for property");
        sheet = new SheetTableOperator();
        sheet.setTextValue("id", "msg1");
        sheet.setComboBoxValue("for", "name");
        Util.wait(2000);
        endTest();
