...
package org.netbeans.modules.visualweb.test.components.converters.numberconverter;
...
public class AcceptanceTest extends RaveTestCase {
    ...
    public String _privateBundle ="org.netbeans.modules.visualweb.test.components.converters.numberconverter.Acceptance";
    ...
    public String _numberFormat = Bundle.getStringTrimmed(_sharedBundle,"numberFormat");
    ...
    public String _converterNumber = Bundle.getStringTrimmed(_sharedBundle,"converterNumber");
    
    //drop points
    public int _x = Integer.parseInt(Bundle.getStringTrimmed(_privateBundle,"x"));
    public int _number1y = Integer.parseInt(Bundle.getStringTrimmed(_privateBundle,"number1y"));
    ...
    public String _textField1 = Bundle.getStringTrimmed(_privateBundle,"textField1");
    public String _numberConverter1 = Bundle.getStringTrimmed(_privateBundle,"numberConverter1");
    
    //properties
    public String _propertyConverter = Bundle.getStringTrimmed(_sharedBundle,"propertyConverter");
    public String _newNumberConverter = Bundle.getStringTrimmed(_sharedBundle,"newNumberConverter");
    ...
    public static Test suite() {
        ...
        suite.addTest(new AcceptanceTest("testSetNumberConverter"));
        suite.addTest(new AcceptanceTest("testVerifyNumberConverter"));
        //Modal Number Format dialog is blocking testsuite - can't DnD component
        //suite.addTest(new AcceptanceTest("testAddNumberConverter"));
...
//        palette.addComponent(_basicTextField, designer, dropPoint);
    ...
    /*
     *    Set number converter of textfield1
     */
    public void testSetNumberConverter() {
        startTest();
        log("**Initialize");
        DocumentOutlineOperator doo = new DocumentOutlineOperator(Util.getMainWindow());
        Util.wait(2000);

        log("**Select textfield1 from Outline window");
        String path = _outlineForm1 + _textField1;
        doo.verify();
//        doo.clickOnPath(path);
        doo.selectPath(path);
        Util.wait(5000);       
        
        log("**Set converter property to (New NumberConverter)");
        sheet = new SheetTableOperator();
        sheet.setComboBoxValue(_propertyConverter, _newNumberConverter);
/*
        log("**Initialize");
        designer = new DesignerPaneOperator(RaveWindowOperator.getDefaultRave());
        ...
        log("Close number format dialog");
        JDialogOperator numberFormat = new JDialogOperator(_numberFormat);
        JButtonOperator okButton = new JButtonOperator(numberFormat, _ok);
 ...
 */      
    ...
    /*
     *    Add a number converter to designer
     */
    public void testAddNumberConverter() {
        startTest();
        log("**Initialize");
        designer = new DesignerPaneOperator(RaveWindowOperator.getDefaultRave());
        palette = new PaletteContainerOperator(_converterPalette);
        ...
        log("**Add number converter to designer");
        clickPoint = palette.getClickPoint(_converterNumber);
        dropPoint = new Point(_x, _number1y);
        palette.addComponent(_converterNumber, designer, dropPoint);
        try { Thread.sleep(5000); } catch(Exception e) {} //add wait
    ...
    public void testVerifyNumberConverter() {
        startTest();
        log("**Initialize");
        DocumentOutlineOperator doo = new DocumentOutlineOperator(Util.getMainWindow());
        Util.wait(2000);

        log("**Select numberConverter1 from Outline window");
        String path = _outlinePage1 + _numberConverter1;
        doo.verify();
//        doo.clickOnPath(path);
        doo.selectPath(path);
        Util.wait(5000); 
