...
public class SearchField extends JPanel implements ActionListener {
    ...
    private static final String ODS = "ods"; //NOI18N
    ...
    public SearchField(CloudServer server) {
        super();
        selected = server;
        initComponents();
        setTooltip();
    ...
    protected JTextComponent createCommandField() {
        ...
        res.getAccessibleContext().setAccessibleName(NbBundle.getMessage(ODSSearchPanel.class, "ODSSearchPanel.searchLabel.text"));
        res.getAccessibleContext().setAccessibleDescription(NbBundle.getMessage(ODSSearchPanel.class, "ODSSearchPanel.searchLabel.AccessibleContext.accessibleDescription"));
        res.setRows(1);
    ...
    protected void maybeShowPopup(MouseEvent evt) {
        ...
        for (CloudServer odsServer : CloudServerManager.getDefault().getServers()) {
            JRadioButtonMenuItem item = new JRadioButtonMenuItem(getODSDisplayName(odsServer));
            item.setIcon(odsServer.getIcon());
            item.setSelected(odsServer == getSelectedCloud());
            item.putClientProperty(ODS, odsServer);
        ...
        item.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                AddInstanceAction addInstanceAction = new AddInstanceAction(CloudServerProviderImpl.getDefault());
                addInstanceAction.actionPerformed(e);
                CloudUiServer teamServer = (CloudUiServer) addInstanceAction.getTeamServer();
                if (teamServer != null) {
                    CloudServer last = (teamServer).getServer();
                    if (last != null) {
                        selected = last;
                        setTooltip();
    ...
    private void setTooltip() {
        leftIcon.setToolTipText(getODSDisplayName(selected)); // NOI18N
    }

    private static String getODSDisplayName(CloudServer server) {
        if (server == null) {
            return ""; //NOI18N
        }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        Object k = source.getClientProperty(ODS);
        if (k != null) {
            CloudServer toBeSelected = (CloudServer) k;
    ...
    public CloudServer getSelectedCloud() {
        return selected;
    }
}
