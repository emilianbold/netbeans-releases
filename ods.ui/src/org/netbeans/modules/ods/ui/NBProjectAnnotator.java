...
public class NBProjectAnnotator implements ProjectIconAnnotator, PropertyChangeListener {
    ...
    private static final Image kenaiBadge = ImageUtilities.loadImage("org/netbeans/modules/ods/ui/resources/ods-badge.png");
    @Messages("MSG_TeamProject=Part of a Team Project")
    private static final String tooltip = "<img src=\""
            + NBProjectAnnotator.class.getResource("/org/netbeans/modules/ods/ui/resources/ods-badge.png")
            + "\">&nbsp;"
            + Bundle.MSG_TeamProject();
    private final Set<ChangeListener> listeners;
    private final Map<Project, Boolean> odsProjects;
    
    public NBProjectAnnotator () {
        listeners = new LinkedHashSet<ChangeListener>();
        odsProjects = new WeakHashMap<Project, Boolean>(10);
        Utils.getRequestProcessor().post(new Runnable() {
            @Override
            public void run () {
                CloudServerManager.getDefault().addPropertyChangeListener(
                        WeakListeners.propertyChange(NBProjectAnnotator.this, 
                        CloudServerManager.getDefault()));
            }
        });
    }
    ...
    public Image annotateIcon (Project p, Image original, boolean openedNode) {
        Boolean isOds = odsProjects.get(p);
        if (Boolean.TRUE.equals(isOds)) {
            ...
            public void run () {
                ...
                if (s != null) {
                    for (String url : s.split(";")) {
                        if (CloudServer.findServerForRepository(url) == null) {
                            odsProjects.put(p, false);
                        } else {
                            odsProjects.put(p, true);
                            EventQueue.invokeLater(new Runnable() {
                                @Override
                                public void run () {
                                    fireChange();
