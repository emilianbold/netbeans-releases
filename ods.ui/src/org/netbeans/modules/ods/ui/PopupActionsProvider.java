...
class PopupActionsProvider implements PopupMenuProvider {
    ...
    abstract class LazyProjectAction extends AbstractAction {
        ...
        public final void actionPerformed (final ActionEvent e) {
            ...
            Utils.getRequestProcessor().post(new Runnable() {
                ...
                public void run() {
                    try {
                        CloudServer server = CloudServer.findServerForRepository(repositoryUrl);
                        if (server != null) {
                            ...
                            final ODSProject project = ODSProject.findProjectForRepository(repositoryUrl);
                            if (project != null) {
                                if (project.hasTasks()) {
                                    final ProjectHandleImpl pHandle = new ProjectHandleImpl(uiServer, project);
                                    final DefaultDashboard<CloudUiServer, ODSProject> dashboard = uiServer.getDashboard();
        ...
        protected abstract void performAction (DefaultDashboard<CloudUiServer, ODSProject> dashboard, ProjectHandle<ODSProject> pHandle, ActionEvent e);
    }
    ...
    class LazyFindIssuesAction extends LazyProjectAction {
        ...
        @Override
        protected void performAction (DefaultDashboard<CloudUiServer, ODSProject> dashboard, ProjectHandle<ODSProject> pHandle, ActionEvent e) {
            dashboard.getDashboardProvider().getQueryAccessor(ODSProject.class).getFindIssueAction(pHandle).actionPerformed(e);
        }
    }
    ...
    class LazyNewIssuesAction extends LazyProjectAction {
        ...
        @Override
        protected void performAction (DefaultDashboard<CloudUiServer, ODSProject> dashboard, ProjectHandle<ODSProject> pHandle, ActionEvent e) {
            dashboard.getDashboardProvider().getQueryAccessor(ODSProject.class).getCreateIssueAction(pHandle).actionPerformed(e);
        }
