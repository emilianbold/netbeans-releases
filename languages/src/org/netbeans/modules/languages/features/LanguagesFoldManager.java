 ...
 * @author Jan Jancura
 */
public class LanguagesFoldManager implements FoldManager {
    
    private static Map<Document,LanguagesFoldManager> documentToFoldManager = new WeakHashMap<Document,LanguagesFoldManager> ();
...
//    }
    ...
    public static final class AASTEvaluatorFactory implements ASTEvaluatorFactory {
    ...
    private static class AASTEvaluator extends ASTEvaluator {
        ...
        public AASTEvaluator (Document document) {
        ...
        public void afterEvaluation (ParserResult parserResult) {
            SwingUtilities.invokeLater (new Runnable () {
                public void run () { 
                    ...
                    try {
                        ...
                        collectFolds(fold, operation, existingFolds);
                        ...
                        while(genItr.hasNext()) {
                            ...
                            int fiLineOffset = Utilities.getLineOffset ((BaseDocument) document, fi.start);
            ...
            //                if(!zombies.contains(f)) { //check if not alread scheduled to remove
                            ...
                            if(!found) {
                                ...
                                int lineoffset = Utilities.getLineOffset ((BaseDocument) document, f.getStartOffset ());
            ...
            //                }
                        ...
                        while(newbornsItr.hasNext()) {
                            ...
                            Fold existing = (Fold)linesToFoldsCache.get (Integer.valueOf (Utilities.getLineOffset ((BaseDocument) document, fi.start)));
                            if(existing != null) {
        ...
        private void collectFolds(Fold fold, FoldOperation operation, List<Fold> existingFolds) {
            ...
            for (i = 0; i < k; i++) {
                ...
                try {
                    ...
                    //no ISE thrown - my fold
                    existingFolds.add(f);
                    collectFolds(f, operation, existingFolds);
                } catch (IllegalStateException e) {
                    //not my fold
                }
            }
        ...
        public void evaluate (List<ASTItem> path, Feature fold) {
            ASTItem item = path.get (path.size () - 1);
            int s = item.getOffset (),
                e = item.getEndOffset ();
            int sln = NbDocument.findLineNumber ((StyledDocument)document, s),
                eln = NbDocument.findLineNumber ((StyledDocument)document, e);
            if (sln == eln) return;
            ...
            if (!isTokenFold) {
                ...
                try {
                    ...
                    sln = NbDocument.findLineNumber ((StyledDocument)document, s);
                    eln = NbDocument.findLineNumber ((StyledDocument)document, e);
                    if (eln - sln < 1) return;
                } finally {
                    if (document instanceof NbEditorDocument)
            ...
            if (fold.hasSingleValue ()) {
                String foldName = LocalizationSupport.localize (language, (String) fold.getValue (SyntaxContext.create (document, ASTPath.create (path))));
                if (foldName == null) return;            
                addFold (new FoldItem(foldName, s, e, defaultFoldType));
                return;
            }
            String foldName = LocalizationSupport.localize (language, (String) fold.getValue ("fold_display_name", SyntaxContext.create (document, ASTPath.create (path))));
            if (foldName == null) {
                foldName = "..."; // NOI18N
            }
        ...
        private FoldOperation getOperation () {
            LanguagesFoldManager foldManager = documentToFoldManager.get (document);
            if (foldManager == null) return null;
            return foldManager.operation;
        }
    }
}
