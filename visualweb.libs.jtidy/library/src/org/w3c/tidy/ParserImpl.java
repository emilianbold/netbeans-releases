...
//                   node.tag == tt.tagHead ||
//                    node.tag == tt.tagBody)
                if (lexer.configuration.inputJspMode && node.tag == tt.tagHtml) {
                   Node.insertNodeAtEnd(element, node);
                   getParseHTML().parse(lexer, node, (short)0); // TODO?
                   continue;
                } else if (
                   !lexer.configuration.inputJspMode &&
                   (node.tag == tt.tagHtml ||
                    node.tag == tt.tagHead ||
                    node.tag == tt.tagBody)
                   )
    ...
    */
    public static Node parseDocument(Lexer lexer)
    {
        ...
        while (true)
        {
            ...
            // BEGIN RAVE MODIFICATIONS
            if (node.tag == tt.tagJspRoot) {
                lexer.configuration.inputJspMode = true;
                Node root = node;
                Node.insertNodeAtEnd(document, root);
                getParseBlock().parse(lexer, root, (short)0); // TODO?
                break;
            } else
            // END RAVE MODIFICATIONS
