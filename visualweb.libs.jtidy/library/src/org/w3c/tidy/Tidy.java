...
public class Tidy implements java.io.Serializable {
    ...
    /** Wraps an input stream, and "escapes" entities such that
     *  JTidy doesn't see them (and doesn't mess with them). The
     *  corresponding EntityWrapperOutputStream will undo its effects.
     * */
    public static class EntityWrapperInputStream extends InputStream {
    ...
    /** Wraps an output stream, and translates escaped entities back
     * into proper entities
     * */
    public static class EntityWrapperOutputStream extends OutputStream {
        ...
        public void write(int b) throws IOException {
            if (sb.length() != 0) {
                sb.append((char) b);
                String s = sb.toString(); // UGH! super inefficient
                if (s.equals(replacement)) {
                    ...
                    sb.setLength(0);
                } else if (!replacement.startsWith(s)) {
                    outputStream.write(s.getBytes());
                    sb.setLength(0);
                }
            } else if (b == (int)replacement.charAt(0)) {
                sb.append((char) b);
            } else {
    ...
    /** When parseDOM is called, there's no output stream to fix the
     * nodes. This method achieves that.
     */
    public static void cleanEntities(org.w3c.dom.Node node, boolean convertHtmlToJspx) {
        if (node instanceof CharacterData) {
            ...
            while (true) {
                ...
                if (s.indexOf(replacement) == -1) {
                    break;
                }
                // Don't change text ampersands within text nodes because those 
                // will get expanded anyway by the dom serializer
                //s = s.replaceAll(replacement, convertHtmlToJspx ? "&amp;" : "&");
                s = s.replaceAll(replacement, "&");
                text.setData(s);
            }
        }
        NamedNodeMap nmn = node.getAttributes();
        if (nmn != null) {
            for (int j = 0, siz = nmn.getLength(); j < siz; j++) {
                org.w3c.dom.Node item = nmn.item(j);
                if (item instanceof org.w3c.dom.Attr) {
                    org.w3c.dom.Attr attr = (org.w3c.dom.Attr)item;
                    ...
                    s = s.replaceAll(replacement, convertHtmlToJspx ? "&amp;" : "&");
                    s = expand(s);
                    // XXX It would be nice if I could find a way to set
                    // the node value of the Node in such a way that it
                    // preserve entities.  Can I add text nodes and entity
                    // nodes?? That would kick ass!
                    attr.setValue(s);
                }
            }
        }
        ...
        for (int i = 0, n = nl.getLength(); i < n; i++) {
            ...
            cleanEntities(child, convertHtmlToJspx);
        }
    }


    /** Expand entities one level in the given source string.
     * Copied from insync.markup. This method has the Sun copyright.
     */
