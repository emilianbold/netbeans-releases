...
import org.netbeans.modules.websvc.wsitconf.UndoManagerHolder;
...
public class WSITModelSupport {
    ...
    public static WSDLModel getModel(Node node, JaxWsModel jaxWsModel, UndoManagerHolder umHolder, boolean create) throws MalformedURLException, Exception {
        ...
        if (client != null) { //it is a client
            return getModelForClient(node, jaxWsModel, client, create);
        } else if (service != null) {  //it is a service
            try {
                String wsdlUrl = service.getWsdlUrl();
                if (wsdlUrl == null) { // WS from Java
                    model = getModelForServiceFromJava(node, jaxWsModel, create);
                } else {
        ...
        if ((model != null) && (umHolder.getUndoManager() == null)) {
     ...
     */ 
    private static WSDLModel getModelForClient(Node node, JaxWsModel jaxWsModel, Client client, boolean create) throws IOException {
        ...
        try {
            ...
            if (create) {
                ...
                if (mainConfigFO == null) {
                    mainConfigFO = createMainConfig(srcFolder);
                }
                ...
                copyImports(originalwsdlmodel, srcFolder);
    ...
    private static void copyImports(final WSDLModel model, final FileObject srcFolder) throws CatalogModelException {
        
//        FileObject modelFO = getFOForModel(model);
//        if (traverseSet.contains(modelFO)) {
//            return;
//        }
//        traverseSet.add(modelFO);
        ...
        for (Import i : imports) {
            ...
            try {
                ...
                copyImports(importedModel, srcFolder);
            } catch (IOException e) {
    ...
    /* Retrieves WSDL model for a WS from Java - if config file exists, reuses that one, otherwise generates new one
     */ 
    private static WSDLModel getModelForServiceFromWsdl(Node node, JaxWsModel jaxWsModel, Service service) throws IOException, Exception {
    ...
    private static WSDLModel getModelForServiceFromJava(Node node, JaxWsModel jaxWsModel, boolean create) throws IOException, Exception {
        JavaClass jc = (JavaClass)node.getLookup().lookup(JavaClass.class);
        Project p = FileOwnerQuery.getOwner(jaxWsModel.getJaxWsFile());
        JAXWSSupport supp = JAXWSSupport.getJAXWSSupport(p.getProjectDirectory());
        return getModelForServiceFromJava(jc, supp, create);
    }
    
    /* Retrieves WSDL model for a WS from Java - if config file exists, reuses that one, otherwise generates new one
     */ 
    public static WSDLModel getModelForServiceFromJava(JavaClass jc, JAXWSSupport supp, boolean create) throws IOException {
