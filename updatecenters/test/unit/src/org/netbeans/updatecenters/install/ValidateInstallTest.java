 ...
 */
public class ValidateInstallTest extends NbTestCase {
    ...
    private String currentElementToInstall = null;

    public ValidateInstallTest(String testName, String moduleName) {
        super(testName);
        this.currentElementToInstall = moduleName;
    }
    ...
    protected void runTest() throws Throwable {
        ...
        if (currentElementToInstall == null) {
            ...
            for (UpdateElement updateElement : newPlugins) {
                if (updateElement.getCodeName().equals(currentElementToInstall)) {
                    log("[" + updateElement.getDisplayName() + "] " + updateElement.getCodeName() + " is available as new plugin");
                    fail("[" + updateElement.getDisplayName() + "] " + updateElement.getCodeName() + " is not installed!");
                }
            }
    ...
    public static NbTestSuite suite() {
        ...
        ArrayList<String> listOfCodeNames = OperationUtils.readSavedData(OperationUtils.INSTALL_DATA_FILE_NAME);
...
//sleep 5 sec
        ...
        suite.addTest(new ValidateInstallTest(TEST_INSTALLED_PLUGINS_EMPTY, null));

        int testNumber = 0;
        for (String moduleCodeName : listOfCodeNames) {
            suite.addTest(new ValidateInstallTest("test" + OperationUtils.getModuleCanonicalName(getDisplayName(moduleCodeName), testNumber), moduleCodeName));
            testNumber++;
        }
        return suite;
    }
