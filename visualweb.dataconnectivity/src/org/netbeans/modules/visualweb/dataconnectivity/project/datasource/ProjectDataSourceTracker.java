...
public class ProjectDataSourceTracker{
   ...
   public static String[] getDynamicDataSources(Project project) {        
    ...
    public static Project[] getProjects() {
        ...
        for (int icnt = 0 ; icnt < getInstance().trackers.size() ; icnt++ ) {
            al.add( ( (DSTracker)getInstance().trackers.get(icnt) ).project ) ;
        }
        return (Project[])al.toArray( new Project[getInstance().trackers.size()]) ;
    }
    ...
    public static synchronized DSTracker getDSTracker(Project project) {
        ...
        ArrayList tt = getInstance().trackers ;
        for (int icnt = 0 ; icnt < tt.size() ; icnt++ ) {
            if ( project == ((DSTracker)tt.get(icnt)).getProject()) {
                tracker = (DSTracker)tt.get(icnt) ;
                break ;
            }
        }
        if (tracker == null ) {
