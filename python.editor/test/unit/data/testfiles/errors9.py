...
class _NbTextTestResult(TestResult):
    ...
    def _handle_doctest(self, message, stackstr):
        ...
        if message_str.startswith("Failed doctest test for "):
            ...
            m = re.match('Failed doctest test for (.*)\n'
                    '  File "(.*)", line (.*), in (.*)\n\n(.*)', message_str)
            print "Match was " + str(m)
            if m:
                test_name = m.group(1)
                filename = m.group(2)
                lineno = m.group(3)
...
#                print "err = " + err
                ...
                # From output_difference() in doctest
----------------------------------------------------------------------
File "/Users/tor/Downloads/diveintopython-5.4/py/roman/stage9/pythontest.py", line 20, in pythontest.factorial
Failed example:
    factorial(30L)
Expected:
    265252859812191058636308480000000L
Got:
    132626429906095529318154240000000L
        ...
        if want and got:
            ...
            return 'Expected nothing\nGot nothing\n'
                ...
                # Modify stack and message
                message = message_str[m.start(5):]
                stackstr = test_name + "() in " + filename + ":" + lineno + "%BR%"
