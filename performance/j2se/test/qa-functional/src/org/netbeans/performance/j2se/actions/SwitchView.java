 ...
 * 
 */
public class SwitchView  extends PerformanceTestCase {
    
        String operator;
        TopComponentOperator op;
	ProjectsTabOperator pto;
        RuntimeTabOperator rto;
        FavoritesOperator fo;
        FilesTabOperator fto;
    ...
    public ComponentOperator open() {
        log(":: open");
        op = new TopComponentOperator(operator);
        return op;
    }
    ...
    public static void main(String[] args) {
        repeat = 3;
        junit.textui.TestRunner.run(new SwitchView("measureTime"));
    }
}