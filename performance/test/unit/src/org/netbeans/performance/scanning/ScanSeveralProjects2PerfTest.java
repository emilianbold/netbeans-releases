 ...
 * @author Pavel Flaska
 */
public class ScanSeveralProjects2PerfTest extends NbTestCase {

    private final List<PerformanceData> data;
    private String nb_all_root;
    
    public ScanSeveralProjects2PerfTest(String name) {
        super(name);
            ...
            throws IOException, ExecutionException, InterruptedException
    {
        ...
        String[][] files = {
                ...
                { null, null, "openide.io" },
                { null, null, "openide.execution" }
            };
        for (String[] row : files) {
            final String networkFileLoc = row[0];
            final String compressedProject = row[1];
            if (networkFileLoc == null || compressedProject == null)
                continue;
        ...
        for (String[] row : files) {
            ...
            if (row[0] == null) {
                projectsDir = FileUtil.normalizeFile(new File(nb_all_root));
            }
    ...
    protected void tearDown() throws Exception {
        ...
        for (PerformanceData rec : getPerformanceData()) {
            Utilities.processUnitTestsResults(ScanSeveralProjects2PerfTest.class.getCanonicalName(), rec);
        }
        data.clear();
    }
