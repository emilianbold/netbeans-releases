...
public class JGitSshSessionFactory extends JschConfigSessionFactory {
    ...
    private static Map<FileSystem, SshSessionFactory> INSTANCE = new HashMap<FileSystem, SshSessionFactory>();
    ...
    public static synchronized SshSessionFactory getDefault (VCSFileProxy repositoryLocation) {
        FileSystem fileSystem = VCSFileProxySupport.getFileSystem(repositoryLocation);
        SshSessionFactory factory = INSTANCE.get(fileSystem);
        if (factory == null) {
            factory = new JGitSshSessionFactory(fileSystem);
            INSTANCE.put(fileSystem, factory);
        }
        return factory;
    }
    ...
    private JGitSshSessionFactory (FileSystem fileSystem) {
    ...
    protected JSch getJSch (Host hc, FS fs) throws JSchException {
        ...
        if (defaultJSch == null) {
            ...
            final VCSFileProxy home = VCSFileProxySupport.getHome(VCSFileProxy.createFileProxy(fileSystem.getRoot()));
            if (home != null) {
                VCSFileProxy known_hosts = VCSFileProxy.createFileProxy(home, ".ssh/known_hosts"); //NOI18N
                defaultJSch.setKnownHosts(known_hosts.getPath());
            }
