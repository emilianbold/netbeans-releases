...
public class CheckoutRevisionCommand extends GitCommand {
    ...
    private void cacheContents (List<String> conflicts) throws IOException {
        ...
        ObjectInserter inserter = getRepository().getRepository().newObjectInserter();
        WorkingTreeOptions opt = getRepository().getRepository().getConfig().get(WorkingTreeOptions.KEY);
        boolean autocrlf = opt.getAutoCRLF() != CoreConfig.AutoCRLF.FALSE;
        try {
            for (String path : conflicts) {
                VCSFileProxy f = VCSFileProxy.createFileProxy(workTree, path);
                boolean symlink = VCSFileProxySupport.isSymlink(f);
                if (symlink) {
                    String link = VCSFileProxySupport.readSymbolicLinkPath(f);
                    cachedContents.put(path, inserter.insert(Constants.OBJ_BLOB, Constants.encode(link)));
                } else if (f.isFile()) {
                    long sz = VCSFileProxySupport.length(f);
    ...
    private void mergeConflicts (List<String> conflicts, DirCache cache) throws GitException {
        ...
        TreeWalk walk = new TreeWalk(getRepository().getRepository());
        ObjectDatabase od = null;
        DiffAlgorithm.SupportedAlgorithm diffAlg = getRepository().getRepository().getConfig().getEnum(
        ...
        try {
            od = getRepository().getRepository().getObjectDatabase();
            walk.addTree(dci);
        ...
        if (oursId != null) {
            if (theirs != null && (theirs.getFileMode().getBits() & FileMode.TYPE_FILE) == FileMode.TYPE_FILE) {
                ...
                try (OutputStream fos = opt.getAutoCRLF() != CoreConfig.AutoCRLF.FALSE
                    ? new AutoCRLFOutputStream(VCSFileProxySupport.getOutputStream(file))
                    : VCSFileProxySupport.getOutputStream(file)) {
                    loader.copyTo(fos);
                }
            }
        }
