...
public class UndeployAction extends NodeAction {
    ...
    protected void performAction(final Node[] activatedNodes) {
        ...
        for (int i=0;i<activatedNodes.length;i++){
            Node parentNode = activatedNodes[i].getParentNode();
            parents.add(parentNode);
        }
        final Object[] parentNodes = parents.toArray();
        ...
        RequestProcessor.getDefault().post(new Runnable() {
            public void run() {                                
                for (int i=0;i<activatedNodes.length;i++){
                    ...
                    try {
                        if(obj instanceof Undeployable) {
                            Undeployable undeployableObj = (Undeployable)obj;
                            undeployableObj.undeploy();
                        }
                ...
                for(int i=0; i<parentNodes.length; i++){
                    Node parentNode = (Node)parentNodes[i];
