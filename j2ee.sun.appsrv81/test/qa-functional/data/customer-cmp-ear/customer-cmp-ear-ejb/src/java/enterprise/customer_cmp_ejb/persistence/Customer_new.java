...
@Entity
@NamedQueries(
    value={@NamedQuery(name="findCustomerByFirstName", query="select object(c) from Customer_new c where c.firstName= :firstName"),
    @NamedQuery(name="findCustomerByLastName", query="select object(c) from Customer_new c where c.lastName= :lastName")}
)

public class Customer_new implements java.io.Serializable{
    ...
    public Customer_new(String id, String firstName, String lastName) {          
        setCustomerID(id);
