    ...
    public void testInheritedMethods() throws Exception {
        performTest(new CancellableTask<CompilationInfo>() {
            ...
            public void run(CompilationInfo parameter) throws Exception {
                ...
                FileScope fileStructure = model.getFileScope();
                List<? extends ClassScope> filter = ModelUtils.filter(fileStructure.getDeclaredClasses(),
                        "cls4");
                ClassScope cls4 = ModelUtils.getFirst(filter);
                assertNotNull(cls4);
                List<? extends MethodScope> inheritedMethods = cls4.getInheritedMethods();
                assertEquals(inheritedMethods.size(), 5);
                IndexScope indexScope = fileStructure.getIndexScope();
                for (MethodScope methodScope : inheritedMethods) {
                    MethodScope meth = ModelUtils.getFirst(indexScope.findMethods(methodScope.getClassScope(), methodScope.getName()));
                    assertNotNull(meth);
                    assertEquals(meth, methodScope);
                }

            }
        },prepareTestFile("testfiles/model/scope.php"));
    }
    public void testInheritedMethods2() throws Exception {
        performTest(new CancellableTask<CompilationInfo>() {
            public void cancel() {}
            public void run(CompilationInfo parameter) throws Exception {
                Model model = ModelFactory.getModel(parameter);
                ...
                List<? extends ClassScope> filter = ModelUtils.filter(fileStructure.getDeclaredClasses(),
                        "cls2");
                ClassScope cls2 = ModelUtils.getFirst(filter);
                assertNotNull(cls2);
                List<? extends MethodScope> inheritedMethods = cls2.getInheritedMethods();
                List<? extends MethodScope> declaredMethods = cls2.getDeclaredMethods();
                assertEquals(inheritedMethods.size(), 2);
                assertEquals(declaredMethods.size(), 1);
                IndexScope indexScope = fileStructure.getIndexScope();
                for (MethodScope methodScope : inheritedMethods) {
                    MethodScope meth = ModelUtils.getFirst(indexScope.findMethods(methodScope.getClassScope(), methodScope.getName()));
                    assertNotNull(meth);
