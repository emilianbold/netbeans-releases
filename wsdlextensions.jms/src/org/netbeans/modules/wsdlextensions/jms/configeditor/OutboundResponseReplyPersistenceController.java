 ...
 * @author jalmero
 */
public class OutboundResponseReplyPersistenceController {
    private OutboundResponseReplyPanel jmsEditorPanel = null;
    private WSDLComponent mWSDLComponent = null;
    private OutboundResponseReplyPersistenceController mController = null;
        
    public OutboundResponseReplyPersistenceController(WSDLComponent modelComponent,
            OutboundResponseReplyPanel visualComponent) {
...
//                    getOutputMessageText());
            ...
            if (jmsEditorPanel.getOutputUse().equals(JMSConstants.ENCODED)) {
                jmsMessage.setJMSEncodingStyle(jmsEditorPanel.getOutputEncodingStyle());
                jmsMessage.setUse(jmsEditorPanel.getOutputUse());
            } else {
                if ((jmsMessage.getUse() != null) &&
                        (jmsEditorPanel.getOutputUse().equals(JMSConstants.LITERAL))) {
   ...
   private boolean commitPort(Port port) {
    ...
    private void commitMessageType(Binding binding, String opName,
        ...
        if (partName != null) {
            // set input part 
//            Collection parts = Utilities.getInputParts(binding, opName);
//            if ((parts != null) && (parts.size() > 0)) {
//                Iterator iter = parts.iterator();
//                while(iter.hasNext()) {
//                    Part partEntry = (Part) iter.next();
//                    if ((partEntry.getName().equals(partName)) && 
//                            (partEntry.getType() == null)) {
//                        // then create a type per Message Type selected
//                        // ie xsd:string, xsd:<...>, or ns:typeA something
//                        if (jmsEditorPanel.getMessageType() ==
//                                JMSConstants.XML_MESSAGE_TYPE) {
//                                    
//                        } else if (jmsEditorPanel.getMessageType() ==
//                                JMSConstants.TEXT_MESSAGE_TYPE) {
//                            partEntry.setType(partEntry.createSchemaReference(
...
//            }
            ...
            // set out part
            Collection outParts = JMSUtilities.getOutputParts(binding, opName);
...
//            Message newMessageIn = Utilities.
//                    createMessage(mWSDLComponent.getModel(), "JMSInputMessage");
//            if (newMessageIn != null) {
//                Part newPart = Utilities.createPartAndSetType(mWSDLComponent.getModel(),
//                        newMessageIn, "part1", jmsEditorPanel.getMessageType());
//                if (newPart != null) {
//                    inputFileMessage.setTextPart(newPart.getName());
//                }
//            }
            
            // handle the outbound as well but not set to binding section
            Message newMessageOut = JMSUtilities.
                    createMessage(mWSDLComponent.getModel(), "JMSOutputMessage");
            if (newMessageOut != null) {
                Part newPart = JMSUtilities.createPartAndSetType(mWSDLComponent.getModel(),
                        newMessageOut, "part1", jmsEditorPanel.getMessageType(),
                        gT, gE);
                if (newPart != null) {
                    JMSUtilities.setPart(jmsEditorPanel.getMessageType(),
