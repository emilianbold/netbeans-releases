 ...
 * @author jalmero
 */
public class InboundRequestResponseMainConfigurationEditorComponent
        implements ExtensibilityElementConfigurationEditorComponent {

    private InboundRequestResponseMainPanel editorPanel = null;
    private WSDLComponent component;
    private QName mQName;
  
    private InboundPersistenceController mInController = null;
    private InboundResponsePersistenceController mRespMsgController = null;
    private InboundRequestResponsePersistenceController mRespMsgConsumerController = null;
    
    public InboundRequestResponseMainConfigurationEditorComponent(QName qName,
            WSDLComponent component) {
        if (editorPanel == null) {
            editorPanel = new InboundRequestResponseMainPanel(component);
        } else {
        ...
        if (mInController == null) {
            mInController = new InboundPersistenceController(component, 
                    editorPanel.getInboundPanel());
        }
        mInController.setAdvancedPanel(editorPanel.getAdvancedPanel());
        if (mRespMsgController == null) {
            mRespMsgController = new 
                    InboundResponsePersistenceController(component,
                    editorPanel.getResponseMessagePanel());        
        }        
        
        if (mRespMsgConsumerController == null) {
            mRespMsgConsumerController = new 
                    InboundRequestResponsePersistenceController(component,
                    editorPanel.getResponseMessageConsumerPanel());        
        } 
    ...
    public String getTitle() {
        return NbBundle.getMessage(InboundRequestResponseMainConfigurationEditorComponent.class,
                "InboundOneWayConfigurationEditorComponent.CONFIGURE_TITLE");                                 //NOI18N
    }
    ...
    public boolean commit() {
        ...
        if (status) {
            status = mRespMsgController.commit();
            if (status) {
                status = mRespMsgConsumerController.commit();
            }            
        }
        return status;
    }
