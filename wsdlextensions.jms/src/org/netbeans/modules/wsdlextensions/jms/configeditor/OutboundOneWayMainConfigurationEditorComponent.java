 ...
 * @author jalmero
 */
public class OutboundOneWayMainConfigurationEditorComponent
        implements ExtensibilityElementConfigurationEditorComponent {

    private OutboundOneWayMainPanel editorPanel = null;
    private WSDLComponent component;
    private QName mQName;
  
    private OutboundInPersistenceController mInController = null;
    private OutboundInPersistenceController mPublisherController = null;
    
    public OutboundOneWayMainConfigurationEditorComponent(QName qName,
            WSDLComponent component) {
        if (editorPanel == null) {
            editorPanel = new OutboundOneWayMainPanel(component);
        } else {
        ...
        if (mInController == null) {
            mInController = new OutboundInPersistenceController(component, 
                    editorPanel.getOutboundPanel());
        }
        mInController.setAdvancedPanel(editorPanel.getAdvancedPanel());
        if (mPublisherController == null) {
            mPublisherController = new 
                    OutboundInPersistenceController(component,
                    editorPanel.getPublisherPanel());        
        }        
    ...
    public String getTitle() {
        return NbBundle.getMessage(OutboundOneWayMainConfigurationEditorComponent.class,
    ...
    public boolean commit() {
        ...
        if (status) {
            status = mPublisherController.commit();
        }
        return status;
    }
