 ...
 * @author  jalmero
 */
public class InboundOneWayMessagePanel extends javax.swing.JPanel {
    ...
    private static final Logger mLogger = Logger.
            getLogger(InboundOneWayMessagePanel.class.getName());
    ...
    public InboundOneWayMessagePanel(QName qName, WSDLComponent component) {
        this(qName, component, false);
    }
...
//        }
   ...
   void setProject(Project project) {
    ...
    private void initCustomComponents() {
        ...
        mMessageTypePanel = new MessageTypePanel(mQName, mWsdlComponent, null, null, null);
        gridBagConstraints.gridx = 0;
    ...
    private void initComponents() {
        ...
        org.openide.awt.Mnemonics.setLocalizedText(jLabel26, "null");
        jLabel26.setName("jLabel26"); // NOI18N

        org.openide.awt.Mnemonics.setLocalizedText(portTypeLabel, "null");
        ...
        org.openide.awt.Mnemonics.setLocalizedText(jLabel42, "null");
        ...
        org.openide.awt.Mnemonics.setLocalizedText(operationNameLabel, "null");
        ...
        org.openide.awt.Mnemonics.setLocalizedText(bindingNameLabel, "null");
    ...
    private void populateJMSBinding(JMSBinding jmsBinding,
        ...
        if (jmsBinding != null) {
            ...
            if (operationNameComboBox.getItemCount() > 0) {
                ...
                JMSMessage msgToUse = null;
                if (binding != null) {
                    if (mSolicited) {
                        msgToUse = getOutputJMSMessage(binding,
                            operationNameComboBox.getSelectedItem().toString());                        
                    } else {
                        msgToUse = getInputJMSMessage(binding,
                            operationNameComboBox.getSelectedItem().toString());
                    }
                    updateInputMessageView(binding, msgToUse);
                }
            }
    ...
    private void populatePartBoxes(Binding binding,
        ...
        if (bindingOperations != null) {
            ...
            for (BindingOperation bop : bindingOperations) {
                ...
                // populate text part
                if (mSolicited) {
                    populateInputPartComboBox(outputTextParts);
                } else {
...
//                populateOutputPartComboBox(outputTextParts);
                ...
                if (mSolicited) {
                    ...
                    if ((outputJMSMessages != null) &&
                        ...
                        inputTypeComboBox.setSelectedItem(outputJMSMessage.
                                getMessageType());
                        // get input text
                        inputTextComboBox.setSelectedItem(outputJMSMessage.
                                getTextPart());                        
...
//                    getAttribute(JMSAddress.ATTR_JNDI_SECURITY_CRDENTIALS));
    ...
    private void populateInputPartComboBox(Collection textItems) {
        if ((textItems != null) && (textItems.size() > 0)) {
            ...
            int count = inputTextComboBox.getItemCount();
            Collection inputTexts = new ArrayList();
            for (int i = 0; i < count; i++) {
                inputTexts.add(inputTextComboBox.getModel().getElementAt(i));
            }
            while(iter.hasNext()) {
                String partName = (String) iter.next();
                if (!inputTexts.contains(partName)) {
...
//                outputTextComboBox.addItem(partName);
   ...
   private void populateListOfBindings(JMSBinding jmsBinding) {
    ...
    private void updateInputMessageText(JMSMessage inputJMSMessage) {
        if ((inputJMSMessage.getTextPart() == null) ||
                ((inputJMSMessage.getTextPart() != null) &&
                (inputJMSMessage.getTextPart().equals("change me")))) {
            
            // create a message with text
            try {
                if (!mWsdlComponent.getModel().isIntransaction()) {
                    mWsdlComponent.getModel().startTransaction();
                }
                Message newMessage = JMSUtilities.createMessage(mWsdlComponent.getModel(), "JMSInMessage");
                if (newMessage != null) {
                    Part newPart = JMSUtilities.createPart(mWsdlComponent.getModel(),
                            newMessage, "jmsInPart");
                    if (newPart != null) {
                        if (!JMSUtilities.isMessagePartExists(newPart.getName(),
                                newMessage)) {
                            newMessage.addPart(newPart);
                        }
                        inputTextComboBox.addItem(newPart.getName());
                        inputTextComboBox.setSelectedItem(newPart.getName());
                    }
                }
            } catch (Exception ex) {
                ...
                if (mWsdlComponent.getModel().isIntransaction()) {
                    mWsdlComponent.getModel().endTransaction();
                }                
            }
        }      
    }
   ...
   public FileError validateMe() {
    ...
    public void doFirePropertyChange(String name, Object oldValue, Object newValue) {  
        ...
        if (panelToFireTo != null) {  
            if (getParent() instanceof InboundOneWayMainPanel) {
                ((InboundOneWayMainPanel) getParent()).doFirePropertyChange(name, oldValue, newValue);
            }  
        } else {
...
//                    NbBundle.getMessage(InboundOneWayMessagePanel.class,
