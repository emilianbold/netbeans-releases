 ...
 * @author sdedic
 */
public class ScriptEmbeddingProcessor extends FxNodeVisitor.ModelTraversal implements ModelBuilderStep {
    private BuildEnvironment env;

    public ScriptEmbeddingProcessor() {
    }
    
    ScriptEmbeddingProcessor(BuildEnvironment env) {
        this.env = env;
    }
    ...
    public void visitEvent(EventHandler eh) {
        ...
        TextPositions pos = env.getTreeUtilities().positions(eh);
        int from = pos.getContentStart();
        int to = pos.getContentEnd();
    }
    
    @Override
    public FxNodeVisitor createVisitor(BuildEnvironment env) {
        return new ScriptEmbeddingProcessor(env);
    }
