...
public class ControllerGenerator implements Task<WorkingCopy> {
    ...
    private void removeHandler(ExecutableElement el, MethodTree mt) {
        boolean emptyMethod = true;
        
        if (mt.getBody() != null) {
            List<? extends StatementTree> stmts = mt.getBody().getStatements();
            emptyMethod = stmts.isEmpty();
        }
        
        TreeMaker mk = wcopy.getTreeMaker();
        if (emptyMethod) {
            controllerClass = mk.removeClassMember(controllerClass, mt);
        } else {
            if (FxClassUtils.isFxmlAnnotated(el)) {
                // remove the annotation:
                wcopy.rewrite(mt.getModifiers(), 
                        mk.removeModifiersAnnotation(mt.getModifiers(), fxmlAnnotationTree));
            }
        }
    }
    ...
    private VariableTree removeFxmlAnnotation(VariableTree vt) {
        for (AnnotationTree ann : vt.getModifiers().getAnnotations()) {
            TreePath tp = new TreePath(new TreePath(wcopy.getCompilationUnit()), ann.getAnnotationType());
            Element  e  = wcopy.getTrees().getElement(tp);
            if (fxmlAnnotationType.equals(e)) {
                // found, now remove it:
                TreeMaker mk = wcopy.getTreeMaker();
                wcopy.rewrite(vt.getModifiers(), 
                    mk.removeModifiersAnnotation(vt.getModifiers(), ann));
                return vt;
            }
        }
        return vt;
