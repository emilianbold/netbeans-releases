 ...
 */
package org.netbeans.modules.javafx2.editor.completion.model.processors;
...
import static org.netbeans.modules.javafx2.editor.completion.model.processors.Bundle.*;
...
public class PropertyResolver extends FxNodeVisitor.ModelTreeTraversal implements ModelBuilderStep {
    ...
    private ImportProcessor getImporter() {
        ...
        ImportProcessor proc = new ImportProcessor(env.getCompilationInfo(), env.getHierarchy(), env, env.getNodes());
    ...
    private boolean doVisitStaticProperty(StaticProperty p) {
        ...
        int offs = env.getNodes().positions(p).getStart();
        if (names == null) {
    ...
    private void processDefaultProperty(PropertySetter p) {
        ...
        if (pi == null) {
            int start = env.getNodes().positions(p).getStart();
            int len = 1;
