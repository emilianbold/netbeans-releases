...
public final class FxTreeUtilities {
    ...
    FxTreeUtilities(FxModel model) {
        this.model = model;
    }
    
    public boolean containsPos(FxNode n, int position) {
        return n.i().contains(position);
    }
    
    public boolean contentContainsPos(FxNode n, int position) {
        return n.i().contentContains(position);
    }
    ...
    public List<? extends FxNode> findEnclosingElements(final int position, boolean ignoreTag) {
        class T extends FxNodeVisitor.ModelTreeTraversal {
            ...
            public void visitNode(FxNode node) {
                if (node.i().contains(position)) {
                    nodeStack.push(node);
                    super.visitNode(node);
                }
                if (node.i().getStart() > position) {
                    throw new Error();
                }
            }
        ...
        if (!visitor.nodeStack.isEmpty() && ignoreTag) {
            ...
            if (!n.i().contentContains(position)) {
