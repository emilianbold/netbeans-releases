...
public class TypeResolver extends FxNodeVisitor.ModelTreeTraversal implements ModelBuilderStep {
    ...
    public void visitSource(FxModel model) {
        importProcessor = new ImportProcessor(env.getCompilationInfo(), env.getHierarchy(), env);
    ...
    public void visitInstance(FxNewInstance decl) {
        ...
        if (el == null) {
            resolveClassname(sourceName, decl);
        } else {
            ElementHandle<TypeElement> handle = ElementHandle.create(el);
            env.getAccessor().resolveClassName(decl, sourceName, handle);
        }
    ...
    private void resolveClassname(String name, FxNewInstance decl) {
        ...
        if (names != null && names.size() == 1) {
            ...
            TypeElement el = env.getCompilationInfo().getElements().getTypeElement(fqn);
            if (el != null) {
                ElementHandle<TypeElement> handle = ElementHandle.create(el);
                env.getAccessor().resolveClassName(decl, fqn, handle);
                return;
            }
        }
        
        int start = (int) decl.i().getStart() + 1; // skip ">"
