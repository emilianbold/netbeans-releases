...
public class TypeResolver extends FxNodeVisitor.ModelTreeTraversal implements ModelBuilderStep {
    ...
    public void visitSource(FxModel model) {
        importProcessor = new ImportProcessor(env.getCompilationInfo(), env.getHierarchy(), null, env.getNodes());
        ...
        if (controller != null) {
            ...
            env.getAccessor().resolve(model, null, null, handle, null);
        }
    ...
    public void visitInstance(FxNewInstance decl) {
        ...
        if (el != null) {
            int start = env.getNodes().positions(decl).getStart() + 1; // skip ">"
            if (bean == null) {
