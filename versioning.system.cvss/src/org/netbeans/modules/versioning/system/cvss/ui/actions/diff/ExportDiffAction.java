...
public class ExportDiffAction extends AbstractSystemAction {
    ...
    private void async(File destination) {
        ...
        try {
            ...
            File root = context.getRootFiles()[0];
            if (root.isFile()) {
                root = root.getParentFile();
            }
            out.write( "# This patch file was generated by NetBeans IDE\n".getBytes("utf8"));  // NOI18N
            out.write(("# Following Index: paths are relative to: " + root.getAbsolutePath() + "\n").getBytes("utf8"));  // NOI18N
            out.write( "# This patch can be applied using context Tools: Patch action on respective folder.\n".getBytes("utf8"));  // NOI18N
            out.write( "# Above lines and this line are ignored by the patching process.\n".getBytes("utf8"));  // NOI18N
            ...
            for (int i = 0; i < files.length; i++) {
                ...
                if (filePath.startsWith(rootPath)) {
                    index += filePath.substring(rootPath.length() + 1).replace(File.separatorChar, '/') + "\n";
