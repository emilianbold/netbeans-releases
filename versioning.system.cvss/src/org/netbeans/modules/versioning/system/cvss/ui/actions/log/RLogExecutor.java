...
public class RLogExecutor extends ExecutorSupport {
    ...
    public static RLogExecutor [] executeCommand(RlogCommand cmd, File [] roots, GlobalOptions options) {
        ...
        RLogExecutor [] executors = new RLogExecutor[splitRoots.length];
        ...
        for (int i = 0; i < splitRoots.length; i++) {
            ...
            String commandContext = NbBundle.getMessage(RLogExecutor.class, "MSG_RLogExecutor_CmdContext", Integer.toString(files.length));
            command.setDisplayName(MessageFormat.format(cmd.getDisplayName(), new Object [] { commandContext }));
            executors[i] = new RLogExecutor(cvs, command, currentOptions);
            executors[i].execute();
        }
        return executors;
    }

    private RLogExecutor(CvsVersioningSystem cvs, RlogCommand cmd, GlobalOptions options) {
