...
public class AnnotationColorProvider extends OptionsPanelColorProvider {
    ...
    public String getName() {
        if (name == null) {
            name = NbBundle.getMessage(org.netbeans.modules.versioning.system.cvss.CVS.class, "CTL_CVS_DisplayName"); //NOI18N
        }
        return name;
    }
    ...
    protected Color getSavedColor (String key, Color defaultColor) {
        return CvsModuleConfig.getDefault().getColor(key, defaultColor);
    }
    ...
    protected void saveColors (Collection<AnnotationFormat> colors) {
        for (AnnotationFormat af : colors) {
            if (af != null) {
                CvsModuleConfig.getDefault().setColor(getColorKey(af.getKey()), af.getActualColor());
            }
        }
