    ...
    private synchronized Element getUpdatedSharedConfigurationData () {
        if (cachedElement == null) {
            ...
            if (oldRoot != null) {
                ...
                Element testRoots = doc.createElementNS(ArchiveProjectType.PROJECT_CONFIGURATION_NS,"test-roots");  //NOI18N
...
//        }
    ...
    private static Element updateMinAntVersion (final Element root, final Document doc) {
        NodeList list = root.getElementsByTagNameNS (ArchiveProjectType.PROJECT_CONFIGURATION_NS,MINIMUM_ANT_VERSION_ELEMENT);
        if (list.getLength() == 1) {
