 ...
 */
public class PercentageGraphComponent extends JComponent {

    private static final boolean TRACE = Boolean.getBoolean("PercentageGraph.trace");
    private final PercentageGraph graph;

    public PercentageGraphComponent(PercentageGraph.Descriptor... descriptors) {
        graph = new PercentageGraph(descriptors);
...
//            repaint(getUpdateRectangle());
    ...
    public String getToolTipText() {
        short[] last = graph.getLastData();
        PercentageGraph.Descriptor[] descriptors = graph.getDescriptors();        
        ...
        for (int i = 0; i < descriptors.length; i++) {
            ...
            sb.append(String.format("<tr><td>%s%s</td><td>%s%3d%%</td></tr>\n", font, descriptors[i].description, font, last[i]));
        }
        sb.append("</html>");
