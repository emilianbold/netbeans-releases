...
public final class ODCSProject {
    ...
    ODCSProject(Project project, ODCSServer server) {
    ...
    public synchronized ODCSServer getServer() {
        return server;
    }
    /**
     ...
     */
    public Collection<ScmRepository> getRepositories() throws ODCSException {
        assertLoggedIn();
        if(!hasScm()) {
            return null;
        }
        ...
        synchronized(REPOSITORIES_LOCK) {
            if(repositories == null) {
                ODCSClient client = ODCSFactory.getInstance().createClient(server.getUrl().toString(), server.getPasswordAuthentication());
    ...
    public synchronized boolean equals(Object obj) {
        ...
        final ODCSProject other = (ODCSProject) obj;
    ...
    public synchronized String toString() {
        return "ODCSProject " + project.getIdentifier(); // NOI18N
    }
    ...
    public static ODCSProject findProjectForRepository (String uri) throws ODCSException {
        Map.Entry<ODCSServer, String> pair = ODCSServer.findServerAndProjectForRepository(uri);
