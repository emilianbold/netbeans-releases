 ...
 * @author anjeleevich
 */
public class LDAPChildFactory extends ChildFactory<LDAPConnection>
        implements LDAPListener
{
    private boolean subscibed = false;

    public LDAPChildFactory() {
        
    }

    @Override
    protected boolean createKeys(List<LDAPConnection> toPopulate) {
        if (!subscibed) {
            subscibed = true;
        ...
        toPopulate.addAll(LDAP.INSTANCE.getConnections());
        return true;
    }

    @Override
    protected Node createNodeForKey(LDAPConnection connection) {
        return new LDAPConnectionNode(connection);
    }
