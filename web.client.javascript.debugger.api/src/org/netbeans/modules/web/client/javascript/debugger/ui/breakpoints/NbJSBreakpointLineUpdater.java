...
final class NbJSBreakpointLineUpdater implements PropertyChangeListener {

    private final NbJSBreakpoint breakpoint;
    private Line line;

    public NbJSBreakpointLineUpdater(NbJSBreakpoint breakpoint) {
    ...
    public synchronized void attach() throws IOException {
        ...
        if (fo != null) {
            ...
            try {
                this.line = lc.getLineSet().getCurrent(
    ...
    public synchronized void propertyChange(PropertyChangeEvent evt) {
        if (Line.PROP_LINE_NUMBER.equals(evt.getPropertyName())) {
            breakpoint.notifyUpdated(evt.getSource());
            return;
        } 
    }
}