...
public final class NbJSPersistenceManager extends DebuggerManagerAdapter {

    private static final String JAVASCRIPT_PROPERTY = "javascript"; // NOI18N
    private static final String DEBUGGER_PROPERTY = "debugger"; // NOI18N

    @Override
    public Breakpoint[] initBreakpoints() {
        Properties p = loadBreakpointProperties();
        Breakpoint[] breakpoints = (Breakpoint[]) p.getArray(JAVASCRIPT_PROPERTY, new Breakpoint[0]);
        ...
        for (Breakpoint breakpoint : breakpoints) {
            if (breakpoint != null) {
                ...
                NbJSUtil.warning("null stored in the array obtained from \"" + JAVASCRIPT_PROPERTY + "\" property"); // TODO: why?
            }
        }
    ...
    public void propertyChange(final PropertyChangeEvent evt) {
        if (evt.getSource() instanceof NbJSBreakpoint) {
            storeBreakpoints();
        }
    }

    private void storeBreakpoints() {
        loadBreakpointProperties().setArray(JAVASCRIPT_PROPERTY, NbJSBreakpointManager.getBreakpoints());
    }
