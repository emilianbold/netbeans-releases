...
import org.netbeans.modules.cnd.toolchain.spi.CompilerSetProviderFactoryImpl;
 ...
 */
public class CompilerSetProviderFactory {
    /**
    ...
    public static CompilerSetProvider createNew(ExecutionEnvironment execEnv) {
        CompilerSetProviderFactoryImpl factory = Lookup.getDefault().lookup(CompilerSetProviderFactoryImpl.class);
        if (factory == null) {
            throw new IllegalStateException(CompilerSetProviderFactoryImpl.class.getName() +" not found in lookup"); //NOI18N
        }
        return factory.createNew(execEnv);
    }

    private CompilerSetProviderFactory() {
    }
}
