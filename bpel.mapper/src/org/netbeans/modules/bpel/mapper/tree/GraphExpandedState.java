 ...
 * @version 1.0
 */
public class GraphExpandedState implements TreeExpandedState {
    private Mapper myMapper;
    private ArrayList<ArrayList<Step>> expanedGraphNodes = new ArrayList<ArrayList<Step>>();

    public GraphExpandedState(Mapper mapper) {
        myMapper = mapper;
    }
    ...
    private MapperSwingTreeModel getTreeModel() {
        ...
        return mapperModel != null
                ? ((BpelMapperModel)mapperModel).getRightTreeModel() : null;
    }

    private RightTree getGraphTree() {
    ...
    public void save() {
        ...
        List<TreePath> expanded = myMapper.getExpandedGraphsPathes();

        if (expanded != null) {
            for (TreePath treePath : expanded) {
                ArrayList<Step> path = getStepsByTreePath(treePath);
                expanedGraphNodes.add(path);
            }
        }
    ...
    public void restore() {
        ...
        for (ArrayList<Step> steps : expanedGraphNodes) {
        ...
        myMapper.applyExpandedGraphsPathes(paths);
    }
