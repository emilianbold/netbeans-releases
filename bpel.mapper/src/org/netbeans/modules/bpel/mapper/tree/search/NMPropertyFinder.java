 ...
 * @author anjeleevich
 */
public class NMPropertyFinder extends SimpleFinder {
    private AbstractVariableDeclaration variable;
    private String nmProperty;
    
    public NMPropertyFinder(AbstractVariableDeclaration variable,
            String nmProperty) 
    {
    ...
    protected boolean isFit(Object treeItem) {
        if (treeItem instanceof FileObject) {
            FileObject fileObject = (FileObject) treeItem;
            if (!fileObject.isFolder()) {
                return isEqual(nmProperty, fileObject.getAttribute(
                        PropertiesConstants.NM_PROPERTY_ATTR));
            }
        }
        
        if (treeItem instanceof NMProperty) {
            String nmPropertyName = ((NMProperty) treeItem).getNMProperty();
            ...
            if (nmPropertyName.length() > 0) {
                return isEqual(nmPropertyName, nmProperty); // NOI18N
            }
        }
    ...
    protected boolean drillDeeper(Object treeItem) {
        if (!(treeItem instanceof BpelEntity)) {
            ...
            return (treeItem instanceof FileObject) 
                    && ((FileObject) treeItem).isFolder();
        }
        
        if (treeItem instanceof Variable) {
            return (treeItem == variable);
        }
        
        if (treeItem instanceof VariableDeclarationWrapper) {
            return (treeItem == ((VariableDeclarationWrapper) treeItem)
                    .getDelegate());
        }
