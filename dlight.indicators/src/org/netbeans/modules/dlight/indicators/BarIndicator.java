...
public class BarIndicator extends Indicator {
  ...
  public BarIndicator(IndicatorMetadata metadata, BarIndicatorConfig config) {
    super(metadata);
    ...
    this.config = config;
    this.panel = new BarIndicatorPanel(metadata);
    barData = new Number[metadata.getColumnsCount()];
  }

  public BarIndicatorConfig getConfig() {
    return config;
  }
  ...
  public void updated( List<DataRow> data) {
    ...
    for (Column col : getMetadataColumns()) {
      ...
      if (count == 0) {
        ...
        String aggregationType = (String)(config.getNode(col.getColumnName()).get("aggregation"));
