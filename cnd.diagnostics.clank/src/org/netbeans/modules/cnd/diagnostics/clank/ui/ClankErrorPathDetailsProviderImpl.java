                ...
                RequestProcessor.getDefault().post(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            for (ClankDiagnosticInfo note : ClankCsmErrorInfoAccessor.getDefault().getDelegate(errorInfo).notes()) {
                                ...
                                int offset = note.getEndOffset();
                                if (document instanceof LineDocument) {
                                    offset = LineDocumentUtils.getLineEnd((LineDocument) document, offset);
                                }
                                final int locOffset = offset;
                                final int[] lineColumnByOffset = CsmFileInfoQuery.getDefault().getLineColumnByOffset(csmNoteFile, note.getStartOffset());
