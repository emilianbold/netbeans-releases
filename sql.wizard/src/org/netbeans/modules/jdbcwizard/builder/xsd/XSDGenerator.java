...
public class XSDGenerator {
                    ...
                    if (rs != null) {
                        try {
                            ...
                            if (isBuiltInType(colType)) {
                                elem = createElement(rs.getName(), (String) builtInTypes.get(colType));
                            } else {
            ...
            if (sequenceElement != null) {
                ...
                for (int i = 0; i < prep.getNumParameters(); i++) {
                    if(params[i].getParamType().equalsIgnoreCase(Procedure.IN) ||
                        ...
                        Element elem2 = createElement(params[i].getName(), (String) builtInTypes.get(params[i].getJavaType()));
            ...
            if (rs != null && rs.length > 0) {
                for (int i = 0; i < rs.length; i++) {
                    try {
                        ...
                        Element elem = null;
                        if (isBuiltInType(colType)) {
                            elem = createElement(rs[i].getName(), (String) builtInTypes.get(colType));
                        } else {
                            throw new WSDLException(WSDLException.INVALID_WSDL, "Invalid datatype encountered");
                        }
                        sequenceElement.appendChild(elem);
	...
	private ResultSetColumn[] validColumnNames(ResultSetColumn[] colNames){
        for(int i=0;i<colNames.length-1;i++){
            for(int j=i+1;j<colNames.length;j++){
            if(colNames[i].getName().equalsIgnoreCase(colNames[j].getName())){
			String temp=colNames[j].getName()+"_1";
			do{
                 temp= validate(colNames, temp);
               }while(status);
                colNames[j].setName(temp);                        
               }
            }
        }
		return colNames;
	...
	private String validate(ResultSetColumn[] colNames, String temp){
        for(int i=0;i<colNames.length;i++){
            if(temp.equalsIgnoreCase(colNames[i].getName())){
				temp=temp+"_1";
				status = true;
				return temp;
			} else{
