...
class ClassTracker<T> {
    private final Function<String, Collection<T>>        classIdSupplier;
    private final HashMap<String, ClassInfo> map;

    ClassTracker(Function<String, Collection<T>> idSupplier) {
        this.classIdSupplier = idSupplier;
    ...
    class ClassInfo {
        ...
        private T nameToRef(String name) {
            Collection<T> rtl = classIdSupplier.apply(name);
            if (rtl == null || rtl.size() != 1) {
                return null;
            }
            return rtl.iterator().next();
        }

        @Override
        public boolean equals(Object o) {
            return ClassInfo.class.isInstance(o)
