...
public class CloneWSDLPortAction extends NodeAction {
    ...
    private void doClone(WSDLEndpointNode node) {
        try {
            ...
            if (wFile.exists()) {
                FileObject srcFO = FileUtil.toFileObject(new File(srcPath + "/"+JBI_SOURCE_DIR+"/"));  // NOI18N
                Retriever ret = Retriever.getDefault();
                FileObject fo = ret.retrieveResourceClosureIntoSingleDirectory(srcFO, wFile.toURI());
