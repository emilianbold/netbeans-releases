...
perl nbbuild/misc/i18ncheck.pl `pwd`/cnd* `pwd`/asm* `pwd`/dlight* `pwd`/lib.terminalemulator/src terminal | grep -v "/test/" | grep -v "cnd.antlr/" | grep -v "generated/" | grep -v "parser/FortranLexicalPrepass.java" | grep -v "parser/FortranTokenStream.java" | tee ${LOG}
cnt=`cat ${LOG} | wc -l`
if [ ${cnt} -gt 0 ]; then
