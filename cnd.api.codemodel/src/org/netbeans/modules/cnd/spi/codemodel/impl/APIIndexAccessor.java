 ...
 */
public abstract class APIIndexAccessor {
    private static APIIndexAccessor INSTANCE;
    
    public static synchronized APIIndexAccessor get() {
        if (INSTANCE == null) {
            Class<?> c = CMVisitQuery.class;
            try {
    ...
    public static synchronized void register(APIIndexAccessor accessor) {
        assert INSTANCE == null : "must be registered once " + INSTANCE;
        INSTANCE = accessor;
    }
    
    public abstract CMEntity createEntity(CMEntityImplementation impl);

    public abstract CMDeclaration createDeclaration(CMDeclarationImplementation impl);

    public abstract CMEntityReference createEntityReference(CMEntityReferenceImplementation impl);

    public abstract CMInclude createInclude(CMIncludeImplementation impl);

    public abstract CMVisitLocation createVisitLocation(CMVisitLocationImplementation impl);

    public abstract CMReferenceImplementation getReferenceImpl(CMReference ref);

    public abstract CMDeclarationContextImplementation getDeclarationContextImpl(CMDeclarationContext dc);
}
