...
public final class VisualizerFactory {
  ...
  public static final VisualizerFactory getInstance(){
    if(instance == null){
      instance = new VisualizerFactory();
    }
    return instance;
  }
   ...
   */
   public Visualizer createVisualizer(VisualizerConfiguration configuraiton, DataProvider provider){
    Collection<? extends Visualizer> result = Lookup.getDefault().lookupAll(Visualizer.class);
    if (result.isEmpty()){
      return null;
    }
    Iterator<? extends Visualizer> iterator = result.iterator();
    while (iterator.hasNext()){
      Visualizer visualizer = iterator.next();
      if (visualizer.getID().equals(configuraiton.getID())){
        return visualizer.create(configuraiton, provider);
      }
    }
    return null;
  }
