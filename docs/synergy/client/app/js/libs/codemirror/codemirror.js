...
window.CodeMirror = (function() {
    ...
    function updateDisplay(cm, changes, viewPort) {
        ...
        if (updated) {
            ...
            if (cm.display.showingFrom != oldFrom || cm.display.showingTo != oldTo) signalLater(cm, cm, "viewportChange", cm, cm.display.showingFrom, cm.display.showingTo);
        }
        updateSelection(cm);
        updateScrollbars(cm.display, cm.view.doc.height);
    ...
    function updateDisplayInner(cm, changes, viewPort) {
        ...
        if (changes !== true && changes.length == 0 && visible.from > display.showingFrom && visible.to < display.showingTo) return;
        ...
        if (changes !== true && sawCollapsedSpans) for (var i = 0; i < changes.length; ++i) {
            var ch = changes[i],
                merged;
        ...
        if (cm.options.lineNumbers && changes && changes !== true) for (var i = 0; i < changes.length; ++i)
        if (changes[i].diff) {
        ...
        if (visibleLines(display, doc, viewPort).to >= to) updateDisplayInner(cm, [], viewPort);
        return true;
    }
    ...
    function patchDisplay(cm, from, to, intact, updateNumbersFrom) {
        ...
        cm.view.doc.iter(from, to, function(line) {
            ...
            if (lineIsHidden(line)) {
                ...
                if (lineNumbers && updateNumbersFrom <= lineNo && cur.lineNumber) setTextContent(cur.lineNumber, lineNumberFor(cm.options, lineNo));
                cur = cur.nextSibling;
            } else {
    ...
    function buildLineElement(cm, line, lineNo, dims) {
        ...
        if (cm.options.lineNumbers || markers) {
            var gutterWrap = wrap.appendChild(elt("div", null, null, "position: absolute; left: " + (cm.options.fixedGutter ? dims.fixedPos : -dims.gutterTotalWidth) + "px"));
            if (cm.options.fixedGutter) wrap.alignable = [gutterWrap];
            if (cm.options.lineNumbers && (!markers || !markers["CodeMirror-linenumbers"])) wrap.lineNumber = gutterWrap.appendChild(
            elt("div", lineNumberFor(cm.options, lineNo), "CodeMirror-linenumber CodeMirror-gutter-elt", "left: " + dims.gutterLeft["CodeMirror-linenumbers"] + "px; width: " + display.lineNumInnerWidth + "px"));
        ...
        if (line.bgClass) wrap.appendChild(elt("div", "\u00a0", line.bgClass + " CodeMirror-linebackground"));
        wrap.appendChild(lineElement);
        if (line.widgets) for (var i = 0, ws = line.widgets; i < ws.length; ++i) {
            var widget = ws[i],
                node = buildLineWidget(widget, wrap, dims);
            if (widget.above) wrap.insertBefore(node, cm.options.lineNumbers && line.height != 0 ? gutterWrap : lineElement);
    ...
    function updateSelectionRange(cm) {
        ...
        if (sel.from.line == sel.to.line) {
            ...
            if (singleLine) {
                ...
                else middleBot = drawForLine(sel.to.line, collapsedSpanAtStart(toObj) ? null : 0, sel.to.ch, true);
    ...
    function startWorker(cm, time) {
        if (cm.view.mode.startState && cm.view.frontier < cm.display.showingTo) cm.view.highlight.set(time, bind(highlightWorker, cm));
    }
    ...
    function measureLine(cm, line) {
        ...
        for (var i = 0; i < cache.length; ++i) {
            var memo = cache[i];
            if (memo.text == line.text && memo.markedSpans == line.markedSpans && display.scroller.clientWidth == memo.width) return memo.measure;
        }
    ...
    function coordsChar(cm, x, y) {
        ...
        if (lineNo >= doc.size) return {
            line: doc.size - 1,
            ch: getLine(doc, doc.size - 1).text.length
        };
        if (x < 0) x = 0;
        ...
        for (;;) {
            ...
            if (merged && found.ch >= mergedPos.from.ch) lineNo = mergedPos.to.line;
            else return found;
        }
    }
    ...
    function endOperation(cm) {
        ...
        if (view.maxLineChanged && !cm.options.lineWrapping) {
            ...
            if (maxScrollLeft < view.scrollLeft) setScrollLeft(cm, Math.min(display.scroller.scrollLeft, maxScrollLeft), true);
        }
        ...
        if (op.changes.length || newScrollPos && newScrollPos.scrollTop != null) updated = updateDisplay(cm, op.changes, newScrollPos && newScrollPos.scrollTop);
    ...
    function readInput(cm) {
        ...
        var from = sel.from,
            to = sel.to;
        if (same < prevInput.length) from = {
            line: from.line,
            ch: from.ch - (prevInput.length - same)
        };
        else if (view.overwrite && posEq(from, to) && !cm.display.pasteIncoming) to = {
            line: to.line,
    ...
    function resetInput(cm, user) {
        ...
        if (!posEq(view.sel.from, view.sel.to)) {
            ...
            minimal = hasCopyEvent && (view.sel.to.line - view.sel.from.line > 100 || (selected = cm.getSelection()).length > 1000);
    ...
    function focusInput(cm) {
        if (cm.options.readOnly != "nocursor" && (ie || document.activeElement != cm.display.input)) cm.display.input.focus();
    }
    ...
    function eventInWidget(display, e) {
        for (var n = e_target(e); n != display.wrapper; n = n.parentNode) {
            if (!n) return true;
            if (/\bCodeMirror-(?:line)?widget\b/.test(n.className) || n.parentNode == display.sizer && n != display.mover) return true;
        }
    }
    ...
    function posFromMouse(cm, e, liberal) {
        ...
        if (!liberal) {
            ...
            if (target == display.scrollbarH || target == display.scrollbarH.firstChild || target == display.scrollbarV || target == display.scrollbarV.firstChild || target == display.scrollbarFiller) return null;
        }
    ...
    function onDrop(e) {
        var cm = this;
        if (eventInWidget(cm.display, e) || (cm.options.onDragEvent && cm.options.onDragEvent(cm, addStop(e)))) return;
        e_preventDefault(e);
    ...
    function handleKeyBinding(cm, e) {
        ...
        if (next && !isModifierKey(e)) maybeTransition = setTimeout(function() {
            if (getKeyMap(cm.options.keyMap) == startMap) cm.options.keyMap = (next.call ? next.call(null, cm) : next);
        }, 50);
        ...
        if (e_prop(e, "shiftKey")) {
            ...
            }, stop) || lookupKey(name, keymaps, function(b) {
    ...
    function onKeyDown(e) {
        ...
        if (opera) {
            ...
            if (!handled && code == 88 && !hasCopyEvent && e_prop(e, mac ? "metaKey" : "ctrlKey")) cm.replaceSelection("");
        }
    }
    ...
    function onKeyPress(e) {
        ...
        if (this.options.electricChars && this.view.mode.electricChars && this.options.smartIndent && !isReadOnly(this) && this.view.mode.electricChars.indexOf(ch) > -1) setTimeout(operation(cm, function() {
    ...
    function onFocus(cm) {
        ...
        if (!cm.view.focused) {
            ...
            if (cm.display.scroller.className.search(/\bCodeMirror-focused\b/) == -1) cm.display.scroller.className += " CodeMirror-focused";
            resetInput(cm, true);
        }
        slowPoll(cm);
        restartBlink(cm);
    ...
    function onContextMenu(cm, e) {
        ...
        if (posEq(sel.from, sel.to) || posLess(pos, sel.from) || !posLess(pos, sel.to)) operation(cm, setSelection)(cm, pos, pos);

        var oldCSS = display.input.style.cssText;
        display.inputDiv.style.position = "absolute";
        display.input.style.cssText = "position: fixed; width: 30px; height: 30px; top: " + (e.clientY - 5) + "px; left: " + (e.clientX - 5) + "px; z-index: 1000; background: white; outline: none;" + "border-width: 0; outline: none; overflow: hidden; opacity: .05; filter: alpha(opacity=5);";
        focusInput(cm);
        resetInput(cm, true);
    ...
    function updateDoc(cm, from, to, newText, selUpdate, origin) {
        ...
        if (split) {
            ...
            if (split.length) return updateDocInner(cm, split[0].from, split[0].to, newText, selUpdate, origin);
        } else {
    ...
    function unredoHelper(cm, type) {
        ...
        for (var i = set.events.length - 1; i >= 0; i -= 1) {
            ...
            updateDocNoUndo(cm, {
                ...
                line: end - 1,
                ch: getLine(doc, end - 1).text.length
            },
    ...
    function updateDocNoUndo(cm, from, to, lines, selUpdate, origin) {
        ...
        if (from.ch == 0 && to.ch == 0 && hlText(lastHL) == "") {
            ...
            if (lines.length == 1) {
                updateLine(cm, firstLine, firstLine.text.slice(0, from.ch) + hlText(lines[0]) + firstLine.text.slice(to.ch), hlSpans(lines[0]));
            } else {
            ...
            updateLine(cm, firstLine, firstLine.text.slice(0, from.ch) + hlText(lines[0]) + lastLine.text.slice(to.ch), hlSpans(lines[0]));
        ...
        if (selUpdate && typeof selUpdate != "string") {
            ...
            var adjustPos = function(pos) {
                ...
                var ch = pos.ch;
                if (pos.line == to.line) ch += hlText(lastHL).length - (to.ch - (to.line == from.line ? from.ch : 0));
                return {
                    line: line,
                    ch: ch
                };
    ...
    function setSelection(cm, anchor, head, bias, checkAtomic) {
        ...
        if (checkAtomic || !posEq(anchor, sel.anchor)) anchor = skipAtomic(cm, anchor, bias, checkAtomic != "push");
        if (checkAtomic || !posEq(head, sel.head)) head = skipAtomic(cm, head, bias, checkAtomic != "push");
    ...
    function indentLine(cm, n, how, aggressive) {
        ...
        if (indentString != curSpaceString) replaceRange(cm, indentString, {
            line: n,
            ch: 0
        }, {
            line: n,
    ...
    CodeMirror.prototype = {
        ...
        setValue: operation(null, function(code) {
            ...
            updateDocInner(this, top, {
                line: doc.size - 1,
                ch: lastLen
            }, splitLines(code), top, top, "setValue");
        }),
        ...
        setBookmark: operation(null, function(pos, widget) {
            ...
            return markText(this, pos, pos, widget ? {
                replacedWith: widget
            } : {}, "bookmark");
        }),
        ...
        addWidget: function(pos, node, scroll, vert, horiz) {
            ...
            if (vert == "over") top = pos.top;
                ...
                if (pos.bottom + node.offsetHeight > vspace && pos.top > node.offsetHeight) top = pos.top - node.offsetHeight;
        ...
        setCursor: operation(null, function(line, ch, extend) {
            var pos = clipPos(this.view.doc, typeof line == "number" ? {
                line: line,
                ch: ch || 0
            } : line);
        ...
        setLine: operation(null, function(line, text) {
            if (isLine(this.view.doc, line)) replaceRange(this, text, {
                line: line,
                ch: 0
            }, {
                line: line,
                ch: getLine(this.view.doc, line).text.length
            });
        }),

        removeLine: operation(null, function(line) {
            if (isLine(this.view.doc, line)) replaceRange(this, "", {
                line: line,
                ch: 0
            }, clipPos(this.view.doc, {
                line: line + 1,
                ch: 0
            }));
        }),
        ...
        moveH: operation(null, function(dir, unit) {
            ...
            if (sel.shift || sel.extend || posEq(sel.from, sel.to)) pos = findPosH(this, dir, unit, this.options.rtlMoveVisually);
        ...
        toggleOverwrite: function() {
            if (this.view.overwrite = !this.view.overwrite) this.display.cursor.className += " CodeMirror-overwrite";
            else this.display.cursor.className = this.display.cursor.className.replace(" CodeMirror-overwrite", "");
        },

        posFromIndex: function(off) {
            var lineNo = 0,
                ch, doc = this.view.doc;
            doc.iter(0, doc.size, function(line) {
        ...
        refresh: function(that) {
            clearCaches(that || this);
            var sTop = this.view.scrollTop,
                sLeft = this.view.scrollLeft;
            if (this.display.scroller.scrollHeight > sTop) this.display.scrollbarV.scrollTop = this.display.scroller.scrollTop = sTop;
            if (this.display.scroller.scrollWidth > sLeft) this.display.scrollbarH.scrollLeft = this.display.scroller.scrollLeft = sLeft;
    ...
    TextMarker.prototype.clear = function() {
        ...
        for (var i = 0; i < this.lines.length; ++i) {
            ...
            else if (this.collapsed && !lineIsHidden(line)) updateLineHeight(line, textHeight(this.cm.display));
        }
        if (this.collapsed && !this.cm.options.lineWrapping) for (var i = 0; i < this.lines.length; ++i) {
            var visual = visualLine(view.doc, this.lines[i]),
                len = lineLength(view.doc, visual);
            if (len > view.maxLineLength) {
                view.maxLine = visual;
                view.maxLineLength = len;
                view.maxLineChanged = true;
    ...
    function markText(cm, from, to, options, type) {
        ...
        if (options) for (var opt in options) if (options.hasOwnProperty(opt)) marker[opt] = options[opt];
        ...
        doc.iter(curLine, to.line + 1, function(line) {
            if (marker.collapsed && !cm.options.lineWrapping && visualLine(doc, line) == cm.view.maxLine) cm.curOp.updateMaxLine = true;
            var span = {
                from: null,
                to: null,
                marker: marker
    ...
    function removeReadOnlyRanges(doc, from, to) {
        ...
        for (var i = 0; i < markers.length; ++i) {
            ...
            for (var j = 0; j < parts.length; ++j) {
                ...
                var newParts = [j, 1];
                if (posLess(p.from, m.from)) newParts.push({
                    from: p.from,
                    to: m.from
                });
                if (posLess(m.to, p.to)) newParts.push({
                    from: m.to,
                    to: p.to
                });
    ...
    function lineIsHiddenInner(line, span) {
        if (span.to == null) {
            var end = span.marker.find().to,
                endLine = getLine(lineDoc(line), end.line);
            return lineIsHiddenInner(endLine, getMarkedSpanFor(endLine.markedSpans, span.marker));
        }
        if (span.marker.inclusiveRight && span.to == line.text.length) return true;
        for (var sp, i = 0; i < line.markedSpans.length; ++i) {
            sp = line.markedSpans[i];
            if (sp.marker.collapsed && sp.from == span.to && (sp.marker.inclusiveLeft || span.marker.inclusiveRight) && lineIsHiddenInner(line, sp)) return true;
        }
    }
    ...
    function addLineWidget(cm, handle, node, options) {
        ...
        changeLine(cm, handle, function(line) {
            ...
            if (!lineIsHidden(line) || widget.showIfHidden) {
                ...
                if (aboveVisible) setTimeout(function() {
    ...
    function highlightLine(cm, line, state) {
        ...
        for (var o = 0; o < cm.view.overlays.length; ++o) {
            var overlay = cm.view.overlays[o],
                i = 1;
    ...
    function getLineStyles(cm, line) {
        if (!line.styles || line.styles[0] != cm.view.modeGen) line.styles = highlightLine(cm, line, line.stateAfter = getStateBefore(cm, lineNo(line)));
        return line.styles;
    }
    ...
    function lineContent(cm, realLine, measure) {
        ...
        if (measure && !builder.addedOne) measure[0] = builder.pre.appendChild(simple ? elt("span", "\u00a0") : zeroWidthElement(cm.display.measure));
        if (!builder.pre.firstChild && !lineIsHidden(realLine)) builder.pre.appendChild(document.createTextNode("\u00a0"));
    ...
    function buildTokenMeasure(builder, text, style, startStyle, endStyle) {
        for (var i = 0; i < text.length; ++i) {
            if (i && i < text.length && builder.cm.options.lineWrapping && spanAffectsWrapping.test(text.slice(i - 1, i + 1))) builder.pre.appendChild(elt("wbr"));
    ...
    function addChange(cm, start, added, old, origin, fromBefore, toBefore, fromAfter, toAfter) {
        ...
        if (cur && (history.lastOp == cm.curOp.id || history.lastOrigin == origin && (origin == "input" || origin == "delete") && history.lastTime > time - 600)) {
    ...
    function moveVisually(line, start, dir, byUnit) {
        ...
        for (var i = 0; i < bidi.length; ++i) {
            ...
            if ((part.from < start && part.to > start) || (sticky && (part.from == start || part.to == start))) break;
        }
