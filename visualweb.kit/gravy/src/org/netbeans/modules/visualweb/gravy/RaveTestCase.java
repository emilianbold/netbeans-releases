...
public class RaveTestCase extends JellyTestCase {
    ...
    /** Full Thread Dump feature in case of failure is disabled by default
     */
    public boolean doFullThreadDump = Boolean.valueOf(System.getProperty("xtest.FullThreadDump.do", "false")).booleanValue();
...
//        }
	...
	try {
	    ...
	    // Full Thread Dump is performed when test fails and in dependency on system property
	    if (doFullThreadDump) {
		try {
                    doFullThreadDump();
		}
                    ...
                    e4.printStackTrace();
                    System.out.println("Dump for all threads is not created");
                }
	    }
    ...
    private void doFullThreadDump() {
        try {
            ...
            System.out.println();
            System.out.println("Full Thread Dump - BEGIN");
        ...
        try {
            FileOutputStream fos = new FileOutputStream(getWorkDir().getAbsolutePath()+File.separator+"threads.dump");
            PrintWriter pw = new PrintWriter(fos);
            pw.println("Full thread dump Java HotSpot(TM) Client VM");
            pw.println();
            Map threadsMap = Thread.getAllStackTraces();
            Iterator keys = threadsMap.keySet().iterator();
            Iterator values = threadsMap.values().iterator();
            Thread key;
            StackTraceElement[] value;
            while (keys.hasNext()) {
                key = ((Thread) (keys.next()));
                String daemon = key.isDaemon()?" daemon":"";
                pw.println("\"" + key.getName() + "\"" + daemon + 
                           " prio=" + key.getPriority() + " id=" + key.getId() + 
                           " ThreadGroup=" + key.getThreadGroup().getName());
                value = ((StackTraceElement[]) (values.next()));
                for (int i = 0; i < value.length; i++)
                    pw.println("    " + value[i].toString());
                pw.println();
            }
            pw.close();
        }
