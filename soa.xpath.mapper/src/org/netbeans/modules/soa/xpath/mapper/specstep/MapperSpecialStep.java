 ...
 */
public class MapperSpecialStep implements XPathSpecialStep {

    private SsType mSsType;
    private XPathSchemaContext mSContext;

    public MapperSpecialStep(SsType ssType, XPathSchemaContext sParentContext) {
        mSsType = ssType;
        //
        // Schema contex available only for the following types:
        switch (ssType) {
            case ALL_ATTRIBUTES:
            case ALL_ELEMENTS:
            case NODE:
                mSContext = new WildcardSchemaContext(sParentContext, ssType);
                break;
            default:
                assert false : "Incorrect ssType"; // NOI18N
                break;
        }
    }
    ...
    public int hashCode() {
        int hash = 3;
        hash = 71 * hash + (this.mSsType != null ? this.mSsType.hashCode() : 0);
        return hash;
    }
