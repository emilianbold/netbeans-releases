 ...
 */
public class MapperDeleteGraphSelectionAction extends NodeAction implements
        MapperSelectionListener
{
    private Mapper mapper;
    
    public MapperDeleteGraphSelectionAction() {
        super();
    }
    ...
    protected void performAction(Node[] arg0) {
        mapper.getActionMap().get(SystemAction.get(DeleteAction.class).getActionMapKey()).
                actionPerformed(new ActionEvent(mapper, 0, "Bpel-Delet-Action"));
    }
    ...
    protected boolean enable(Node[] arg0) {
        ...
        if (mapper.getSelectionModel().getSelectedLinks().size() > 0) {
            return true;  
        }
    ...
    public void mapperSelectionChanged(MapperSelectionEvent event) {
        if (mapper == null) { setEnabled(false); }
