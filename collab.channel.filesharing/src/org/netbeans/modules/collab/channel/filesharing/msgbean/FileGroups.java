 ...
 */
/**
 *        This generated bean class FileGroups
 *        matches the schema element '_file-groups'.
 *
...
public class FileGroups {
    private java.util.List _FileGroup = new java.util.ArrayList(); // List<FileGroup>

    public FileGroups() {
    }

    // Deep copy
    public FileGroups(org.netbeans.modules.collab.channel.filesharing.msgbean.FileGroups source) {
        for (java.util.Iterator it = source._FileGroup.iterator(); it.hasNext();) {
            _FileGroup.add(
                new org.netbeans.modules.collab.channel.filesharing.msgbean.FileGroup(
                    (org.netbeans.modules.collab.channel.filesharing.msgbean.FileGroup) it.next()
                )
            );
        }
    }
    ...
    public void setFileGroup(org.netbeans.modules.collab.channel.filesharing.msgbean.FileGroup[] value) {
        if (value == null) {
            value = new FileGroup[0];
        }

        _FileGroup.clear();

        for (int i = 0; i < value.length; ++i) {
            _FileGroup.add(value[i]);
        }
    }

    public void setFileGroup(int index, org.netbeans.modules.collab.channel.filesharing.msgbean.FileGroup value) {
        _FileGroup.set(index, value);
    }

    public org.netbeans.modules.collab.channel.filesharing.msgbean.FileGroup[] getFileGroup() {
        FileGroup[] arr = new FileGroup[_FileGroup.size()];

        return (FileGroup[]) _FileGroup.toArray(arr);
    }

    public java.util.List fetchFileGroupList() {
        return _FileGroup;
    }

    public org.netbeans.modules.collab.channel.filesharing.msgbean.FileGroup getFileGroup(int index) {
        return (FileGroup) _FileGroup.get(index);
    }

    // Return the number of fileGroup
    public int sizeFileGroup() {
        return _FileGroup.size();
    }

    public int addFileGroup(org.netbeans.modules.collab.channel.filesharing.msgbean.FileGroup value) {
        _FileGroup.add(value);

        return _FileGroup.size() - 1;
    }

    // Search from the end looking for @param value, and then remove it.
    public int removeFileGroup(org.netbeans.modules.collab.channel.filesharing.msgbean.FileGroup value) {
        int pos = _FileGroup.indexOf(value);

        if (pos >= 0) {
            _FileGroup.remove(pos);
        }
    ...
    throws java.io.IOException {
        ...
        for (java.util.Iterator it = _FileGroup.iterator(); it.hasNext();) {
            org.netbeans.modules.collab.channel.filesharing.msgbean.FileGroup element = (org.netbeans.modules.collab.channel.filesharing.msgbean.FileGroup) it.next();

            if (element != null) {
                element.writeNode(out, "file-group", nextIndent);
            }
        }
    ...
    public void readNode(org.w3c.dom.Node node) {
        ...
        for (int i = 0, size = children.getLength(); i < size; ++i) {
            ...
            if (childNodeName == "file-group") {
                FileGroup aFileGroup = new org.netbeans.modules.collab.channel.filesharing.msgbean.FileGroup();
                aFileGroup.readNode(childNode);
                _FileGroup.add(aFileGroup);
            } else {
    ...
    public void changePropertyByName(String name, Object value) {
        ...
        if (name == "fileGroup") {
            addFileGroup((FileGroup) value);
        } else if (name == "fileGroup[]") {
            setFileGroup((FileGroup[]) value);
        } else {
            throw new IllegalArgumentException(name + " is not a valid property name for FileGroups");
        }
    }
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof org.netbeans.modules.collab.channel.filesharing.msgbean.FileGroups)) {
            return false;
        }

        org.netbeans.modules.collab.channel.filesharing.msgbean.FileGroups inst = (org.netbeans.modules.collab.channel.filesharing.msgbean.FileGroups) o;

        if (sizeFileGroup() != inst.sizeFileGroup()) {
            return false;
        }

        // Compare every element.
        for (
            java.util.Iterator it = _FileGroup.iterator(), it2 = inst._FileGroup.iterator();
                it.hasNext() && it2.hasNext();
        ) {
            org.netbeans.modules.collab.channel.filesharing.msgbean.FileGroup element = (org.netbeans.modules.collab.channel.filesharing.msgbean.FileGroup) it.next();
            org.netbeans.modules.collab.channel.filesharing.msgbean.FileGroup element2 = (org.netbeans.modules.collab.channel.filesharing.msgbean.FileGroup) it2.next();
    ...
    public int hashCode() {
        int result = 17;
        result = (37 * result) + ((_FileGroup == null) ? 0 : _FileGroup.hashCode());
