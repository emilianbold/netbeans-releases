 ...
 */
/**
 *        This generated bean class FileChangedData
 *        matches the schema element '_file-changed-data'.
 *
...
public class FileChangedData {
    ...
    private java.lang.String _Digest;
    private java.util.List _RegionChanged = new java.util.ArrayList(); // List<RegionChanged>
    ...
    // Deep copy
    public FileChangedData(org.netbeans.modules.collab.channel.filesharing.msgbean.FileChangedData source) {
        _FileName = source._FileName;
        _Digest = source._Digest;

        for (java.util.Iterator it = source._RegionChanged.iterator(); it.hasNext();) {
            _RegionChanged.add(
                new org.netbeans.modules.collab.channel.filesharing.msgbean.RegionChanged(
                    (org.netbeans.modules.collab.channel.filesharing.msgbean.RegionChanged) it.next()
                )
            );
        }
    }
    ...
    public void setRegionChanged(org.netbeans.modules.collab.channel.filesharing.msgbean.RegionChanged[] value) {
        if (value == null) {
            value = new RegionChanged[0];
        }

        _RegionChanged.clear();

        for (int i = 0; i < value.length; ++i) {
            _RegionChanged.add(value[i]);
        }
    }

    public void setRegionChanged(
        int index, org.netbeans.modules.collab.channel.filesharing.msgbean.RegionChanged value
    ) {
        _RegionChanged.set(index, value);
    }

    public org.netbeans.modules.collab.channel.filesharing.msgbean.RegionChanged[] getRegionChanged() {
        RegionChanged[] arr = new RegionChanged[_RegionChanged.size()];

        return (RegionChanged[]) _RegionChanged.toArray(arr);
    }

    public java.util.List fetchRegionChangedList() {
        return _RegionChanged;
    }

    public org.netbeans.modules.collab.channel.filesharing.msgbean.RegionChanged getRegionChanged(int index) {
        return (RegionChanged) _RegionChanged.get(index);
    }

    // Return the number of regionChanged
    public int sizeRegionChanged() {
        return _RegionChanged.size();
    }

    public int addRegionChanged(org.netbeans.modules.collab.channel.filesharing.msgbean.RegionChanged value) {
        _RegionChanged.add(value);

        return _RegionChanged.size() - 1;
    }

    // Search from the end looking for @param value, and then remove it.
    public int removeRegionChanged(org.netbeans.modules.collab.channel.filesharing.msgbean.RegionChanged value) {
        int pos = _RegionChanged.indexOf(value);

        if (pos >= 0) {
            _RegionChanged.remove(pos);
        }
    ...
    throws java.io.IOException {
        ...
        if (_Digest != null) {
            out.write(nextIndent);
            out.write("<digest"); // NOI18N
            out.write(">"); // NOI18N
            org.netbeans.modules.collab.channel.filesharing.msgbean.CCollab.writeXML(out, _Digest, false);
            out.write("</digest>\n"); // NOI18N
        }

        for (java.util.Iterator it = _RegionChanged.iterator(); it.hasNext();) {
            org.netbeans.modules.collab.channel.filesharing.msgbean.RegionChanged element = (org.netbeans.modules.collab.channel.filesharing.msgbean.RegionChanged) it.next();

            if (element != null) {
                element.writeNode(out, "region-changed", nextIndent);
            }
        }
    ...
    public void readNode(org.w3c.dom.Node node) {
        ...
        for (int i = 0, size = children.getLength(); i < size; ++i) {
            ...
            } else if (childNodeName == "digest") {
                _Digest = childNodeValue;
            } else if (childNodeName == "region-changed") {
                RegionChanged aRegionChanged = new org.netbeans.modules.collab.channel.filesharing.msgbean.RegionChanged();
                aRegionChanged.readNode(childNode);
                _RegionChanged.add(aRegionChanged);
            } else {
    ...
    public void changePropertyByName(String name, Object value) {
        ...
        } else if (name == "regionChanged") {
            addRegionChanged((RegionChanged) value);
        } else if (name == "regionChanged[]") {
            setRegionChanged((RegionChanged[]) value);
        } else {
            throw new IllegalArgumentException(name + " is not a valid property name for FileChangedData");
        }
    }
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof org.netbeans.modules.collab.channel.filesharing.msgbean.FileChangedData)) {
            return false;
        }

        org.netbeans.modules.collab.channel.filesharing.msgbean.FileChangedData inst = (org.netbeans.modules.collab.channel.filesharing.msgbean.FileChangedData) o;

        if (!((_FileName == null) ? (inst._FileName == null) : _FileName.equals(inst._FileName))) {
            return false;
        }

        if (!((_Digest == null) ? (inst._Digest == null) : _Digest.equals(inst._Digest))) {
            return false;
        }

        if (sizeRegionChanged() != inst.sizeRegionChanged()) {
            return false;
        }

        // Compare every element.
        for (
            java.util.Iterator it = _RegionChanged.iterator(), it2 = inst._RegionChanged.iterator();
                it.hasNext() && it2.hasNext();
        ) {
            org.netbeans.modules.collab.channel.filesharing.msgbean.RegionChanged element = (org.netbeans.modules.collab.channel.filesharing.msgbean.RegionChanged) it.next();
            org.netbeans.modules.collab.channel.filesharing.msgbean.RegionChanged element2 = (org.netbeans.modules.collab.channel.filesharing.msgbean.RegionChanged) it2.next();
    ...
    public int hashCode() {
        ...
        result = (37 * result) + ((_Digest == null) ? 0 : _Digest.hashCode());
        result = (37 * result) + ((_RegionChanged == null) ? 0 : _RegionChanged.hashCode());
