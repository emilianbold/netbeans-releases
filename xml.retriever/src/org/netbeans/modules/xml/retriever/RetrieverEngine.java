 ...
 * @author girix
 */
public class RetrieverEngine implements Runnable{
    ...
    public void run() {
        ProgressHandle ph = ProgressHandleFactory.createHandle(
                NbBundle.getMessage(RetrieverEngine.class,"LBL_PROGRESSBAR_Retrieve_XML"),
                new Cancellable(){
            public boolean cancel() {
                synchronized(RetrieverEngine.this){
    ...
    private void handleException(RetrieveEntry rent, Exception ex) {
        ...
        if(ex instanceof UnknownHostException){
            String errorMess = NbBundle.getMessage(RetrieverEngine.class, "MSG_unknown_host_p1")+ex.getLocalizedMessage()+"\n"+NbBundle.getMessage(RetrieverEngine.class, "MSG_unknownhost_p2");
        ...
        if(ex instanceof FileNotFoundException){
            String errorMess = NbBundle.getMessage(RetrieverEngine.class, "MSG_unknown_file", ex.getMessage());
            outputError(errorMess);
            if(showErrorPopup){
                NotifyDescriptor.Message ndm = new NotifyDescriptor.Message(errorMess, NotifyDescriptor.Message.ERROR_MESSAGE);
                DialogDisplayer.getDefault().notify(ndm);
        ...
        if(ex instanceof IOException){
            String exStr = NbBundle.getMessage(RetrieverEngine.class, IConstants.EXCEPTION_CYCLIC_REFERENCE_INDICATOR);
            ...
            String errorMess = NbBundle.getMessage(RetrieverEngine.class, "MSG_general_io_error", ex.getMessage());
            if(showErrorPopup){
                NotifyDescriptor.Message ndm = new NotifyDescriptor.Message(errorMess, NotifyDescriptor.Message.ERROR_MESSAGE);
                DialogDisplayer.getDefault().notify(ndm);
            }
    ...
    String opTabTitle = NbBundle.getMessage(RetrieverEngine.class,
    ...
    private void outputError(String str){
        ...
        err.println(NbBundle.getMessage(RetrieverEngine.class, "MSG_Error_str",
                str)); //NOI18N
        err.flush();
    }
