 ...
 * @version 1.0
 */
public class GoToAction extends TMapAbstractNodeAction {

    private static final long serialVersionUID = 1L;

    private static ActSubMenuModel model = new ActSubMenuModel(null);
    private static TMapAbstractNodeAction[] GO_TO_ACTIONS = new TMapAbstractNodeAction[] {
            (TMapAbstractNodeAction)SystemAction.get(GoToSourceAction.class),
            (TMapAbstractNodeAction)SystemAction.get(GoToTreeAction.class)
    };

    public GoToAction() {
    }

    public static KeyStroke getKeyStroke(Class<? extends TMapAbstractNodeAction> clazz) {
        if (clazz == null) {
            return null;
        }
    ...
    public final String getBundleName() {
        return NbBundle.getMessage(TMapAbstractNodeAction.class, "CTL_GoToAction"); // NOI18N
    }
    ...
    private static final void performAction(Node[] nodes, SystemAction gotoAction) {
        if (gotoAction instanceof TMapAbstractNodeAction) {
            ((TMapAbstractNodeAction)gotoAction).performAction(nodes);
        }
    }

    // TODO m
    public static final TMapAbstractNodeAction[] getGoToActions(Node[] nodes) {
        List<TMapAbstractNodeAction> availableGotoActions = new ArrayList<TMapAbstractNodeAction>();
        if (nodes != null && nodes.length > 0) {
            for (TMapAbstractNodeAction gotoAction : GO_TO_ACTIONS) {
                if (gotoAction.enable(nodes)) {
                    availableGotoActions.add(gotoAction);
                }
            }
            return availableGotoActions.toArray(new TMapAbstractNodeAction[availableGotoActions.size()]);
        }
        return null;
    }
    ...
    private static final TMapAbstractNodeAction[] getGoToActions() {
    ...
    private static class ActSubMenuModel extends EventListenerList implements Actions.SubMenuModel {
        ...
        public String getLabel(int index) {
            TMapAbstractNodeAction[] gotoActions = getGoToActions(getNodes());
            if (gotoActions != null && index >= 0 && index < gotoActions.length) {
    ...
    @Override
    protected void performAction(TMapComponent[] component) {
        // do nothing
    }
