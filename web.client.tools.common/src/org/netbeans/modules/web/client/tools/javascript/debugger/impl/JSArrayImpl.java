...
public class JSArrayImpl extends JSObjectImpl implements JSArray {
    JSArrayImpl(JSCallStackFrameImpl callStackFrame, String fullName) {
        super(callStackFrame, fullName, JSValue.TypeOf.ARRAY, "Array");
    }
    ...
    public JSProperty getIndexedProperty(int index) {
        final JSProperty[] props = getProperties();
        if (props != null) {
            for (JSProperty prop : getProperties()) {
                if (prop.getName().equals(String.valueOf(index))) {
                    return prop;
                }
            }
        }
