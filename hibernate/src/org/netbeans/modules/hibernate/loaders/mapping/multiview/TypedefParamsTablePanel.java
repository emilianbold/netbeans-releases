 ...
 * @author Dongmei Cao
 */
public class TypedefParamsTablePanel extends DefaultTablePanel {
    
    private TypedefParamsTableModel model;
    private HibernateMappingDataObject mappingDataObject;

    /** Creates new form EventTablePanel */
    public TypedefParamsTablePanel(final HibernateMappingDataObject dObj, final TypedefParamsTableModel model) {
    	super(model);
    	this.model=model;
    ...
    private class TableActionListener implements java.awt.event.ActionListener {
        ...
        public void actionPerformed(java.awt.event.ActionEvent evt) {
            ...
            if (!add) {
                String attribute = (String) model.getValueAt(row, 0);
                String inherit = (String)model.getValueAt(row, 1 );
                String value = (String)model.getValueAt(row, 2 );
                dialogPanel.initValues(attribute, inherit, value);
            }
            
            EditDialog dialog = new EditDialog(dialogPanel, NbBundle.getMessage(TypedefParamsTablePanel.class, "LBL_Meta"), add) {
                ...
                protected String validate() {
                    ...
                    if (attribute.length() == 0) {
                        return NbBundle.getMessage(TypedefParamsTablePanel.class, "TXT_Meta_Attribute_Empty");
                    } 
                    
                    if( value.length() == 0 ) {
                        return NbBundle.getMessage(TypedefParamsTablePanel.class, "TXT_Meta_Value_Empty");
                    }
