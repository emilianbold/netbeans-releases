...
public class GlobalDescriptorRegistryTest extends TestCase {
    ...
    /**
     *  Checking if descriptor has been properly removed
     */
    public void testGlobalDecriptorRemoving(){
        try {
            this.firstCD.delete();
            this.secondCD.delete();
        } catch (IOException ex) {
            ex.printStackTrace();
        }
        assertFalse(this.firstCD.isValid());
        assertFalse(this.secondCD.isValid());
        document.getTransactionManager().readAccess(new Runnable() {
            public void run() {
                ComponentDescriptor cdFirst = document.getDescriptorRegistry().getComponentDescriptor(FirstCD.TYPEID_CLASS);
                ComponentDescriptor cdSecond = document.getDescriptorRegistry().getComponentDescriptor(SecondCD.TYPEID_CLASS);
