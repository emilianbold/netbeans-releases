...
//        assert kind == Kind.ARRAY;
    ...
    void collectAllComponentReferences (Collection<DesignComponent> references) {
        assert Debug.isFriend (DesignComponent.class, "writeProperty") // NOI18N
                ||  Debug.isFriend (DesignComponent.class, "setComponentDescriptor") // NOI18N
                ||  Debug.isFriend (PropertyValue.class, "collectAllComponentReferences") // NOI18N
                ||  Debug.isFriend (Debug.class, "collectAllComponentReferences"); // NOI18N
        switch (kind) {
            case USERCODE:
            case NULL:
            case VALUE:
    ...
    public boolean isCompatible (PropertyDescriptor propertyDescriptor) {
        ...
        if (kind == Kind.NULL) {
            if (! propertyDescriptor.isAllowNull ()) {
                ...
                if (defaultValue == null  ||  defaultValue.getKind () != PropertyValue.Kind.NULL) // HACK for PropertyDescriptor for disallowing to use null values
                    return false;
            }
     ...
     * @return the property value
     */
    public static PropertyValue deserialize (String serialized, DesignDocument document, TypeID type) {
