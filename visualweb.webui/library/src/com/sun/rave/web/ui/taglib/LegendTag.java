...
public class LegendTag extends UIComponentTag {
    ...
    protected void setProperties(UIComponent _component) {
        ...
        if (position != null) {
            if (isValueReference(position)) {
                ValueBinding _vb = getFacesContext().getApplication().createValueBinding(position);
                _component.setValueBinding("position", _vb);
            } else {
                _component.getAttributes().put("position", position);
            }
        }
        if (style != null) {
