...
/**
 * For MySQL Database code generations using from JDBC Code generation.
 * @author karthik
 */
public class MySQLDB extends BaseDB {

    /* Defines relative location of vendor-specific template configuration resource file. */
    private static final String TEMPLATE_FILE = "/org/netbeans/modules/sql/framework/codegen/mysql/config/templates.xml";

    /* Reference to JDBC operatordef info file. */
    private static final String MYSQL_OPERATOR_DEFINITION_FILE = "org/netbeans/modules/sql/framework/codegen/mysql/config/operator-script.xml";

    public Statements createStatements() {
        return new MySQLStatements(this);
    }
    ...
    public SQLOperatorFactory getOperatorFactory() {
        if (factory == null) {
            factory = new SQLOperatorFactory(MYSQL_OPERATOR_DEFINITION_FILE, super.getOperatorFactory());
        }
        return factory;
    }

    public AbstractGeneratorFactory createGeneratorFactory() {
        return new MySQLGeneratorFactory(this);
    }
