...
import org.netbeans.modules.etl.utils.ETLException;
...
public class ExecuteTestCookie implements Node.Cookie {
    ...
    public void start() {
        try {
            etlView = DataObjectProvider.getProvider()
            .getActiveDataObject().getETLEditorTC();
        } catch (Exception ex) {
            // ignore
        }
        if (etlView != null) {
        ...
        public synchronized void executionPerformed(ETLEngineExecEvent event) {
            ...
            || (event.getStatus() == ETLEngine.STATUS_COLLAB_EXCEPTION)) {
                ...
                try {
                    ...
                    msgBuf.append(NbBundle.getMessage(ExecuteTestCookie.class, "MSG_execution_time",
                            new Long((endTime - startTime) / 1000),
                            new Long((endTime - startTime) % 1000)));
                    ...
                    if (event.getStatus() == ETLEngine.STATUS_COLLAB_COMPLETED){                        
                        ...
                        while(it.hasNext()) {                            
                            rowsProcessed += engine.getContext().getStatistics().getRowsInsertedCount(
                                    (String)it.next());
                        }
    ...
    private class RunEngineWorkerThread extends SwingWorker {
        ...
        public Object construct() {
            if (execModel != null) {
                ...
                throwableList.add(new ETLException("No eTL collaboration model to execute"));
