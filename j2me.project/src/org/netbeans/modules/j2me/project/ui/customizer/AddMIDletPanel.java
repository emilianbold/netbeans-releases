...
public class AddMIDletPanel extends JPanel implements DocumentListener, ActionListener {
    ...
    public boolean isStateValid() {
        if (getName().length() == 0 || getName().indexOf(',') >= 0) {
            ((ErrorPanel) errorPanel).setErrorBundleMessage("ERR_AddMID_InvalidName"); //NOI18N
            return false;
        }
        if (getClazz().length() == 0 || !isValidClassName(getClazz())) {
            ((ErrorPanel) errorPanel).setErrorBundleMessage("ERR_AddMID_InvalidClass"); //NOI18N
            return false;
        }
        if (getIcon().indexOf(',') >= 0) {
            ((ErrorPanel) errorPanel).setErrorBundleMessage("ERR_AddMID_InvalidIcon"); //NOI18N
            return false;
        }
