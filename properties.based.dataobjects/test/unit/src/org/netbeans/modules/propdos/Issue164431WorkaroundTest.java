...
public class Issue164431WorkaroundTest {
    @Test
    public void testAbstractLookupDoesNotCache() throws Exception {
        A a = new A();
        Lookup one = Lookup.EMPTY;
        Issue164431Workaround lkp = new Issue164431Workaround(a, one);
        B b = lkp.lookup(B.class);
        assertNotNull(b);
        lkp.replaceConverter(new A());
        B b2 = lkp.lookup(B.class);
        assertNotSame(b, b2);
    }
