...
public final class RemoteScreenshot {
    ...
    public ComponentInfo findAt(int x, int y) {
        return findAt(componentInfo, x, y);
    }
    
    public static ComponentInfo findAt(ComponentInfo ci, int x, int y) {
        Rectangle bounds = ci.getBounds();
        ...
        if (subComponents != null) {
            for (int i = 0; i < subComponents.length; i++) {
                ...
                if (sb.contains(x, y)) {
                    ComponentInfo tci = findAt(subComponents[i], x, y);
