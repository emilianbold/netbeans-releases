...
public class ShowDataProjectOperations implements DeleteOperationImplementation, CopyOperationImplementation, MoveOperationImplementation {
    private ShowDataproProject project;
    
   public ShowDataProjectOperations(ShowDataproProject project) {
    ...
    public List getDataFiles() {
        ...
        String prop = evaluator.getProperty(ShowDataproProjectProperties.SOURCE_ROOT);
        if (prop != null) {
    ...
    public List getMetadataFiles() {
        FileObject projectDirectory = project.getProjectDirectory();
        List files = new ArrayList();
        java.util.logging.Logger.getLogger(ShowDataProjectOperations.class.getName()).info("DVProjectOperations********************** "+ projectDirectory.getName());
        addFile(projectDirectory, "nbproject", files); // NOI18N
        addFile(projectDirectory, "private", files); 
        addFile(projectDirectory, "build.xml", files); // NOI18N
        addFile(projectDirectory, "SQLStatements", files); //NOI18N
    ...
    public void notifyDeleting() throws IOException {
       ShowDataproActionProvider ap = (ShowDataproActionProvider) project.getLookup().lookup(ShowDataproActionProvider.class);
        assert ap != null;
