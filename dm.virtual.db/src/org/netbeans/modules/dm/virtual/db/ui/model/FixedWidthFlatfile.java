...
/**
 * Concrete bean wrapper implementation for fixed-width instances of VirtualDBTable,
 * exposing read-only properties for display in a Virtual DB property sheet. 
 * 
 * @author Ahimanikya Satapathy
 */
public class FixedWidthFlatfile extends VirtualTable {

    public FixedWidthFlatfile(VirtualDBTable dbTable) {
        super(dbTable);
    }

    public Integer getHeaderBytesOffset() {
        String valStr = getProperty(PropertyKeys.HEADERBYTESOFFSET);
        return (valStr != null) ? Integer.valueOf(valStr) : VirtualTable.ZERO;
    }
    ...
    public Integer getRecordLength() {
        ...
        return (length != 0) ? new Integer(length) : VirtualTable.ZERO;
    }
}
