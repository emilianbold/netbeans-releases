...
public class DBURL {
    ...
    public DBURL(String url, boolean level) {
        this.url = url;
        ...
        if (colon == -1 || (prefixStripped.length() - 1 == colon)) {
            ...
            if (projectLevel && workDir.startsWith(CommonUtils.PRJ_PATH)) {
                int phome = CommonUtils.PRJ_PATH.length();
                workDir = "${project.home}" + workDir.substring(phome);
            }
        }
    }
