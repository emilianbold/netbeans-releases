        ...
        public ConstructorResultItem(CsmFunction ctr, CsmCompletionExpression substituteExp, int priority, boolean isDeclaration, boolean instantiateTypes) {
            ...
            for (CsmParameter prm : ctr.getParameters()) {
                ...
                if (type == null) {
                    ...
                    params.add(new ParamStr("", "", ((CsmParameter) prm).getName().toString(), prm.getText().toString() + ((prm.getInitialValue() != null) ? " = " + prm.getInitialValue().getText().toString() : ""), true, LFCustoms.shiftColor(KEYWORD_COLOR))); //NOI18N
                    varArgIndex = i;
                } else {
                    String typeName = getTypeName(type, instantiateTypes);
                    params.add(new ParamStr(typeName, typeName, ((CsmParameter) prm).getName().toString(), prm.getText().toString() + ((prm.getInitialValue() != null) ? " = " + prm.getInitialValue().getText().toString() : ""), false, LFCustoms.getTextFgColor() /*getTypeColor(type.getClassifier())*/));
                }
                i++;
            }
            // TODO
