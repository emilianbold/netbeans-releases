...
public abstract class HyperlinkBaseTestCase extends ProjectBasedTestCase {
    ...
    protected void performNullTargetTest(String source, int lineIndex, int colIndex) throws Exception {
        ...
        Token<CppTokenId> jumpToken = getJumpToken(doc, offset);
    ...
    private CsmOffsetable findTargetObject(BaseDocument doc, int offset, Token jumpToken) {
        ...
        if (csmItem == null && declarationsProvider.isValidToken(jumpToken)) {
            csmItem = declarationsProvider.findTargetObject(doc, jumpToken, offset, true);
        }
        return csmItem;
    }
    
    private Token<CppTokenId> getJumpToken(BaseDocument doc, int offset) {
