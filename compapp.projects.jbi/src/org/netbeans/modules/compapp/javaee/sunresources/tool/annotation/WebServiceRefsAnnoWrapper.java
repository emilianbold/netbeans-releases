 ...
 * @author echou
 */
public class WebServiceRefsAnnoWrapper extends AnnotationWrapperBase {
    
    private WebServiceRefAnnoWrapper[] value;
    
    /** Creates a new instance of WebServiceRefsAnnoWrapper */
    public WebServiceRefsAnnoWrapper(Annotation anno) {
        AnnotationComponent ac = anno.getComponent("value"); // NOI18N
        if (ac == null) {
            this.value = new WebServiceRefAnnoWrapper[] {};
        }
        ArrayElementValue aev = (ArrayElementValue) ac.getValue();
        ElementValue[] evs = aev.getValues();
        this.value = new WebServiceRefAnnoWrapper[evs.length];
        for (int i = 0; i < evs.length; i++) {
            NestedElementValue nev = (NestedElementValue) evs[i];
            this.value[i] = new WebServiceRefAnnoWrapper(nev.getNestedValue());
        }
    }
    
    public WebServiceRefAnnoWrapper[] value() {
        return this.value;
    }
}
