 ...
 * @author echou
 */
public class EJBsAnnoWrapper extends AnnotationWrapperBase {
    
    private EJBAnnoWrapper[] value;
    
    /** Creates a new instance of EJBsAnnoWrapper */
    public EJBsAnnoWrapper(Annotation anno) {
        AnnotationComponent ac = anno.getComponent("value"); // NOI18N
        if (ac == null) {
            this.value = new EJBAnnoWrapper[] {};
        }
        ArrayElementValue aev = (ArrayElementValue) ac.getValue();
        ElementValue[] evs = aev.getValues();
        this.value = new EJBAnnoWrapper[evs.length];
        for (int i = 0; i < evs.length; i++) {
            NestedElementValue nev = (NestedElementValue) evs[i];
            this.value[i] = new EJBAnnoWrapper(nev.getNestedValue());
        }
    }
    
    public EJBAnnoWrapper[] value() {
        return this.value;
    }
}
