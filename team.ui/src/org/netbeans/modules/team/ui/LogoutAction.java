...
public final class LogoutAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
        final Collection<TeamServer> servers = TeamServerManager.getDefault().getTeamServers();
        if (servers.size() == 1) {
            RequestProcessor.getDefault().post(new Runnable() {
                @Override
                public void run() {
                    servers.iterator().next().logout();
                }
            });
            return;
        }
        ...
        if (result == LOGOUT) {
            RequestProcessor.getDefault().post(new Runnable() {
                ...
                @Override
                public void run() {
                    for (TeamServer k : panel.getSelectedServers()) {
    ...
    public boolean isEnabled() {
        for (TeamServer ts: TeamServerManager.getDefault().getTeamServers()) {
            if (ts.getStatus()!=TeamServer.Status.OFFLINE) {
                return true;
            }
        }
        return false;
