...
public class EnterpriseModulesNode extends AbstractNode {
    ...
    private static final Logger LOGGER = Logger.getLogger(EnterpriseModulesNode.class.getName());

    private EnterpriseModulesNode(EnterpriseModuleChildFactory factory) {
        super(Children.create(factory, true));
        setDisplayName(NbBundle.getMessage(EnterpriseModulesNode.class, "LBL_EntApps"));
        getCookieSet().add(new RefreshEnterpriseModuleChildren(factory));
    }

    public static EnterpriseModulesNode newInstance(Lookup lookup) {
        EnterpriseModuleChildFactory factory = new EnterpriseModuleChildFactory(lookup);
        return new EnterpriseModulesNode(factory);
    }
    ...
    private static class EnterpriseModuleChildFactory extends ChildFactory<WSDefaultModule> {

        private final Lookup lookup;

        public EnterpriseModuleChildFactory(Lookup lookup) {
            this.lookup = lookup;
        }
        ...
        protected Node createNodeForKey(WSDefaultModule key) {
            return new WSDefaultModuleNode(ModuleType.EAR, key);
        }
        ...
        protected boolean createKeys(List<WSDefaultModule> toPopulate) {
            ...
            try {
                ...
                for (TargetModuleID module : manager.getRunningModules(ModuleType.EAR, new Target[] {target})) {
                ...
                for (TargetModuleID module : manager.getAvailableModules(ModuleType.EAR, new Target[] {target})) {
    ...
    private static class RefreshEnterpriseModuleChildren implements RefreshCookie {
        ...
        private final EnterpriseModuleChildFactory factory;

        RefreshEnterpriseModuleChildren (EnterpriseModuleChildFactory factory){
