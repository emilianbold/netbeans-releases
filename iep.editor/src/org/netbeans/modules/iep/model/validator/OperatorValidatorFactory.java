...
public class OperatorValidatorFactory {
    ...
    public OperatorValidator newOperatorValidator(OperatorComponent operator) {
        ...
        } else if(Constants.TYPE_TUPLE_BASED_AGGREGATOR.equals(type)) {
            validator = new TupleBasedAggregatorValidator(this.mValidator);
        } else if(Constants.TYPE_TIME_BASED_AGGREGATOR.equals(type)){
            validator = new TimeBasedAggregatorValidator(this.mValidator);
        }else if(Constants.TYPE_STREAM_PROJECTION_FILTER.equals(type)){
            validator = new StreamProjectFilterValidator(this.mValidator);
        }else if( Constants.TYPE_TUPLE_SERIAL_CORRELATION.equals(type)
