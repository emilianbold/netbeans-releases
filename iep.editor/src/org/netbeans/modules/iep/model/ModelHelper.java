...
public class ModelHelper {
    ...
    public static SchemaComponent findSchema(String name, IEPModel model) {
        ...
        //This is not used when using this make sure it works from command line
...
//            }
            ...
            public static String getPackageName(DataObject dataObject) {
                ...
                return getPackageName(iepFile);
            }
            
            public static String getPackageName(FileObject iepFile) {
                
                String packageName = "";
                
                //find source folder
                FileObject srcFolder = findParentSourceFolder(iepFile);
                if(srcFolder != null) {
                    packageName = FileUtil.getRelativePath(srcFolder, iepFile.getParent());
                }
//                //This one works only in ide not in command line as 
//                //there are no open project so FileOwnerQuery.getOwner(iepFile)
//                //fails
...
//                    }
            ...
            private static FileObject findParentSourceFolder(FileObject iepFile) {
                FileObject srcFolder = null;
                FileObject parent = iepFile.getParent();
                while(parent != null) {
                    if(parent.getName().equalsIgnoreCase("src")) {
                        srcFolder = parent;
                        break;
                    }
