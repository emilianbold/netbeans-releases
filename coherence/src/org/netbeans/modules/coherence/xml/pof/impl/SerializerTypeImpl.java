 ...
 */
public class SerializerTypeImpl extends PofConfigComponentImpl implements SerializerType {

    public SerializerTypeImpl(PofConfigModelImpl model, Element e) {
        super(model, e);
    }

    public SerializerTypeImpl(PofConfigModelImpl model) {
    ...
    public String getTagName() {
        return SerializerType.XML_TAG_NAME;
    }

    @Override
    public Class<? extends PofConfigComponent> getComponentType() {
        return SerializerType.class;
    }
    ...
    public void setClassName(ClassName element) {
        ...
        if (element != null) {
            ...
            typeList.add(InitParamsImpl.class);
            addBefore(ClassName.XML_TAG_NAME, element, typeList);
        }
    }

    @Override
    public InitParams getInitParams() {
        List<InitParams> elements = getChildren(InitParams.class);
        if (elements != null && elements.size() > 0) {
            return elements.get(0);
        }
        return null;
    ...
    public void setInitParams(InitParams element) {
        InitParams child = getInitParams();
        if (child != null) {
            removeChild(InitParams.XML_TAG_NAME, child);
        }
        if (element != null) {
            Collection typeList = new ArrayList();
            typeList.add(ClassNameImpl.class);
            addAfter(InitParams.XML_TAG_NAME, element, typeList);
        }
    }
}
