 ...
 */
package org.netbeans.modules.collab.core;
...
public class Debug extends Object {
    ...
    static {
        ...
        String filename = System.getProperty("com.sun.collablet.debug.file"); // NOI18N
        ...
        try {
            if ((filename != null) && (filename.trim().length() != 0)) {
                ...
                while (keys.hasMoreElements()) {
                    ...
                    allow(key);
                    System.out.println("Enabling debug messages for \"" + key + "\""); // NOI18N
                }
            } else {
                allow(DISCRIMINATOR_TODO);
            }
        } catch (IOException e) {
            System.out.println("Failed to load debug file \"" + filename + "\""); // NOI18N
    ...
    public static boolean isEnabled() {
        return Boolean.getBoolean("com.sun.collablet.debug");
    }
     ...
     *
     */
    public synchronized static void initializeOut() {
        if (out != null) {
            return;
        }
        ...
        if (isEnabled()) {
            if (Boolean.getBoolean("com.sun.collablet.debug.usesystemout")) {
                out = new PrintWriter(System.out, true);
                out.println("Module debug enabled on System.out");
            } else if (Boolean.getBoolean("com.sun.collablet.debug.usesystemerr")) {
    ...
    public static void debugNotify(Throwable e) {
        if (isEnabled()) {
            errorManager.notify(errorManager.annotate(e, "[DEBUG-only exception]")); // NOI18N
        }
    }
     ...
     *
     */
    public static void dumpMessage(CollabMessage message) {
        try {
            out.println("Message:\t" + message.getID());
            out.println("\tOriginator:\n\t\t" + message.getOriginator());

            //			out.println("\tRecipients:\t");
            //			for (int i=0; i<message.getRecipients().length; i++)
            //				out.println("\t\t"+message.getRecipients()[i]);
            out.println("\tParts:");

            CollabMessagePart[] parts = message.getParts();

            for (int i = 0; i < parts.length; i++)
                dumpMessagePart(parts[i]);
        } catch (Throwable e) {
            out.println("Exception dumping message:");
            e.printStackTrace(out);
        }
    }
     ...
     *
     */
    public static void dumpMessagePart(CollabMessagePart part)
    throws CollabException {
        out.println("\t\tPart");
        out.println("\t\t\tContent name:\t" + part.getContentName());
        out.println("\t\t\tContent type:\t" + part.getContentType());
        out.println("\t\t\tEncoding:\t\t" + part.getContentEncoding());
        out.println("\t\t\tSize:\t\t\t" + part.getSize());
        out.println("\t\t\tContent:\t\t" + part.getContent());
    }
    ...
    private static String getShortPackageName(Class clazz) {
        if (clazz.getPackage().getName().startsWith("com.sun.tools.ide.collab")) // NOI18N
         {
            String packageName = clazz.getPackage().getName();

            if (packageName.length() < 26) {
                return ".";
            } else {
                return packageName.substring(26);
            }
        } else {
     ...
     *
     */
    public static void logPropertyChange(Object receiver, PropertyChangeEvent event) {
     ...
     *
     */
    public static void logPropertyChange(Object discriminator, Object receiver, PropertyChangeEvent event) {
        if (!isAllowed(discriminator)) {
            return;
        }
        ...
        try {
            ...
            if ((event.getOldValue() != null) && event.getOldValue().getClass().isArray()) {
                temp.append("\n\tOld Value: " + delimitedString((Object[]) event.getOldValue(), ","));
            } else {
                temp.append("\n\tOld Value: " + event.getOldValue());
            }

            if ((event.getNewValue() != null) && event.getNewValue().getClass().isArray()) {
                temp.append("\n\tNew Value: " + delimitedString((Object[]) event.getNewValue(), ","));
            } else {
                temp.append("\n\tNew Value: " + event.getNewValue());
            }

            if ((event.getOldValue() != null) && event.getOldValue().getClass().isArray()) {
                temp.append("\n\tValue Type: ARRAY of " + event.getOldValue().getClass().getComponentType());
    ...
    public static void logFileDetails(Object discriminator, FileObject fo) {
        ...
        temp.append("\n\tgetPackageNameExt(/):    " + fo.getPackageNameExt('/', '.'));
     ...
     */
    public static void logDebugException(String note, Throwable e, boolean stackTrace) {
        StringBuffer temp = new StringBuffer();
        temp.append("\nException:");
        temp.append("\n\tNote:    " + note);
        temp.append("\n\tException:    " + e.toString());
    ...
    public static void todo(String msg) {
        log(DISCRIMINATOR_TODO, "TODO: " + msg + " --> " + new Exception().getStackTrace()[1]);
    }
    ...
    public static void notImplemented() {
        ...
        log(DISCRIMINATOR_TODO, "TODO: Not implemented --> " + new Exception().getStackTrace()[1]);
    }
}
