...
public class EdmproProjectGenerator {
    ...
    private static final Logger mLogger = Logger.getLogger(EdmproProjectGenerator.class.getName());
    ...
    public static AntProjectHelper createProject(File dir, String name, String j2eeLevel) throws IOException {
        ...
        assert fo != null : NbBundle.getMessage(EdmproProjectGenerator.class, "MSG_disk_must_be_mounted") + rootF;
        fo.getFileSystem().refresh(false);
        fo = FileUtil.toFileObject(dir);

        // vlv # 113228
        if (fo == null) {
            throw new IOException(NbBundle.getMessage(EdmproProjectGenerator.class, "ERROR_Can't_create") + dir.getName());
        }
        assert fo.isFolder() : NbBundle.getMessage(EdmproProjectGenerator.class, "MSG_Not_a_dir") + dir;
        assert fo.getChildren().length == 0 : NbBundle.getMessage(EdmproProjectGenerator.class, "MSG_Dir_must_have_been_empty") + dir;
