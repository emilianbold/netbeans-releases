...
//        List edmEngineFiles = getFilesRecursively(buildDir, engineExt);
        Map<String, Definition> wsdlMap = new HashMap<String, Definition>();
        try {
            for (int i = 0, I = etlFiles.size(); i < I; i++) {
                File f = (File) etlFiles.get(i);
                
                String edmFileName = f.getName().substring(0, f.getName().indexOf(".edm"));
                String projectName = getBuildDir().getParentFile().getName();
                String engineFileName = projectName + "_" + edmFileName + "_engine";
                System.out.println("enginefilename: "+engineFileName);
