 ...
 */
/*
 *	TestExtension - test the basic features.
 *
 *	The following test assumes that we know the content of the
 *	graph as we get elements, add and change them. Therefore, the TestExtension.xml
...
import complexfunspec.*;


public class TestExtension extends BaseTest {
    public static void main(String[] argv) {
        TestExtension o = new TestExtension();
        if (argv.length > 0)
    ...
    public void run() throws Exception {
        ComplexFunSpec extension;

        this.readDocument();

        out("creating the bean graph");
        extension = ComplexFunSpec.read(doc);
