 ...
 * @author Petr Hejl
 */
public class UnpauseContainerAction extends AbstractContainerAction {

    @NbBundle.Messages("LBL_UnpauseContainerAction=Unpause")
    public UnpauseContainerAction() {
        super(Bundle.LBL_UnpauseContainerAction(), DockerEvent.Status.UNPAUSE);
    }

    @NbBundle.Messages({
        "# {0} - container id",
        "MSG_UnpausingContainer=Unpausing container {0}"
    })
    @Override
    protected String getProgressMessage(DockerContainer container) {
        return Bundle.MSG_UnpausingContainer(DockerUtils.getShortId(container));
    }
