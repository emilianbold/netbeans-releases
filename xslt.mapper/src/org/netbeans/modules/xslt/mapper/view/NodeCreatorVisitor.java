...
public class NodeCreatorVisitor extends AbstractXPathVisitor {
    ...
    private void visitXPathOperatorOrFunction(XPathOperationOrFuntion operator) {
        ...
        if (node != null) {
            IMethoidNode methoid_node = (IMethoidNode) node.getMapperNode();
            Iterator it = methoid_node.getInputFieldNodes().iterator();
            for (Object expr: operator.getChildren()){
                IFieldNode fn = (IFieldNode) it.next();
                
                NodeCreatorVisitor visitor = new NodeCreatorVisitor(mapper);
                ((XPathExpression) expr).accept(visitor);
                
                Node upstream_node = visitor.getResult();
