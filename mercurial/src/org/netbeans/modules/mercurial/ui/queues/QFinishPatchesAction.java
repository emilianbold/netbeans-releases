...
public class QFinishPatchesAction extends ContextAction {
    ...
    protected void performContextAction (Node[] nodes) {
        ...
        if (finishPatch.showDialog()) {
            ...
            if (patchName != null) {
                finishPatch(root, patchName);
            }
        }
    }

    public void finishPatch (final File root, final String patchName) {
        new HgProgressSupport() {
            ...
            protected void perform () {
                ...
                try {
                    ...
                    HgLogMessage[] messages = HgCommand.getLogMessagesNoFileInfo(root, null, toFinish.get(0).getId(), toFinish.get(toFinish.size() - 1).getId(), 
                                true, -1, Collections.<String>emptyList(), logger);
                    ...
                    if (messages.length > 0) {
                        messages = HgCommand.getLogMessagesNoFileInfo(root, null, messages[0].getRevisionNumber(), 
                                messages[messages.length - 1].getRevisionNumber(), true, -1, Collections.<String>emptyList(), logger);
        ...
        }.start(Mercurial.getInstance().getRequestProcessor(root), root, NbBundle.getMessage(QFinishPatchesAction.class, "LBL_QGoToPatchAction.progress")); //NOI18N
    }
