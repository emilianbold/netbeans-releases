...
import org.netbeans.modules.css.prep.CPCssIndexModel;
...
public class CPWhereUsedQueryPlugin implements RefactoringPlugin {
    ...
    private void findVariables(RefactoringElementContext context, RefactoringElementsBag elements) throws IOException, ParseException {
        ...
        Map<FileObject, CPCssIndexModel> indexModels = CPUtils.getIndexModels(context.getFileObject(), useIndexForCurrentFile);
        for (Map.Entry<FileObject, CPCssIndexModel> entry : indexModels.entrySet()) {
            ...
            for (CPElementHandle var : im.getVariables()) {
                if (var.getType() == CPElementType.VARIABLE_GLOBAL_DECLARATION && var.getName().equals(varName)) {
                    CPElement cpElement = var.resolve(CPModel.getModel(file));
                    if (cpElement != null) {
