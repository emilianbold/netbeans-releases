...
import org.netbeans.modules.bpel.design.model.elements.ForEachGateway;
...
public class ForEachPattern extends CompositePattern {
    ...
    private ForEachGateway gateway;
    ...
    public FRectangle layoutPattern(LayoutManager manager) {
        ...
        float gy = height / 2 - gateway.getHeight() / 2;

        gateway.setLocation(gx, gy);
        
        float x0 = gx;
        float y0 = Math.min(gy, 0);
        
        float totalWidth = width - gx;
        float totalHeight = Math.max(height, gateway.getHeight());
