...
import org.netbeans.modules.bpel.design.model.elements.XORGateway;
import org.netbeans.modules.bpel.model.api.BooleanExpr;


public class IfPattern extends CompositePattern {
    
    private XORGateway forkGateway;
    private XORGateway mergeGateway;
    ...
    protected void onAppendPattern(Pattern nestedPattern) {
        if (nestedPattern instanceof ElsePattern) {
            removeElement(elseStub);
        } else if (!(nestedPattern instanceof ElseIfPattern)) {
    ...
    public FRectangle layoutPattern(LayoutManager manager) {
        ...
        float y0 = -forkGateway.getHeight() / 2; // - (elseIfCount > 0) ? 8 : 0; //TODO: Replace with constant
        float y1 = forkGateway.getHeight() / 2 + LayoutManager.VSPACING; 
                //+ ((elseIfCount > 0) ? 8 : 0);
        float x0, x1, x2, y2;
        ...
        if (thenBranch != null) {
            ...
            float width = thenPlaceHolder.getWidth();
            float height = thenPlaceHolder.getHeight();
            x1 = -width / 2;
            x2 = x1 + width;
            y2 = y1 + height;
        ...
        if (elseBranch != null) {
            ...
            float width = elseStub.getWidth();
            float height = elseStub.getHeight();
    ...
    protected void createElementsImpl() {
        ...
        mergeGateway = new XORGateway();
        forkGateway = new XORGateway();
