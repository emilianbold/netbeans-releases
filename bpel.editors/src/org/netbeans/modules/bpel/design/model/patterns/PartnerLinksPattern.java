...
public class PartnerLinksPattern extends CompositePattern {
    ...
    public FRectangle layoutPattern(LayoutManager manager) {
        ...
        for (Pattern p: getNestedPatterns()){
            FRectangle bounds = p.getBounds();
            width = Math.max(width, bounds.width);
            manager.setPatternPosition(p, 0, ypos);
            ypos += bounds.height + LayoutManager.HSPACING;
        }
        
        return new FRectangle(0, 0, width, ypos); //getBorder().getBounds();
    }
    
    public void optimizePositions(LayoutManager manager) {
        FRectangle oldBounds = getBounds();
        float ypos = oldBounds.y;
        float width = 0;
        
        
        
        for (Position pos: new PositionCalculator().getResult()){
            FRectangle bounds = pos.pattern.getBounds();
            float y = Math.max(pos.position - bounds.height / 2, ypos);
        ...
        setBounds(new FRectangle( oldBounds.x, oldBounds.y, oldBounds.width, ypos - oldBounds.y));
    ...
    private class PositionCalculator{
        ...
        private void adjustPosition(Pattern pattern, float new_value){
            Position pos = null;
            for (Position p: optimalPositions){
    ...
    class Position implements Comparable<Position>{
        ...
        public int compareTo(Position p) {
            return Float.compare(this.position, p.position);
        }
        
        
    }
    
    public DiagramExtInfo getDiagramExtInfo() {
