...
import org.netbeans.modules.bpel.design.geom.FRange;
...
public class SequencePattern extends CompositePattern {
    ...
    public FRectangle layoutPattern(LayoutManager manager) {
        ...
        if (patterns.isEmpty()){
            float t = placeHolder.getWidth() / 2;
            ...
            for (BpelEntity a : activities) {
                ...
                assert p != null;
                FDimension clientSize = p.getBounds().getSize();
    ...
    public void createPlaceholders(Pattern draggedPattern,
        ...
        if (activities.length > 0) {
            ...
            if ((inbound == null) && (firstPattern != draggedPattern)) {
                ...
                float cx = border.getCenterX();
                float cy = border.getY() + border.getInsets().top / 2;
            ...
            if ((outbound == null) && (lastPattern != draggedPattern)) {
                BorderElement border = getBorder();
                float cx = border.getCenterX();
                float cy = border.getY() + border.getHeight()
    ...
    class FirstPlaceHolder extends PlaceHolder {
        public FirstPlaceHolder(Pattern draggedPattern, float cx, float cy) {
    ...
    class LastPlaceHolder extends PlaceHolder {
        public LastPlaceHolder(Pattern draggedPattern, float cx, float cy) {
            super(SequencePattern.this, draggedPattern, cx, cy);
        }
