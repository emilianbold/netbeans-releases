...
public class ProcessPattern extends CompositePattern {
    ...
    protected void onAppendPattern(Pattern p) {
        if (p instanceof EventHandlersPattern) {
    ...
    public FRectangle layoutPattern(LayoutManager manager) {
        ...
        if (rootActivityPattern == null) {
            ...
            FDimension contentSize = rootActivityPattern.getBounds().getSize();
            contentWidth = Math.max(contentSize.width, MIN_CONTENT_WIDTH);
            contentHeight = Math.max(contentSize.height, MIN_CONTENT_HEIGHT);
        }
        
        float contentTop = startEvent.getHeight() / 2 + LayoutManager.VSPACING;
        ...
        if (rootActivityPattern == null){
            ...
            FDimension clientSize = rootActivityPattern.getBounds().getSize();
            FPoint origin = manager.getOriginOffset(rootActivityPattern);
            
            float contentPadding = (contentWidth - clientSize.width) / 2;
        ...
        float y0 = -startEvent.getHeight() / 2;
        float y1 = contentBottom + LayoutManager.VSPACING + endEvent.getHeight();
        
        getBorder().setClientRectangle(rangeX.min, y0, rangeX.getSize(), y1 - y0);
        
        FRectangle result = getBorder().getBounds();
        ...
        if (plc != null){
            ...
            if (plc_pattern != null && plc_pattern.isInModel()){
                FDimension client = plc_pattern.getBounds().getSize();
        ...
        float paddingTop = getBorder().getInsets().top;
        
        float badgeX = result.x + result.width;
        float badgeY = startEvent.getY() + eventsBadge.getHeight() / 2;
        
        float lastBadgeBottom = 0;
        
        float handlerX = badgeX + LayoutManager.HSPACING;
        float handlerY = badgeY + LayoutManager.VSPACING;
        ...
        if (eventHandlers != null) {
            ...
            float handlerHeight = p.getBounds().getSize().height
    ...
    public void relayoutPartnerlinks() {
        ...
        for (Pattern pl: plp.getNestedPatterns() ){
            ...
            yPos = yPos / count;
            FRectangle bounds = pl.getBounds();
