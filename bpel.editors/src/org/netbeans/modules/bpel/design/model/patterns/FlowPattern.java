...
import org.netbeans.modules.bpel.design.model.elements.ANDGateway;
...
public class FlowPattern extends CompositePattern {
    ...
    private ANDGateway startGateway;
    private ANDGateway endGateway;
    ...
    public FRectangle layoutPattern(LayoutManager manager) {
        ...
        if (empty) {
           ...
           placeHolder.setLocation(-width / 2, -height / 2);
            ...
            for (ExtendableActivity a : activities) {
                Pattern p = getNestedPattern(a);
                FDimension pSize =  p.getBounds().getSize();
                float y = -pSize.height / 2;
                manager.setPatternPosition(p, x, y);
                x += pSize.width + LayoutManager.HSPACING;
            }
        }

        float y1 = -(height / 2 + startGateway.getHeight() / 2
                + LayoutManager.VSPACING);
        float y2 = height / 2 + endGateway.getHeight() / 2
                + LayoutManager.VSPACING;
        ...
        float x0 = -width  / 2;
        float y0 = -height / 2 - startGateway.getHeight()
        - LayoutManager.VSPACING;
    ...
    public Area createSelection() {
        ...
        a.subtract(GUtils.createArea(startGateway.getShape()));
        a.subtract(GUtils.createArea(endGateway.getShape()));
        return a;
    }
    
    
    private float getNextPlaceHolderX() {
    ...
    class NextPlaceHolder extends PlaceHolder {
        ...
        private FPath createTempPath() {
            float x1 = startGateway.getX() + startGateway.getWidth();
            float y1 = startGateway.getY() + startGateway.getHeight();
            ...
            float x2 = endGateway.getX() + endGateway.getWidth();
            float y2 = endGateway.getY();
            ...
            float xp = getNextPlaceHolderX();
            
            FPath p = new FPath(new FPoint[]{
                new FPoint(x1, y1), 
                new FPoint(xp, y1), 
                new FPoint(xp, y2),
                new FPoint(x2, y2)
