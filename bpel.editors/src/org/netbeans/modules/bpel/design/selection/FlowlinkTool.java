...
public class FlowlinkTool implements DnDTool {
    ...
    private void moveImpl(float x, float y) {
        if (!isActive()) return;
        currentX = x;
        currentY = y;
        
        VisualElement e = getDesignView().findElement(x, y);
    ...
    public void paint(Graphics2D g2) {
        ...
        if (isValidLocation()) {
            ...
            float w1 = startElement.getWidth() / 2;
            float w2 = endElement.getWidth() / 2;
...
//            GUtils.draw(g2, GUtils.convert(path), true);
    ...
    private FPath createPath(float x1, float y1, float x2, float y2, 
            boolean invert) {
