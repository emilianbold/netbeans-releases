...
public class DiagramButton extends JButton implements ZoomableDecorationComponent {
    ...
    public void paintComponent(Graphics g) {
        ...
        Shape border = new RoundRectangle2D.Float(
                strokeWidth / 2, 
                strokeWidth / 2,
                (float) getWidth() - strokeWidth,
                (float) getHeight() - strokeWidth, 6, 6);
        ...
        int inset = 1 + Math.round(zoom * (DEFAULT_INSET - 1));
    ...
    public Dimension getPreferredSize() {
        int w = Math.max(1, Math.round(zoom * DEFAULT_WIDTH));
        int h = Math.max(1, Math.round(zoom * DEFAULT_HEIGHT));
