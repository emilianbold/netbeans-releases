 ...
 * @author Dmitry Lipin
 */
public class AndCondition extends LogicalCondition implements FileCondition {    
   
    public AndCondition(FileCondition ... conds) {
        setConditions(conds);
    }
    public boolean accept(InstallationFileObject fo) {
        for(FileCondition cond:getConditions()) {
            if(!cond.accept(fo)) {
                return false;
            }
        }
