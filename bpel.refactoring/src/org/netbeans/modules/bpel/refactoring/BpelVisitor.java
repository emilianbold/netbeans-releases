...
//out("[PARTNER LINK]: " + Util.getName(partnerLink));
        Util.visit(partnerLink.getPartnerLinkType(), myTarget, partnerLink, myUsage);
        Util.visit(partnerLink.getMyRole(), myTarget, partnerLink, myUsage);
        Util.visit(partnerLink.getPartnerRole(), myTarget, partnerLink, myUsage);
    }

    @Override
    public void visit(Validate validate) {
        List<BpelReference<VariableDeclaration>> variables = validate.getVariables();

        if (variables == null) {
            return;
        }
        for (BpelReference<VariableDeclaration> variable : variables) {
            Util.visit(variable, myTarget, validate, myUsage);
        }
    }
...
//out("[FOR]: " + Util.getName(_for));
    ...
    public void visit(CorrelationSet correlationSet) {
        ...
        for (WSDLReference<CorrelationProperty> reference : references) {
            Util.visit(reference, myTarget, correlationSet, myUsage);
        }
    }

    @Override
    public void visit(Correlation correlation) {
        Util.visit(correlation.getSet(), myTarget, correlation, myUsage);
    }

    @Override
    public void visit(Source source) {
        Util.visit(source.getLink(), myTarget, source, myUsage);
    }

    @Override
    public void visit(Target target) {
        Util.visit(target.getLink(), myTarget, target, myUsage);
    }
...
//out("[FROM]: " + Util.getName(from));
        Util.visit(from.getPart(), myTarget, from, myUsage);
        Util.visit(from.getProperty(), myTarget, from, myUsage);
        Util.visit(from.getVariable(), myTarget, from, myUsage);
        Util.visit(from.getPartnerLink(), myTarget, from, myUsage);
        visitContentElement(from);
    }

    @Override
    public void visit(FromPart fromPart) {
        Util.visit(fromPart.getToVariable(), myTarget, fromPart, myUsage);
    }
...
//out("[TO]: " + Util.getName(to));
        Util.visit(to.getPart(), myTarget, to, myUsage);
        Util.visit(to.getProperty(), myTarget, to, myUsage);
        Util.visit(to.getVariable(), myTarget, to, myUsage);
        Util.visit(to.getPartnerLink(), myTarget, to, myUsage);
        visitContentElement(to);
    }

    @Override
    public void visit(ToPart toPart) {
        Util.visit(toPart.getFromVariable(), myTarget, toPart, myUsage);
    }

    private void visitContentElement(BpelEntity entity) {
//out("[CONTENT]: " + ((ContentElement) entity).getContent());
        if (myXPath != null) {
            myXPath.find(((ContentElement) entity).getContent(), entity);
        }
    }
...
//out("[EVENT]: " + Util.getName(event));
        Util.visit(event.getMessageType(), myTarget, event, myUsage);
        Util.visit(event.getPortType(), myTarget, event, myUsage);
        Util.visit(event.getOperation(), myTarget, event, myUsage);
        Util.visit(event.getMessageExchange(), myTarget, event, myUsage);
        Util.visit(event.getPartnerLink(), myTarget, event, myUsage);
    }
...
//out("[MESSAGE]: " + Util.getName(message));
        Util.visit(message.getPortType(), myTarget, message, myUsage);
        Util.visit(message.getOperation(), myTarget, message, myUsage);
        Util.visit(message.getMessageExchange(), myTarget, message, myUsage);
        Util.visit(message.getVariable(), myTarget, message, myUsage);
        Util.visit(message.getPartnerLink(), myTarget, message, myUsage);
    }
...
//out("[CATCH]: " + Util.getName(_catch));
        Util.visit(_catch.getFaultMessageType(), myTarget, _catch, myUsage);
        Util.visit(_catch.getFaultName(), myTarget, _catch, myUsage);
        Util.visit(_catch.getFaultElement(), myTarget, _catch, myUsage);
    }
...
//out("[REPLY]: " + Util.getName(reply));
        Util.visit(reply.getFaultName(), myTarget, reply, myUsage);
        Util.visit(reply.getPortType(), myTarget, reply, myUsage);
        Util.visit(reply.getOperation(), myTarget, reply, myUsage);
        Util.visit(reply.getMessageExchange(), myTarget, reply, myUsage);
        Util.visit(reply.getVariable(), myTarget, reply, myUsage);
        Util.visit(reply.getPartnerLink(), myTarget, reply, myUsage);
    }
...
//out("[RECEIVE]: " + Util.getName(receive));
        Util.visit(receive.getPortType(), myTarget, receive, myUsage);
        Util.visit(receive.getOperation(), myTarget, receive, myUsage);
        Util.visit(receive.getMessageExchange(), myTarget, receive, myUsage);
        Util.visit(receive.getVariable(), myTarget, receive, myUsage);
        Util.visit(receive.getPartnerLink(), myTarget, receive, myUsage);
    }
...
//out("[INVOKE]: " + Util.getName(invoke));
        Util.visit(invoke.getPortType(), myTarget, invoke, myUsage);
        Util.visit(invoke.getOperation(), myTarget, invoke, myUsage);
        Util.visit(invoke.getInputVariable(), myTarget, invoke, myUsage);
        Util.visit(invoke.getOutputVariable(), myTarget, invoke, myUsage);
        Util.visit(invoke.getPartnerLink(), myTarget, invoke, myUsage);
    ...
    public void visit(CompensateScope scope) {
        Util.visit(scope.getTarget(), myTarget, scope, myUsage);
    }
...
//out("[THROW]: " + Util.getName(_throw));
        Util.visit(_throw.getFaultName(), myTarget, _throw, myUsage);
        Util.visit(_throw.getFaultVariable(), myTarget, _throw, myUsage);
    }
...
//out("[VARIABLE]: " + Util.getName(variable));
        Util.visit(variable.getMessageType(), myTarget, variable, myUsage);
        Util.visit(variable.getType(), variable.getElement(), myTarget, variable, myUsage);
    }
