...
public class TextFieldSessionBeanDataProvider extends JSFComponent {
    public TextFieldSessionBeanDataProvider() {
        this("textField1"); // default Text Field id
    }
    
    public TextFieldSessionBeanDataProvider(String componentID) {
    ...
    private void bindComponentDataProvider() {
        String
            dbURL = TestPropertiesHandler.getDatabaseProperty("DB_URL"),
            rowSetName = TestPropertiesHandler.getTestProperty("ID_RowSet_Binding_DataProvider"),
            dataProviderName = TestPropertiesHandler.getTestProperty("ID_DataProvider_SessionBean");
        
        Utils.createSessionBeanDataProvider(dbURL, dbTableName, rowSetName, 2, 0);
        checkRowSetAppearance(NAVIGATOR_TREE_NODE_SESSION_PREFIX + rowSetName);
        checkDataProviderAppearance(NAVIGATOR_TREE_NODE_SESSION_PREFIX + dataProviderName);
        bindComponentDataProvider(dataProviderName);
    }
}
