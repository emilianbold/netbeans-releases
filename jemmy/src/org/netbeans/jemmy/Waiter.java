 ...
 * ---------------------------------------------------------------------------
 *
 * Contributor(s): Alexandre Iline (alexandre.iline@sun.com),
...
public class Waiter implements Waitable, Timeoutable, Outputable{
    ...
    public Object waitAction(Object waitableObject) throws InterruptedException {
        ...
        if (logger.isLoggable(Level.FINER)) {
            logger.entering(Waiter.class.getName(), "waitAction", waitableObject);
        }
        ...
        while((result = checkActionProduced(waitableObject)) == null) {
            ...
            if(timeoutExpired()) {
                if (logger.isLoggable(Level.FINER)) {
                    logger.throwing(Waiter.class.getName(), "waitAction",
                            new TimeoutExpiredException(getActualDescription()));
                }
        ...
        if (logger.isLoggable(Level.FINER)) {
            logger.exiting(Waiter.class.getName(), "waitAction", result);
        }
