...
static HDDEDATA CALLBACK DdeCallback
        (UINT Type, UINT Fmt, HCONV ConvHan,
	 	 HSZ Str1, HSZ Str2,
	 	 HDDEDATA Data, DWORD Dta1, DWORD Dta2);
...
// 		      sSubKey); fflush (stdout);
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, sSubKey, 0, KEY_READ, &hKey) != ERROR_SUCCESS) {
        jclass excClass = env->FindClass("org/netbeans/modules/extbrowser/NbBrowserException");
		char msg[1024]; 
		strcpy(msg, "RegOpenKeyEx() failed for "); strcat(msg, sSubKey); strcat(msg, ".");
        env->ThrowNew(excClass, msg);
        return NULL;
    }
    ...
    jclass  cls)
{
	...
	sBrowser[0] = '\0';
    // open registry key
    if (RegOpenKeyEx(HKEY_CLASSES_ROOT, ".html", 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
		// query value
		if (RegQueryValueEx(hKey, NULL, NULL, &type, (LPBYTE)sReg, &len) == ERROR_SUCCESS) {
			// process data
			switch (type) {
			case REG_SZ:
				strcpy(sBrowser, sReg);
				break;
			default:
				jclass excClass = env->FindClass("org/netbeans/modules/extbrowser/NbBrowserException");
				env->ThrowNew(excClass, "Unsupported registry type.");
	...
	if (strlen(sBrowser) == 0) {
        jclass excClass = env->FindClass("org/netbeans/modules/extbrowser/NbBrowserException");
        env->ThrowNew(excClass, "Browser not found.");
        return NULL;
	}

    strcat(sBrowser, "\\shell\\open\\command");

	// now query real command
    // open registry key
    if (RegOpenKeyEx(HKEY_CLASSES_ROOT, sBrowser, 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
		// query value
        len = 1024; sReg [0] = '\0';
		if (RegQueryValueEx(hKey, NULL, NULL, &type, (LPBYTE)sReg, &len) == ERROR_SUCCESS) {
			// process data
        ...
        jclass excClass = env->FindClass("org/netbeans/modules/extbrowser/NbBrowserException");
		char msg[1024]; 
		strcpy(msg, "RegOpenKeyEx() failed for "); strcat(msg, sBrowser); strcat(msg, ".");
        env->ThrowNew(excClass, msg);
        return NULL;
    }
