...
public class BrowserRegistry extends JellyTestCase {
	...
	if(!(new JTextFieldOperator(nbo, 1)).getText().equals("{URL}")) {
    ...
    **/
	...
	nbo.cancel();
	if (!pr.getValue().equals(origExec + "  {URL}")) {
	    fail("\"" + pr.getValue() + "\" instead of \"" + origExec + "  {URL}\"");
	}
    }
    ...
    **/
	...
	pr.openEditor();
	NbDialogOperator nbo = new NbDialogOperator(pnameBrowserExecutable);
	new JTextFieldOperator(nbo, 1).setText(newArg);
	nbo.cancel();
