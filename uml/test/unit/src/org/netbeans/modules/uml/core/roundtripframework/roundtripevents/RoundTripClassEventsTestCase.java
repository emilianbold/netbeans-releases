...
public class RoundTripClassEventsTestCase extends AbstractUMLTestCase implements IRoundTripAttributeEventsSink, IRoundTripClassEventsSink, IRoundTripEnumEventsSink, IRoundTripEnumLiteralEventsSink, IRoundTripOperationEventsSink, IRoundTripPackageEventsSink, IRoundTripRelationEventsSink 
{
	private  IClass tstClass;
	private String operName;
	private int operEventState;
	...
	public void testClassCreate()
	{
		...
		readFromFile(propFile,"Class","create");
		...
		assertEquals(operName,tstClass.getName());
		assertEquals(operEventState,eventStateFromInputFile);
		assertEquals(operKind,elemKindFromInputFile);
	...
	public void testClassModify()
	{
		...
		resetVals();
		tstClass.setName("NewTestClass");
		readFromFile(propFile,"Class","modify");
		assertEquals(elemKind,elemKindFromInputFile);
		assertEquals(eventState,eventStateFromInputFile);
	...
	public void testClassDelete()
	{
		...
		tstClass.delete();
		readFromFile(propFile,"Class","delete");
		assertEquals(elemKind,elemKindFromInputFile);
		assertEquals(eventState,eventStateFromInputFile);
		assertEquals(0, count);
	}
	...
	{
		IOperation oper = (IOperation)chngReq.getAfter();
		operName = oper.getName();
		operEventState = chngReq.getState();
