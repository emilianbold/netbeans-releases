/*
 * The contents of this file are subject to the terms of the Common Development
 * and Distribution License (the License). You may not use this file except in
 * compliance with the License.
 *
 * You can obtain a copy of the License at http://www.netbeans.org/cddl.html
 * or http://www.netbeans.org/cddl.txt.

 * When distributing Covered Code, include this CDDL Header Notice in each file
 * and include the License file at http://www.netbeans.org/cddl.txt.
 * If applicable, add the following below the CDDL Header, with the fields
 * enclosed by brackets [] replaced by your own identifying information:
 * "Portions Copyrighted [year] [name of copyright owner]"
 *
 * The Original Software is NetBeans. The Initial Developer of the Original
 * Software is Sun Microsystems, Inc. Portions Copyright 1997-2006 Sun
 * Microsystems, Inc. All Rights Reserved.
 */


package org.netbeans.modules.uml.ui.products.ad.ADDrawEngines;

import org.netbeans.modules.uml.ui.products.ad.drawengines.ETEdgeDrawEngine;
import org.netbeans.modules.uml.ui.support.contextmenusupport.IProductContextMenu;
import org.netbeans.modules.uml.ui.support.contextmenusupport.IProductContextMenuItem;

/**
 * @author KevinM
 *
 */
public class ADEdgeDrawEngine extends ETEdgeDrawEngine implements IADEdgeDrawEngine {

	/// Adds a stereotype and or name label pullright to the context menu
	protected void addStandardLabelsToPullright(int nKind /* StandardLabelKind */, IProductContextMenu pContextMenu)
	{
	
	}

	/// Handles the stereotype and name selections
	protected boolean handleStandardLabelSelection(IProductContextMenu pContextMenu, IProductContextMenuItem pMenuItem)
	{
		return false;
	}

	/// Handles the stereotype and name sensitivity and check
	protected boolean handleStandardLabelSensitivityAndCheck(IProductContextMenu pContextMenu, IProductContextMenuItem pMenuItem, int buttonKind)
	{
		return false;
	}
	
	public IADDrawEngineButtonHandler createButtonHandler()
	{
		return new ADDrawEngineButtonHandler(this);
	}
}
