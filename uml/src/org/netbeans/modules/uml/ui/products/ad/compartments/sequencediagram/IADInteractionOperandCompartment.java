/*
 * The contents of this file are subject to the terms of the Common Development
 * and Distribution License (the License). You may not use this file except in
 * compliance with the License.
 *
 * You can obtain a copy of the License at http://www.netbeans.org/cddl.html
 * or http://www.netbeans.org/cddl.txt.

 * When distributing Covered Code, include this CDDL Header Notice in each file
 * and include the License file at http://www.netbeans.org/cddl.txt.
 * If applicable, add the following below the CDDL Header, with the fields
 * enclosed by brackets [] replaced by your own identifying information:
 * "Portions Copyrighted [year] [name of copyright owner]"
 *
 * The Original Software is NetBeans. The Initial Developer of the Original
 * Software is Sun Microsystems, Inc. Portions Copyright 1997-2006 Sun
 * Microsystems, Inc. All Rights Reserved.
 */



package org.netbeans.modules.uml.ui.products.ad.compartments.sequencediagram;

import org.netbeans.modules.uml.core.metamodel.dynamics.IInteractionOperand;
import org.netbeans.modules.uml.core.support.umlsupport.IETRect;
import org.netbeans.modules.uml.ui.products.ad.compartments.IADCompartment;

/**
 * @author sumitabhk
 *
 */
public interface IADInteractionOperandCompartment extends IADCompartment
{
	/**
	 * Get/Set the IInteractionOperand associated with this compartment
	 */
	public void setInteractionOperand(IInteractionOperand pInteractionOperand );

	/**
	 * Get/Set the IInteractionOperand associated with this compartment
	 */
	public IInteractionOperand getInteractionOperand();

	/**
	 * Get the minimum logical rectangle that this compartment must encompass
	 */
	public IETRect getMinimumEncompassingRect(int lPreviousCompartmentsBottom);

	/**
	 * Expands this compartment, and its parent to include all covered items
	 */
	public void expandToIncludeCoveredItems();

	/**
	 * Graphically selects all meta model covered items for the associated interaction operand
	 */
	public int selectAllCoveredItems();
}


