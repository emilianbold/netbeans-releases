...
final class ContactlessCommand implements Command {
    ...
    public String execute(ShellPanel shellPanel, String[] args) throws ShellException {
        try {
            return APDUSender.getSender(shellPanel).setContactless(true);
        } catch (Exception ioe) {
            Logger.getLogger(ExtendedCommand.class.getPackage().getName()).log(Level.FINER, null, ioe);
            throw new ShellException(ioe.getMessage());
        }
    }

    public String usage() {
        return APDUSender.getString("USAGE_CONTACTLESS"); //NOI18N
    }
}
