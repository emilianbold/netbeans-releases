...
public class ShowAPDUConsoleAction extends Single<Card> {
    ...
    public ShowAPDUConsoleAction() {
        super (Card.class, NbBundle.getMessage(ShowAPDUConsoleAction.class,
                "ACTION_SHOW_CONSOLE"), null); //NOI18N
    }
    ...
    protected void actionPerformed(Card target) {
        TopComponent ours = null;
        for (TopComponent tc : TopComponent.getRegistry().getOpened()) {
            if (tc instanceof ShellTopComponent) {
                Card card = tc.getLookup().lookup(Card.class);
                if (card != null && target.equals(card)) {
                    tc.requestActive();
                    return;
                }
            }
        ...
        if (ours == null) {
            ours = new ShellTopComponent(target);
        }
        ours.open();
        ours.requestActive();
    }
