...
/**
 * Action bringing the comp to the front.
 *
 * @author Peter Zavadsky
 * @author Tor Norbye (old functionality implementation -> performAction impl)
 */
public class BringToFrontAction extends AbstractComponentAction {

    /** Creates a new instance of BringToFrontAction */
    public BringToFrontAction() {
    }

    protected String getDisplayName(Element[] componentRootElements) {
        return NbBundle.getMessage(BringToFrontAction.class, "LBL_BringToFrontAction");
    }
...
//        bringToFront(webform);
        bringToFront(jsfForm);
    }


//    private static void bringToFront(WebForm webform) {
    private static void bringToFront(JsfForm jsfForm) {
...
//            }
//
////            GridHandler.getInstance().front(webform, list);
////            webform.getGridHandler().front(webform, list);
////            webform.getGridHandler().front(webform, list.toArray(new CssBox[list.size()]));
//            webform.getDomDocument().frontComponents(list.toArray(new CssBox[list.size()]));
//        }
        ...
        if (selectedComponents.length > 0) {
            ...
            jsfForm.getDomDocumentImpl().frontComponents(selectedBoxes.toArray(new Box[selectedBoxes.size()]));
        }
    }
