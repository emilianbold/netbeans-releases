 ...
 * @author Sun Microsystems
 */
public class BDIDebugConnector implements IDebugEngineConnector {
    ...
    private AttachingCookie mAttachCookie;
    ...
    public synchronized static BDIDebugConnector getDebugConnector (AttachingCookie attachCookie) {
        return (BDIDebugConnector)allConnectors.get(attachCookie);
    }
    
    public AttachingCookie getAttachingCookie () {
        return mAttachCookie;
    }
    
    public void attach(final AttachingCookie attachCookie) {
        mAttachCookie = attachCookie;
        mBDIDebugger = new BDIDebugger(mDebugger);
        final String host = attachCookie.getHost();
        final int port = attachCookie.getPort();
        ...
        try {
            ...
            synchronized (allConnectors) {
                allConnectors.put(attachCookie, bdiconnector);
            }
    ...
    public void detach() {
        ...
        if (mAttachCookie != null) {
            synchronized (allConnectors) {
                allConnectors.remove(mAttachCookie);
            }
        }
        mServerThread = null;
        mRmiServer = null;
