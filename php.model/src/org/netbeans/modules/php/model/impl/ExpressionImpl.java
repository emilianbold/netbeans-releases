...
abstract class ExpressionImpl extends SourceElementImpl implements Expression {
    ...
    /* (non-Javadoc)
     * @see org.netbeans.modules.php.model.BinaryExpression#getOperator()
     */
    public String getOperator() {
        List<ASTItem> items = getNarrowNode().getChildren();
        List<SourceElementImpl> children = 
            getChildren( SourceElementImpl.class );
    ...
    protected String getText( ASTItem item ) {
        if ( item instanceof ASTToken ) {
            ASTToken token = (ASTToken) item;
            return Utils.skipToken(token) ? null : token.getIdentifier(); 
        }
        else {
            return ((ASTNode)item).getAsText().trim();
        }
    }
}
