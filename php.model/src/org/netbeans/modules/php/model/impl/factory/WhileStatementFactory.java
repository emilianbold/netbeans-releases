 ...
 *
 */
public class WhileStatementFactory {
    
    public static final String ALTERNATIVE   = "WhileStatementColon";        // NOI18N

    private WhileStatementFactory() {
    }
    
    public static WhileStatementFactory getInstance() {
        return INSTANCE;
    }
            ...
            TokenSequence<?> sequence ) 
    {
        if ( Utils.getNarrowType(node).equals( ALTERNATIVE)) {
            ...
            return child == null ? Collections.EMPTY_LIST : 
                BlockFactory.getInstance().build( wile, child , sequence);
        }
        else {
            assert false;
        }
    ...
    private static final WhileStatementFactory INSTANCE 
            = new WhileStatementFactory();
