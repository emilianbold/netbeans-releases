 ...
 *
 */
public class IdentifierBuilder implements SourceElementBuilder {
    ...
    {
        String nt = realNode.getNT();
        if ( nt.equals( CallExpressionFactory.IDENTIFIER )) {
            if ( realNode.getNode( StaticExpressionBuilder.CLASS_STATIC ) != null ) {
                return StaticExpressionBuilder.getInstance().build( parent , 
                        node, realNode.getNode( StaticExpressionBuilder.CLASS_STATIC ), 
                        sequence);
            }
            else {
                ASTNode real = realNode.getNode( StaticExpressionBuilder.VARIABLE );
                if ( real== null ) {
                    real= realNode;
                }
                return new VariableImpl( parent , node , real, sequence );
            }
        }
        assert false : "NT is :" +nt;           // NOI18N
        return null;
    }
    
    public static IdentifierBuilder getInstance() {
        return INSTANCE;
    }
    
    private static final IdentifierBuilder INSTANCE = new IdentifierBuilder();
