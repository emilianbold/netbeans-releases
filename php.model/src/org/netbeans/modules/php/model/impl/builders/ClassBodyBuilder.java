...
import org.netbeans.modules.php.model.impl.ClassBodyImpl;
 ...
 *
 */
public class ClassBodyBuilder implements SourceElementBuilder {
    
    private static final String CLASS_BODY     = "ClassBody";          // NOI18N
    
    private ClassBodyBuilder(){
    }
    
    public boolean isClassBody( ASTNode node ){
        String nt = node.getNT();
        return CLASS_BODY.equals( nt );
    }
    ...
    {
        if ( isClassBody(node) ) { 
            return new ClassBodyImpl( parent , node , realNode,  sequence );
        }
        return null;
    }
    
    public static ClassBodyBuilder getInstance(){
        return INSTANCE;
    }

    private static final ClassBodyBuilder INSTANCE = new ClassBodyBuilder(); 
}
