...
public class ArgumentsFactory {
    ...
    private static final String LIST_ARG        = "ListArgument";       // NOI18N
            ...
            ASTNode node, TokenSequence sequence ) 
    {
        ...
        if ( LIST_ARG.equals( node.getNT()) ){
            realNode = node.getNode( LIST_ARG );
            if ( realNode == null ){
                return null;
            }
        }
        assert ExpressionFactory.getInstance().isExpression( realNode );

        return ExpressionBuilder.getInstance().build( args, node, 
                realNode, sequence);
    }
    
    public static ArgumentsFactory getInstance() {
        return INSTANCE;
    }
    
    private static final ArgumentsFactory INSTANCE = new ArgumentsFactory();
}
