...
/**
 * This is superclass for control statements "break [number]" and "continue [number]".
 * "[number]" is optional argument.
 *   
 * @author ads
 *
 */
abstract class LoopControlStatementImpl extends StatementImpl {
    ...
    public String getSkipLevelsNumber() {
        ASTToken token = getPhpInteger();
        if ( token != null ) {
    ...
    private ASTToken getPhpInteger( ) {
        ...
        for (ASTItem item : list) {
            if ( !( item instanceof ASTToken) ) {
                ...
                if ( token.getType().equals( PHP_INTEGER) ) {
                    return token;
                }
        }*/
        return node.getTokenType( PHP_INTEGER );
    }
}
