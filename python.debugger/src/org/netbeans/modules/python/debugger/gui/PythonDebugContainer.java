 ...
 *
 *
 * "Portions Copyrighted 2008 Jean-Yves Mengant"
 *
...
public class PythonDebugContainer implements PythonContainer
{
  ...
  class _DEBUGEVENT_MANAGER_ implements PythonDebugEventListener
  {
    ...
    private void populateLocalSource(PythonDebugEvent e)
    {
      if (e.get_fName() != null)
      {
        if ((_currentSource == null) || (!_currentSource.equals(e.get_fName())))
        {
    ...
    {
      if ((e.get_fName().endsWith(_STRING_)) || // substituting .equals by
                                                            // endwith for Jython
                            ...
                            throws PythonDebugException
    {
      ...
      _remoteHashSource.put(MiscStatic.nonSensitiveFileName(localfName
                                                                        .getAbsolutePath()
                                                           ),
                            e.get_fName()
                           );
    ...
    public void newDebugEvent(PythonDebugEvent e)
    {
      switch (e.get_type())
      {
        ...
        case PythonDebugEvent.LINE:
          ...
          if ( _threads.isListeningBack()  )
            // refresh threading as well if thread view is active
            _threads.refreshThreads() ;  
          if ( _stackM.isListeningBack()  )
            // refresh stack view
             _stackM.refreshStack() ;  
          if( _variables.isListeningBack() )
