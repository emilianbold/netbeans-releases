 ...
 * @author Maros Sandor
 */
class AddTableModel extends AbstractTableModel {
    ...
    {
        ResourceBundle loc = NbBundle.getBundle(AddTableModel.class);
        columnLabels.put(COLUMN_NAME_NAME, new String [] {
                                          loc.getString("CTL_AddTable_Column_File"), 
                                          loc.getString("CTL_AddTable_Column_File")});
        columnLabels.put(COLUMN_NAME_ACTION, new String [] {
                                          loc.getString("CTL_AddTable_Column_Action"), 
                                          loc.getString("CTL_AddTable_Column_Action")});
        columnLabels.put(COLUMN_NAME_PATH, new String [] {
                                          loc.getString("CTL_AddTable_Column_Folder"), 
                                          loc.getString("CTL_AddTable_Column_Folder")});
    }
    
    private CheckinOptions[] addOptions;
     ...
     */
    public AddTableModel(String[] columns) {
        setColumns(columns);
    ...
    public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
        String col = columns[columnIndex];
        if (col.equals(COLUMN_NAME_ACTION)) {
            addOptions[rowIndex] = (CheckinOptions) aValue;
    ...
    private void computeDefaultAddOptions() {
        addOptions = new CheckinOptions[nodes.length];
        for (int i = 0; i < nodes.length; i++) {
            ClearcaseFileNode node = nodes[i];
            File file = node.getFile();
            if (ClearcaseModuleConfig.isExcludedFromCommit(file.getAbsolutePath())) {
                addOptions[i] = CheckinOptions.EXCLUDE_FROM_ADD;
            } else {
                addOptions[i] = getDefaultAddOptions(node.getFile());
            }
        }
    }
    ...
    private CheckinOptions getDefaultAddOptions(File file) {
        if (file.isFile()) {
