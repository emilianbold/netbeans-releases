...
class MethodLineParser implements AnnotationLineParser {
    ...
    public AnnotationParsedLine parse(final String line) {
        ...
        if (tokens.length > 0 && isMethodAnnotation(tokens[0])) {
    ...
    private static boolean isMethodAnnotation(final String linePart) {
        return linePart.matches("\\\\?(\\w+\\\\)*" + ANNOTATION_NAME + "\\s*"); //NOI18N
    }
