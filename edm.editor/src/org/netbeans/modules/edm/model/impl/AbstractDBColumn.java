...
public abstract class AbstractDBColumn extends AbstractSQLObject implements SQLDBColumn, Cloneable, Comparable {
     ...
     * @exception EDMException thrown while parsing XML, or if columnElement is null
     */
    public void parseXML(Element columnElement) throws EDMException {
        ...
        if (getId() == null || getId().trim().length() == 0) {
            throw new EDMException(NbBundle.getMessage(AbstractDBColumn.class, "ERROR_Invalid_ID_attribute."));
        }

        name = columnElement.getAttribute(COLUMN_NAME_ATTR);
        if (name == null || name.trim().length() == 0) {
            throw new EDMException(NbBundle.getMessage(AbstractDBColumn.class, "ERROR_Invalid_name_attribute."));
        }
        ...
        try {
            ...
            mLogger.log(Level.INFO,NbBundle.getMessage(AbstractDBColumn.class, "Cannot_determine_JDBC_int_type_for_column{0}({1});_will_try_parsing_as_string.",new Object[] {name, jdbcTypeStr}));
            try {
                this.jdbcType = SQLUtils.getStdJdbcType(jdbcTypeStr);
            } catch (IllegalArgumentException iae) {
                mLogger.log(Level.INFO,NbBundle.getMessage(AbstractDBColumn.class, "ERROR_Cannot_determine_JDBC_int_type",new Object[] {name}));
    ...
    public void setParent(DBTable newParent) {
        ...
        if (newParent instanceof SQLObject) {
            try {
                ...
                mLogger.log(Level.INFO,NbBundle.getMessage(AbstractDBColumn.class, "ERROR_could_not_set_parent_object",new Object[] {this.getName()}),ex);
            }
        }
    }
     ...
     */
    public abstract String toXMLString(String prefix) throws EDMException;
    ...
    protected void appendXMLAttributes(StringBuilder xml) {
        if (xml == null) {
            throw new IllegalArgumentException(NbBundle.getMessage(AbstractDBColumn.class, "ERROR_null_StringBuilder_ref"));
        }
