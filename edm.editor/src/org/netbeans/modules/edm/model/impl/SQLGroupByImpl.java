...
public class SQLGroupByImpl extends AbstractSQLObject implements SQLGroupBy, Cloneable {
     ...
     * @exception EDMException thrown while parsing XML, or if orderByElement is null
     */
    public void parseXML(Element groupByElement) throws EDMException {
        String seqList = groupByElement.getAttribute(ATTR_SEQUENCE);
        if (seqList == null || seqList.trim().length() == 0) {
            throw new EDMException(NbBundle.getMessage(SQLGroupByImpl.class, "ERROR_Invalid_sequence_attribute"));
        }
