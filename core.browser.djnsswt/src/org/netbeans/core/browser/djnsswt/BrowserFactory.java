...
public abstract class BrowserFactory implements HtmlBrowser.Factory {
    ...
    protected abstract Impl createImpl();

    public static Boolean isExplorerHidden () {
...
//        return !Platform.platform.is32Bit() || 
    ...
    public static HtmlBrowser.Factory createExplorerInstance() {
        return new BrowserFactory() {
            @Override
            protected Impl createImpl() {
                return new HtmlBrowserImpl(BrowserType.Explorer);
            }
        };
    }
    
    public static HtmlBrowser.Factory createMozillaInstance() {
        return new BrowserFactory() {
            @Override
            protected Impl createImpl() {
                return new HtmlBrowserImpl(BrowserType.Mozilla);
            }
        };
    }
    
    public static HtmlBrowser.Factory createWebkitInstance() {
        return new BrowserFactory() {
            @Override
            protected Impl createImpl() {
                return new HtmlBrowserImpl(BrowserType.Webkit);
            }
        };
    }
}
