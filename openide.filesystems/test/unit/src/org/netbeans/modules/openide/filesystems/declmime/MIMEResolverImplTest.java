...
public class MIMEResolverImplTest extends NbTestCase {
    ...
    public void testPatternElementValidity() throws Exception {
        MIMEResolver declarativeResolver = createResolver(resolversRoot.getFileObject("pattern-resolver-valid.xml"));
        ...
        declarativeResolver = createResolver(resolversRoot.getFileObject("pattern-resolver-invalid1.xml"));
        declarativeResolver.findMIMEType(root.getFileObject("empty.dtd"));
        LOG.info("Verify");
        assertTrue("Pattern elements in patternInvalid1.xml should not be parsed.", failed.get());

        failed.set(false);
        declarativeResolver = createResolver(resolversRoot.getFileObject("pattern-resolver-invalid2.xml"));
        declarativeResolver.findMIMEType(root.getFileObject("empty.dtd"));
