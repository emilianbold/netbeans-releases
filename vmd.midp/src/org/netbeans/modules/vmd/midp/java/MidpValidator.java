 ...
 */
final class MidpValidator implements ActiveDocumentSupport.Listener {

    private static MidpValidator instance;
    
    private String activeProjectID;

    private MidpValidator() {
        ActiveDocumentSupport.getDefault().addActiveDocumentListener(this);
        DesignDocument doc = ActiveDocumentSupport.getDefault().getActiveDocument();
        if (doc != null) {
            MidpJavaSupport.getCache(doc).updateCacheInternally();
        }
    }
    
    synchronized static MidpValidator getDefault() {
        if (instance == null) {
            instance = new MidpValidator();
        }
        return instance;
    }
    ...
    public void activeDocumentChanged(DesignDocument deactivatedDocument, DesignDocument activatedDocument) {
        ...
        String oldProjectID = activeProjectID;
        ...
        if (isProjectIDChanged) {
            MidpJavaSupport.getCache(activatedDocument).updateCacheInternally();
        }
