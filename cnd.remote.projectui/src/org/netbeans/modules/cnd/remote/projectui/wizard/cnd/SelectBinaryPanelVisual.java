...
public class SelectBinaryPanelVisual extends javax.swing.JPanel {
    ...
    private void updateRoot(){
        ...
        if (validBinary()) {
            ...
            RP.post(new Runnable() {
                ...
                @Override
                public void run() {
                    RemoteJavaExecution processor = new RemoteJavaExecution(env, fileSystem);
                    List<SourceFile> compileLines = processor.getCompileLines(((EditableComboBox)binaryField).getText().trim());
                    SharedLibraries libs = processor.getDlls(((EditableComboBox)binaryField).getText().trim());
                    if (libs == null || compileLines == null) {
                        return;
                    }
                    List<String> dlls = libs.getDlls();
                    String root = processor.getSourceRoot(compileLines);
                    if (root == null) {
                        return;
                    }
                    List<String> searchPaths = libs.getPaths();
                    final Map<String, String> resolvedDlls = searchingTable(dlls);
                    updateArtifacts(root, resolvedDlls);
    ...
    private void updateArtifacts(final String root, final Map<String, String> dlls){
    ...
    void read(WizardDescriptor wizardDescriptor) {
        env = (ExecutionEnvironment) wizardDescriptor.getProperty(ProjectTemplatePanel.REMOTE_FILE_ENV);
