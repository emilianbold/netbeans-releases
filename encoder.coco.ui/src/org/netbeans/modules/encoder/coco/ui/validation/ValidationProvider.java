...
/**
 * A validator for validating COBOL Copybook encoder metadata.
 *
 * @author Jun Xu
 */
public class ValidationProvider extends XsdBasedValidator {
    ...
    public String getName() {
        return _bundle.getString("coco_validator.lbl.provider_name");
    }

    protected void validate(Model model, Schema schema, XsdBasedValidator.Handler handler) {
        try {
            new CocoEncoderValidator((SchemaModel) model, handler).validate();
        } catch(Exception ex) {
