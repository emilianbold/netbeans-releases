...
public class MTestCompiler extends Task {
    ...
    public void execute() throws BuildException {
        ...
        for (int i=0; i<testbags.length; i++) {
                ...
                MTestConfig.AntExecType comp = testbag.getCompiler();
                if (comp == null) throw new BuildException("Testbag "+testbag.getName()+" has not a compiler.");

                Ant ant_new = (Ant) getProject().createTask( "ant" );
                ant_new.setOwningTarget( target ); 
                ant_new.setAntfile( comp.getAntFile() );
                ant_new.setTarget( comp.getTarget() );
                if (comp.getDir() != null) ant_new.setDir( project.resolveFile ( comp.getDir()));
                ant_new.init();
