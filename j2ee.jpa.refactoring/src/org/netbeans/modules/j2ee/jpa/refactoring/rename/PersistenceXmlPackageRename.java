...
public class PersistenceXmlPackageRename extends PersistenceXmlRefactoring{
    ...
    public PersistenceXmlPackageRename(RenameRefactoring rename) {
    ...
    public Problem prepare(RefactoringElementsBag refactoringElementsBag) {
        FileObject folder = renameRefactoring.getRefactoringSource().lookup(NonRecursiveFolder.class).getFolder();
        String oldPackageName = RefactoringUtil.getQualifiedName(folder);
        ...
        for (FileObject each : getPersistenceXmls()){
            try {
                ...
                for (String clazz : getClasses(folder, new ArrayList<String>())){
                    ...
                    String newName = clazz.replace(oldPackageName, renameRefactoring.getNewName());
                    for (PersistenceUnit persistenceUnit : punits) {
                        refactoringElementsBag.add(getRefactoring(),
                                new PersistenceXmlPackageRenameRefactoringElement(persistenceUnit, clazz, newName, pUDataObject, each));
                    }
                }
