...
public class QueryBuilderGraphFrame extends JPanel
        ...
        KeyListener, DropTargetListener {
	...
	Log.getLogger().entering("QueryBuilderGraphFrame", "constructor"); // NOI18N
        ...
        _desktopScrollPane.setPreferredSize(new Dimension(100,100)); //very important
        ...
        _dropTarget = new DropTarget(_desktopScrollPane,
    ...
    void resizeDesktop() {
        if (DEBUG) {
            System.out.println(" resizeDesktop() called " + "\n" ); // NOI18N
            System.out.println(" width = " + _graph.getSize().getWidth() + " Height = " + _graph.getSize().getHeight() + "\n" ); // NOI18N
        }
        int graphWidth = (int) _graph.getSize().getWidth();
        int graphHeight = (int) _graph.getSize().getHeight();
        
        int frameWidth = (int) this.getSize().getWidth();
        int frameHeight = (int) this.getSize().getHeight();
        
        // when graph width height is zero set canvas to the frame size.
        if ( graphWidth == 0 )
            graphWidth = frameWidth;
        if ( graphHeight == 0 )
            graphHeight = frameHeight;
        
        if (DEBUG) {
            System.out.println(" width = " + graphWidth + " Height = " + graphHeight + "\n" ); // NOI18N
        }
        _canvas.setSize(new Dimension( graphWidth, graphHeight ) );
        _canvas.updateUI();
        _desktopPane.setPreferredSize(new Dimension(graphWidth, graphHeight));
        _desktopPane.revalidate();
    }
    ...
    JPopupMenu createBackgroundPopup() {
        ...
        /*
        menuItem = new JMenuItem(NbBundle.getMessage(QueryBuilderGraphFrame.class, "SELECT_ALL"));      // NOI18N
        menuItem.addActionListener(this);
        backgroundPopup.add(menuItem);
         */
        ...
        subMenu = new JMenu(NbBundle.getMessage(QueryBuilderGraphFrame.class, "INPUT_TABLE_CHANGE_TYPE"));      // NOI18N
         
        subMenuItem = new JMenuItem(NbBundle.getMessage(QueryBuilderGraphFrame.class, "SELECT"));       // NOI18N
        subMenuItem.addActionListener(this);
        subMenu.add(subMenuItem);
         
        subMenuItem = new JMenuItem(NbBundle.getMessage(QueryBuilderGraphFrame.class, "UPDATE"));       // NOI18N
        subMenuItem.addActionListener(this);
        subMenu.add(subMenuItem);
         
        subMenuItem = new JMenuItem(NbBundle.getMessage(QueryBuilderGraphFrame.class, "INSERT"));       // NOI18N
        subMenuItem.addActionListener(this);
        subMenu.add(subMenuItem);
         
        subMenuItem = new JMenuItem(NbBundle.getMessage(QueryBuilderGraphFrame.class, "DELETE"));       // NOI18N
        subMenuItem.addActionListener(this);
        subMenu.add(subMenuItem);
         
        backgroundPopup.add(subMenu);
         
        menuItem = new JMenuItem(NbBundle.getMessage(QueryBuilderGraphFrame.class, "INPUT_TABLE_COLLAPSE_PANE"));       // NOI18N
        menuItem.addActionListener(this);
        backgroundPopup.add(menuItem);
         
        menuItem = new JMenuItem(NbBundle.getMessage(QueryBuilderGraphFrame.class, "PROPERTIES"));      // NOI18N
        menuItem.addActionListener(this);
        backgroundPopup.add(menuItem);
         */
    ...
    JPopupMenu createTableTitlePopup() {
        ...
        /*
        menuItem = new JMenuItem(NbBundle.getMessage(QueryBuilderGraphFrame.class, "COLUMN_NAMES")); // NOI18N
        menuItem.addActionListener(this);
        tableTitlePopup.add(menuItem);
         
        menuItem = new JMenuItem(NbBundle.getMessage(QueryBuilderGraphFrame.class, "TABLE_NAME_ONLY")); // NOI18N
        menuItem.addActionListener(this);
        tableTitlePopup.add(menuItem);
         
        menuItem = new JMenuItem(NbBundle.getMessage(QueryBuilderGraphFrame.class, "SELECT_ALL_COLUMNS")); // NOI18N
        menuItem.addActionListener(this);
        tableTitlePopup.add(menuItem);
         */
    ...
    private void tableModelChanged(TableModelEvent e) {
        ...
        // ask Val if this should be THE behavior
...
//             }
        ...
        if (value==Boolean.TRUE) {      // A column has been selected
            ...
            // handle this in createNode.
            
            // Add a row to the InputTableModel
...
//            _queryBuilderInputTable.selectColumn(tableSpec, columnName, Boolean.TRUE);
            ...
            // Remove a row from the InputTableModel
//            _queryBuilderInputTable.removeRow(tableSpec, columnName);
            
            // If we've removed the last selected column from a table, drop it.
	...
	Log.getLogger().entering("QueryBuilderGraphFrame", "insertTableInteractively", fullTableName); // NOI18N
        ...
        try {
            ...
            List edges = insertFKEdges(internalFrame.getGraphCell(), fullTableName);
...
//      }
    ...
    QueryBuilderInternalFrame createNode(JoinTable joinTable, List selectColumnNames) {
        ...
        // Fencepost case for the first row
        // Remove for the time being
        // Restore later if we have decide to handle '* { All Columns }'
        /*
          dbData[0][0] = Boolean.FALSE;
          dbData[0][1] = null;
          dbData[0][2] = new String("* { All Columns }"); // NOI18N
         */
    ...
    private List insertFKEdges(DefaultGraphCell newCell, String newFullTableName) {
        ...
        // Iterate through the current set of roots, drawing arcs to the new node as needed
        for (int i=0; i<_graphModel.getRootCount(); i++) {
            
            Object root = _graphModel.getRootAt(i);
            
            // Check whether this root is related to the new one
            String[] fk;
		...
		&& (root != newCell))                   // ... and is not the cell we just created?
            {
                ...
                {
                    String oldTableSpec=((QueryBuilderInternalFrame)oldCell.getUserObject()).getTableSpec();
                    String newTableSpec=((QueryBuilderInternalFrame)newCell.getUserObject()).getTableSpec();
...
//          }
    ...
    void insertJoinEdge(Predicate pred, String joinType) {
        ...
        if ((val1 instanceof Column) && (val2 instanceof Column)) {
            ...
            DefaultGraphCell cell1 = findNode(tableSpec1);
            ...
            DefaultGraphCell cell2 = findNode(tableSpec2);
...
//      // Test whether this And is suitable
//      if ()
//      {
...
//     }
    ...
    // Insert an edge from one cell to another
	...
	Log.getLogger().entering("QueryBuilderGraphFrame", "insertEdge", new Object[] { fullTableName1, fullTableName2 }); // NOI18N
        ...
        cell1.add(cell1Port);
        DefaultPort cell2Port = new DefaultPort();
        cell2.add(cell2Port);
        
        // Initialize the set of attributes for the new edge, and start setting properties
        Map atts = GraphConstants.createMap();
        
        // Create the node object for the edge (for the Property Sheet), and also
        ...
        if (fk!=null) {
            if (fk[0].equalsIgnoreCase(fullTableName1)) {
                GraphConstants.setLineEnd(atts,GraphConstants.ARROW_TECHNICAL);
                GraphConstants.setEndFill(atts,true);
            } else {
                GraphConstants.setLineBegin(atts,GraphConstants.ARROW_TECHNICAL);
                GraphConstants.setBeginFill(atts,true);
            }
        }
        
        GraphConstants.setLineWidth(atts,2);
//            GraphConstants.setBeginSize(atts,20);
        
        // Create a JGraph edge with the property as its userObject
        DefaultEdge edge = new DefaultEdge(an);
        
        // Wrap the edge and its attributes in a HashMap
        HashMap map = new HashMap();
        map.put(edge, atts);
        
        // Create a ConnectionSet with the edge connecting the two cells
        ConnectionSet cs = new ConnectionSet(edge, cell1Port, cell2Port);
        
        // Create a singleton root set containing the edge
        Object insertEdges[] = new Object[] {edge};
        
        // Finally insert the edge, with the associated attributes and connections
...
//      // Temporary hack -- redraw frame, because of arrow location glitch
...
//      redrawFrame((QueryBuilderInternalFrame)cell2.getUserObject());
    ...
    void removeTable(QueryBuilderInternalFrame currentSelectedFrame) {
        ...
        DefaultGraphCell graphCell = currentSelectedFrame.getGraphCell();
        
        // Before removing the node, remove all the edges that were connected to it
        
        // Get all ports on the node
        List ports = graphCell.getChildren();
        
        // Traverse from back to front, so removals don't cause instability
        for (int i=ports.size()-1; i>=0; i--) {
            Port p = (Port)ports.get(i);
            
            // Get all edges for the port
            Iterator edges = p.edges();
            while (edges.hasNext()) {
                
                // Remove this edge
                Edge e = (Edge)edges.next();
                p.removeEdge(e);
                _graphModel.remove(new Object[] {e});
            }
            
            // Remove the port itself from its parent
            graphCell.remove(i);
        }
        
        // Remove the node itself from the graph
        _graphModel.remove(new Object[] {graphCell});
        
        // Remove the InternalFrame (graphical node) from the desktop
        // Required because we added it earlier
        _desktopPane.remove(currentSelectedFrame);
        
        _desktopPane.setSelectedFrame(null);
        ...
        int tableCount = 0;
        Component[] comps = _desktopPane.getComponents();
        for(int i=0; i<comps.length; i++) {
            Component comp = comps[i];
            if (comp instanceof QueryBuilderInternalFrame)  {
    ...
    void clearGraph() {
        ...
        // Start with a fresh graph model
...
//         _graph.setModel(_graphModel);
        ...
        _graphModel.remove((_graphModel.getDescendants(_graphModel,
                _graphModel.getRoots(_graphModel))).toArray());
        ...
        // Remove any nodes (internal frames) that have been added to the desktop pane
        Component[] comps = _desktopPane.getComponents();
        for(int i=0; i<comps.length; i++) {
            Component comp = comps[i];
            if (comp instanceof QueryBuilderInternalFrame)
                _desktopPane.remove(comp);
        }
        
        // Reset the location for placing nodes
    ...
    DefaultGraphCell findNode(String tableSpec) {
        ...
        for (int i=0; i<_graphModel.getRootCount(); i++) {
            ...
            {
                QueryBuilderInternalFrame internalFrame =
                        (QueryBuilderInternalFrame) (((DefaultGraphCell) root).getUserObject());
                if (internalFrame.getTableSpec().equals(tableSpec))
                    return (DefaultGraphCell) root;
            }
        }
    ...
    public void  removeNode(TableNode node) {
        ...
        for(int i=0; i<comps.length; i++) {
            ...
            if (comp instanceof QueryBuilderInternalFrame)  {
                QueryBuilderInternalFrame qbif =
                                         (QueryBuilderInternalFrame) comp;
                TableNode tNode = qbif.getNode();
                System.out.println(" tNode.getTableName() = " + tNode.getTableName() + " node.getTableName() = " + node.getTableName() + "\n" ); // NOI18N
                System.out.println(" tNode.getCorrName() = " + tNode.getCorrName() + " node.getCorrName() = " + node.getCorrName() + "\n" ); // NOI18N
                if ( (tNode.getTableName().equals(node.getTableName()) )  &&
                     (tNode.getCorrName().equals(node.getCorrName()) ) ) {
                    removeTable ( qbif );
                }
            }
        }
     ...
     */
    Point getNextLocation(QueryBuilderInternalFrame internalFrame) {
        
        if (!_firstTableInserted ) {
            _location = new Point(initX, initY);
        } else {
            Dimension size = internalFrame.getSize();
            _location = new Point((int)(
                    _location.getX() +
                    size.getWidth() +
                    offsetX +
                    randomVal.nextInt((int)size.getWidth()/2)),
                    (int)(_location.getY() +
                    offsetY +
                    randomVal.nextInt((int)size.getHeight()/2)));
            offsetY *= -1;
            if ( _location.getX() >
                    (int) size.getWidth() * MAX_TABLES_IN_A_ROW ) {
                _location = new Point(initX, (int)_location.getY()+(int)size.getHeight()+2*offsetY);
            }
        }
        return _location;
    }
...
//          _acifa.setEnabled(false);
    ...
    // Utility method for printing graph information
    
    private void printRoots() {
        if (DEBUG) {
            System.out.println("Root Count" + _graphModel.getRootCount()); // NOI18N
            for (int i=0; i<_graphModel.getRootCount(); i++)
                System.out.println("Root["+i+"] class: " + _graphModel.getRootAt(i).getClass()); // NOI18N
        }
    }
