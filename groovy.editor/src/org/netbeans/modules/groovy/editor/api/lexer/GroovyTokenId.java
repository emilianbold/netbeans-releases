...
//    CHAR_LITERAL(null, "character"),
    ...
    private static final Language<GroovyTokenId> language = new GroovyLanguageHierachy().language();
    ...
    private static class GroovyLanguageHierachy extends LanguageHierarchy<GroovyTokenId> {
        ...
        protected Map<String, Collection<GroovyTokenId>> createTokenCategories() {
            ...
            for (GroovyTokenId tokenId : GroovyTokenId.values()) {
                String category = tokenId.primaryCategory();
                Collection<GroovyTokenId> categoryItems = categories.get(category);

                // Collection for this category has not been created yet
                if (categoryItems == null) {
                    categoryItems = new HashSet<GroovyTokenId>();
                    categories.put(category, categoryItems);
                }

                // Put token into right category
                categoryItems.add(tokenId);
            }
            return categories;
        }
