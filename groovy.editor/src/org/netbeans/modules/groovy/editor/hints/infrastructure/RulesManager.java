...
public class RulesManager {
    ...
    private static Map<GroovyCompilerErrorID,List<ErrorRule>> errors = new HashMap<GroovyCompilerErrorID, List<ErrorRule>>();
    ...
    public HintSeverity getSeverity(UserConfigurableRule rule) {
        return HintSeverity.ERROR; // FIXME: return fixed value...
    ...
    public Preferences getPreferences(UserConfigurableRule rule, String profile) { 
        // FIXME: profile = profile == null ? HintsSettings.getCurrentProfileId() : profile;
        return NbPreferences.forModule(this.getClass()).node(profile).node(rule.getId());
    }
    
    public Map<GroovyCompilerErrorID,List<ErrorRule>> getErrors() {
        return errors;
    }
    ...
    private static void categorizeErrorRules( List<Pair<Rule,FileObject>> rules,
                                             Map<GroovyCompilerErrorID,List<ErrorRule>> dest,
    ...
    private static void addRule( ErrorRule rule, Map<GroovyCompilerErrorID,List<ErrorRule>> dest ) {

        for(GroovyCompilerErrorID code : (Set<GroovyCompilerErrorID>) rule.getCodes()) {
