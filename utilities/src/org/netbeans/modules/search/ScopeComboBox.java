...
public class ScopeComboBox extends org.netbeans.api.search.ui.ScopeComboBox {
    ...
    public ScopeComboBox(String prefferedId) {
        ...
        SearchScopeRegistry.getDefault().addChangeListener(
                WeakListeners.change(searchScopeChangeListener,
                SearchScopeRegistry.getDefault()));
    ...
    private void initGlobalSelectionListener() {
        lookupResult = Utilities.actionsGlobalContext().lookupResult(
                Node.class);
        lookupResult.addLookupListener(new LookupListener() {

            @Override
            public void resultChanged(LookupEvent ev) {
                searchScopeChangeListener.stateChanged(new ChangeEvent(this));
            }
        });
    }
    ...
    /**
     * Moves the node selection search scope to the last position. The
     * implementation assumes that the node selection search scope is the first
     * search scope among all registered search scopes.
     */
    private Collection<Map.Entry<SearchScope, Boolean>> orderSearchScopes() {
        Collection<Map.Entry<SearchScope, Boolean>> currentCollection =
                SearchScopeRegistry.getDefault().getSearchScopes().entrySet();
        ...
        Collection<Map.Entry<SearchScope, Boolean>> newCollection =
                new ArrayList<Map.Entry<SearchScope, Boolean>>(
    ...
    public SearchInfo getSearchScopeInfo() {
        SearchScope ss = getSelectedSearchScope();
        if (ss == null) {
            return null;
        } else {
