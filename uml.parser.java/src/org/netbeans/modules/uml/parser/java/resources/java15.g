...
tokens {
	...
	TYPE_UPPER_BOUNDS; TYPE_LOWER_BOUNDS; GENERIC_TYPE; START_CLASS_BODY;
        END_CLASS_BODY;
} 
...
classOrInterfaceType[boolean addImagNode]
	:	//IDENT (t:typeArguments)?
		//(options{greedy=true;}: // match as many as possible
		//	DOT^
		//	IDENT (t1:typeArguments)?
		//)*
                identifier (t:typeArguments)?
		{
                        //boolean generic = ((#t == null) && (#t1 == null) ? false : true);
                        boolean generic = ((#t == null) ? false : true);
			if ( addImagNode ) {
                            if(generic == false) {
				#classOrInterfaceType = #(#[TYPE,"TYPE"], #classOrInterfaceType);
                            }
                            else {
                               #classOrInterfaceType = #(#[GENERIC_TYPE,"GENERIC_TYPETYPE"], #classOrInterfaceType);
                            }
			}
...
// set of characters allowed in an identifier
protected
IDENT_LETTER
    :  '\u0041'..'\u005a' |
       '\u0061'..'\u007a' |
...
IDENT
	...
	:	(IDENT_LETTER|'_'|'$') (IDENT_LETTER|'_'|'0'..'9'|'$')*
		{
