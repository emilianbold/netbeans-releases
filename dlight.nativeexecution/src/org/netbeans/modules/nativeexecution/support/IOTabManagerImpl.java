...
final class IOTabManagerImpl implements IOTabManager {
    ...
    private static final boolean wrapIO;


    static {
        String value = System.getProperty("dlight.io.wrapper"); // NOI18N
        wrapIO = value == null ? false : Boolean.parseBoolean(value);
    }
    ...
    public InputOutput getIO(NativeTask task, boolean reuse) {
        NativeTaskAccessor taskAccess = NativeTaskAccessor.getDefault();
        InputOutput delegate = provider.getIO(task.toString(),
                taskAccess.getTaskControlActions(task));

        return wrapIO ? new IOWrapper(delegate) : delegate;
    }
