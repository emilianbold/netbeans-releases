...
public final class CreateTestsAction extends NodeAction {
    ...
    private static final String NAMESPACE_TPL_BROKEN = "namespace \\"; // NOI18N
    private static final String NAMESPACE_TPL_FIXED = "namespace "; // NOI18N
    ...
    private File adjustFileContent(File generatedFile, File testFile, File sourceFile, String requireOnce) {
        try {
            ...
            try {
                ...
                try {
                    String line;
                    boolean namespaceWritten = false;
                    boolean requireWritten = false;
                    String filename = sourceFile.getName();
                    while ((line = in.readLine()) != null) {
                        if (!namespaceWritten && line.startsWith(NAMESPACE_TPL_BROKEN)) {
                            // http://www.phpunit.de/ticket/904
                            out.write(line.replace(NAMESPACE_TPL_BROKEN, NAMESPACE_TPL_FIXED));
                            namespaceWritten = true;
                        } else if (!requireWritten && PhpUnit.isRequireOnceSourceFile(line.trim(), filename)) {
