        ...
        for (String line : listMerge) {
            ...
            if (HgCommand.isMergeFailedMsg(line)) {
                ...
                if (line.contains(HgCommand.HG_MERGE_FAILED2_ERR)) {
                    filepath = line.substring(HgCommand.HG_MERGE_FAILED1_ERR.length(), line.length() - HgCommand.HG_MERGE_FAILED2_ERR.length()).trim();
                } else if (line.contains(HgCommand.HG_MERGE_FAILED3_ERR)) {
                    filepath = line.substring(HgCommand.HG_MERGE_FAILED1_ERR.length(), line.lastIndexOf(HgCommand.HG_MERGE_FAILED3_ERR)).trim();
                }
                filepath = root.getPath() + "/" + filepath; // NOI18N
