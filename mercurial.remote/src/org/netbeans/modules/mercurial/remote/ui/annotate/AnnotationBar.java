...
final class AnnotationBar extends JComponent implements Accessible, PropertyChangeListener, DocumentListener, ChangeListener, ActionListener, Runnable, ComponentListener {
     ...
     */
    private Map<String, VCSFileProxy> originalFiles;
    /**
    ...
    public void annotationLines(VCSFileProxy file, List<AnnotateLine> annotateLines) {
        ...
        doc.render(new Runnable() {
            ...
            public void run() {
                ...
                originalFiles = Collections.synchronizedMap(new HashMap<String, VCSFileProxy>());
    ...
    private JPopupMenu createPopup(MouseEvent e) {
        ...
        final VCSFileProxy originalFile = al == null ? null : VCSFileProxy.createFileProxy(repositoryRoot, al.getFileName());
        ...
        if (changesetIdPerLine != null) {
            ...
            showCommitMenu.addActionListener(new ActionListener() {
                @Override
                public void actionPerformed(ActionEvent e) {
                    LogAction.openHistory(repositoryRoot, new VCSFileProxy[] { file }, changesetIdPerLine);
                }
            });
        ...
        previousAnnotationsMenu.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                pri.runWithRevision (new Runnable() {
                    ...
                    public void run() {
                        try {
                            if (lineNumber > 0 && !"-1".equals(pri.getPreviousRevision().getRevisionNumber())) { //NOI18N
        ...
        if (revisionPerLine != null) {
            String key = getPreviousRevisionKey(originalFile.getPath(), revisionPerLine);
    ...
    private class PreviousRevisionInvoker {
        ...
        private PreviousRevisionInvoker(String revisionPerLine, VCSFileProxy originalFile) {
        ...
        private VCSFileProxy getOriginalFile () {
            return originalFile;
        }
    }

    private void revert(final VCSFileProxy file, String revision) {
        final VCSFileProxy root = Mercurial.getInstance().getRepositoryRoot(file);
        if(root == null) {
            return;
        }
    ...
    private HgRevision getParentRevision(VCSFileProxy file, String revision) {
        String key = getPreviousRevisionKey(file.getPath(), revision);
        HgRevision parent = getPreviousRevisions().get(key);
        if (parent == null) {
            VCSFileProxy originalFile = getOriginalFile(file, revision);
            try {
    ...
    private VCSFileProxy getOriginalFile (VCSFileProxy file, String revision) {
        String key = getPreviousRevisionKey(file.getPath(), revision);
        VCSFileProxy originalFile = getOriginalFiles().get(key);
    ...
    private Map<String, VCSFileProxy> getOriginalFiles () {
        Map<String, VCSFileProxy> files = originalFiles;
        return files == null ? new HashMap<String, VCSFileProxy>(0) : files;
    }
