...
import static org.netbeans.modules.mercurial.remote.ui.branch.Bundle.*;
...
public class SwitchToBranchAction extends ContextAction {
    ...
    public void doSwitch (final VCSFileProxy root, final String revStr, final boolean doForcedUpdate, final VCSContext ctx) throws MissingResourceException {
        ...
        HgProgressSupport support = new HgProgressSupport() {
            ...
            public void perform() {
                ...
                try {
                    ...
                    logger.output(NbBundle.getMessage(SwitchToBranchAction.class, "MSG_SWITCH_INFO_SEP", revStr, root.getPath())); //NOI18N
                    HgUtils.runWithoutIndexing(new Callable<Void>() {
                        @Override
                        public Void call () throws Exception {
                            List<String> list = HgCommand.doUpdateAll(root, doForcedUpdate, revStr);
                            Utils.insert(NbPreferences.forModule(SwitchToBranchAction.class), PREF_KEY_RECENT_BRANCHES + root.getPath(), revStr, 5);
