...
# JDK 1.5.0_11 installation directory. (Full JDK, not just JRE.)
#
# nbtestjdk=/opt/java/j2se/1.5.0_11
...
if [ "$override" != yes ]
then
    if $nbjdk/bin/java -version 2>&1 | fgrep -q -v 1.5.0_11
    then
        echo "You need to set the variable 'nbjdk' to a JDK 1.5.0_11 installation" 1>&2
        exit 2
    fi
