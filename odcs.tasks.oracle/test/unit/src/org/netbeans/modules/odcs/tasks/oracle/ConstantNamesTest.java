...
public class ConstantNamesTest extends NbTestCase {
    ...
    public static Test suite() {
        ...
        for (Field f : C2CData.class.getDeclaredFields()) {
            if (f.getName().startsWith("ATTR_")) {
                s.addTest(new ConstantNamesTest(
                    ...
                    CloudDevAttribute.class, f.getName(), f.getName().substring(5)
                ));
                cnt++;
            }
        }
        ...
        s.addTest(new ConstantNamesTest(
            "CUSTOM_FIELD_PREFIX", C2CData.class, CloudDevConstants.class, 
            "CUSTOM_FIELD_PREFIX", "CUSTOM_FIELD_PREFIX"
        ));
        cnt = 0;
        for (Field orig : CloudDevAttribute.class.getFields()) {
            try {
            ...
            try {
                ...
                s.addTest(new ConstantNamesTest(
                    ...
                    TaskAttribute.class, CloudDevAttribute.class, f.getName(), f.getName()
                ));
                cnt++;
            } catch (Exception ex) {
                continue;
    ...
    protected void runTest() throws Throwable {
        ...
        if (v2 instanceof CloudDevAttribute) {
            CloudDevAttribute ta = (CloudDevAttribute)v2;
            v2 = ta.getName();
        }
