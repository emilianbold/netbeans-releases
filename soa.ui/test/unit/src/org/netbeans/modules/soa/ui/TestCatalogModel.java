...
public class TestCatalogModel implements CatalogModel {

    private Map<FileObject, Document> mFileToDocumentMap;
    private boolean mDocumentPooling = true;

    public TestCatalogModel(boolean documentPooling) {
    ...
    public ModelSource getModelSource(URI locationURI,
        ...
        //
        Lookup lookup = Lookups.fixed(fo, doc, this);
    ...
    private Document getDocument(FileObject fo){
        ...
        if (result == null) {
            try {
                result = TestUtils.loadDocument(fo.getInputStream());
                if (mDocumentPooling) {
