...
public class ComponentDiagramRelationshipDiscoveryTests extends UMLTestCase {
    ...
    public ComponentDiagramRelationshipDiscoveryTests(String name) {
        super(name);
    }
    
    public static NbTestSuite suite() {
        NbTestSuite suite = new NbTestSuite(org.netbeans.test.uml.relationshipdiscovery.ComponentDiagramRelationshipDiscoveryTests.class);
        return suite;
    }
   public static void main(java.lang.String[] arg){
    ...
    /******************const section*************************/
    private String DIAGRAM_NAME = "EmptyComponentDiagram";  
    ...
    protected void setUp() {
        ...
        counter++;
        workPkg="pkg_cmp_"+counter;
        diagram="dgr"+counter;
        ProjectsTabOperator pto = ProjectsTabOperator.invoke();
        ProjectRootNode root = new ProjectRootNode(pto.tree(),prName);
        
        org.netbeans.test.umllib.Utils.RetAll rt=org.netbeans.test.umllib.Utils.createDiagram(prName,workPkg,diagram,NewDiagramWizardOperator.COMPONENT_DIAGRAM);
    ...
    public void testRecoverDelegate() throws NotFoundException {        
         lastTestCase=getCurrentTestMethodName();
            boolean result = false;
            try
            {
                result = classVerifier.verifyElementsRelationRecovery(ElementTypes.COMPONENT, ElementTypes.INTERFACE, LinkTypes.DELEGATE);
            }
            catch(ElementVerificationException ex)
            {
                assertFalse(91055,"delegate link isn't restored",ex.getId()==classVerifier.ID_NOLINK);
                throw ex;
            }
            if (!result){
                fail("testRecoverDelegate failed. Reason unknown");
            }
    }
    ...
    public void testRecoverNestedLink() throws NotFoundException {
         lastTestCase=getCurrentTestMethodName();
            ...
            catch(ElementVerificationException ex)
            {
                ...
                catch(ElementVerificationException ex2)
                {
    ...
    public void testRecoverAssembly() throws NotFoundException {        
         lastTestCase=getCurrentTestMethodName();
            BaseRelatioshipRecoveryVerifier custom = new BaseRelatioshipRecoveryVerifier(classVerifier){                               
                protected LinkOperator addRelatioship(DiagramElementOperator el1, DiagramElementOperator el2, LinkTypes linkType) throws NotFoundException{
                    dia.createGenericRelationshipOnDiagram(linkType, el1, el2);                    
                    DiagramElementOperator port = new DiagramElementOperator(dia, new DiagramElementOperator.ElementByTypeChooser(ElementTypes.PORT), 0);
                    eventTool.waitNoEvent(1000);     
                    try{Thread.sleep(100);}catch(Exception ex){}
                    return new LinkOperator(port, el2);        
                }
                
                protected boolean checkRelationshipOnDiagram(DiagramElementOperator source, DiagramElementOperator destination, LinkTypes linkType){
                    try{
                        DiagramElementOperator port = new DiagramElementOperator(dia, new DiagramElementOperator.ElementByTypeChooser(ElementTypes.PORT), 0);                    
                        return super.checkRelationshipOnDiagram(port, destination, LinkTypes.USAGE);
                    }catch(Exception e){
                        return false;
                    }
                }  
            ...
            boolean result = custom.verifyElementsRelationRecovery(ElementTypes.COMPONENT, ElementTypes.INTERFACE, LinkTypes.ASSEMBLY);
            
            long timeoutValDlg = JemmyProperties.getCurrentTimeout("DialogWaiter.WaitDialogTimeout");
            try{                        
                JemmyProperties.setCurrentTimeout("DialogWaiter.WaitDialogTimeout", 1000);
                new JDialogOperator(EXCEPTION_DLG);                
                fail("Unknown excpetion");
            }catch(Exception excp){
            }finally{
                JemmyProperties.setCurrentTimeout("DialogWaiter.WaitDialogTimeout", timeoutValDlg);
            }
            ...
            if (!result){
                fail("testRecoverAssembly failed. Reason unknown");
            }
    }
