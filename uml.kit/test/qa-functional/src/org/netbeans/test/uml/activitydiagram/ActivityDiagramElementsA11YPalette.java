 ...
 */
public class ActivityDiagramElementsA11YPalette extends UMLTestCase {
    ...
    private static String prName= "ActivityDiagramProjectEA11YP";
    ...
    public ActivityDiagramElementsA11YPalette(String name) {
        super(name);
    }
     public static NbTestSuite suite() {
        NbTestSuite suite = new NbTestSuite(org.netbeans.test.uml.activitydiagram.ActivityDiagramElementsA11YPalette.class);
        return suite;
    }
   ...
   private void testElementPlace(String workPkg,String diagramName,String element,String elementName,ElementTypes elementType)
   {
       ...
       //
        ...
        MainWindowOperator.getDefault().pushKey(KeyEvent.VK_ENTER,KeyEvent.CTRL_MASK);
        //pl.pushKey(KeyEvent.VK_ENTER,KeyEvent.CTRL_MASK);
        new EventTool().waitNoEvent(500);
        //
        try
        {
            ...
            //
            DiagramElementOperator pkg=null;
            //
            try
            {
                pkg=new DiagramElementOperator(d,workPkg,ElementTypes.PACKAGE);
                
            }catch(Exception ex)
            {
               
            }
            assertFalse(103231,"Ctrl-enter added element from project tree and palette at the same time",pkg!=null && d.getAllDiagramElements().size()==2);
            //check number of elements
            assertTrue("There are extra elemnents on diagram after addition from Palette",d.getAllDiagramElements().size()==1);
        }
        catch(Exception ex)
        {
            try
       ...
       //
        ...
        projectTab=new TopComponentOperator("Projects");
        JTreeOperator tr=new JTreeOperator(projectTab);
        
        if(elementName.equals(defaultNewElementName))
        {    
           tr.selectPath(tr.findPath(prName+"|"+"Model|"+workPkg+"|"+diagramName+"|"+elementName));
           tr.waitSelected(tr.findPath(prName+"|"+"Model|"+workPkg+"|"+diagramName+"|"+elementName));
        }
        else
        {
           tr.selectPath(tr.findPath(prName+"|"+"Model|"+workPkg+"|"+diagramName+"|"+elementType));
           tr.waitSelected(tr.findPath(prName+"|"+"Model|"+workPkg+"|"+diagramName+"|"+elementType));  
        }
        PropertySheetOperator ps=new PropertySheetOperator();
        if(elementName.equals(""))
        {
            assertTrue("Property sheet isn't for element type "+elementType+", but for "+ps.getName(),(elementType+" - Properties").equals(ps.getName()));
        }
        else
        {
            assertTrue("Property sheet isn't for element name "+elementName+", but for "+ps.getName(),(elementName+" - Properties").equals(ps.getName()));
        }
        //
