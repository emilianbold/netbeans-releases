...
public class SQDSyncMessageContextMenuTests  extends UMLTestCase {
    
    public SQDSyncMessageContextMenuTests(String name) {
        super(name);
    }
    
    public static NbTestSuite suite() {
        NbTestSuite suite = new NbTestSuite(SQDSyncMessageContextMenuTests.class);
        return suite;
    }
        
        
    /******************const section*************************/
    private String PROJECT_NAME = "SQD_umlSMCMT";      
    ...
    private LinkOperator createWorkingLink(String lnName, String className) throws NotFoundException{
        ...
        return dia.createLinkOnDiagram(LinkTypes.SYNC_MESSAGE, line1, line2);
    }
   ...
   public void testEdit_Copy(){
        ...
        boolean enabled = false;                
            LinkOperator lnk = createWorkingLink(lineName, className);
   ...
   public void testEdit_Delete(){
        ...
        boolean enabled = true;                
            LinkOperator lnk = createWorkingLink(lineName, className);
            final DiagramElementOperator ln1 = line1;
            final DiagramElementOperator ln2 = line2;        
            eventTool.waitNoEvent(1500);
            final SequenceDiagramOperator tmpDia = new SequenceDiagramOperator(DIAGRAM);
            GenericContextMenuVerifier verifier = new GenericContextMenuVerifier(lnk, dia){
                protected boolean checkActionResult(){                                      
                    ...
                    try{
                        ...
                        new LinkOperator(ln1, ln2, LinkTypes.CREATE_MESSAGE);
                        return false;
                    }catch(Exception e){                        
                    }finally{
   ...
   public void testEdit_SelectAll(){
        ...
        boolean enabled = true;                
            final LinkOperator lnk1 = createWorkingLink(lineName, className);
            final DiagramElementOperator ln1 = line1;
            final DiagramElementOperator ln2 = line2;        
            final LinkOperator lnk2 = dia.createLinkOnDiagram(LinkTypes.SYNC_MESSAGE, ln1, ln2);
            eventTool.waitNoEvent(1500);
            final SequenceDiagramOperator tmpDia = new SequenceDiagramOperator(DIAGRAM);
            GenericContextMenuVerifier verifier = new GenericContextMenuVerifier(lnk1, dia){
                protected boolean checkActionResult(){
                    eventTool.waitNoEvent(2000);
   ...
   public void testEdit_SelectAllSimilar(){
        ...
        boolean enabled = true;                
            final LinkOperator lnk1 = createWorkingLink(lineName, className);
            final LinkOperator lnk2 = dia.createLinkOnDiagram(LinkTypes.SYNC_MESSAGE, line1, line2);            
            ...
            GenericContextMenuVerifier verifier = new GenericContextMenuVerifier(lnk1, dia){
                protected boolean checkActionResult(){
                    eventTool.waitNoEvent(2000);
                    ArrayList<DiagramElementOperator> al = tmpDia.getDiagramElements();
                    String elType = lnk1.getType();
                    for(int i=0;i<al.size();i++){
                        DiagramElementOperator tmpEl = al.get(i);
                        if ((lnk1.isSelected()&&!lnk1.getType().equals(elType)) || (!lnk1.isSelected()&&lnk1.getType().equals(elType))){
                            return false;
                        }
        ...
        boolean enabled = true;                
            ...
            GenericContextMenuVerifier verifier = new GenericContextMenuVerifier(lnk1, dia){
                protected boolean checkActionResult(){
                    ...
                    HashSet<LinkOperator> set = ln1.getLinks();                    
                    String elType = lnk1.getType();
                    Iterator<LinkOperator> it = set.iterator();
                    while (it.hasNext()){
                        LinkOperator tmpEl = it.next();
                        if (lnk1.getSource()!=tmpEl.getSource() && !tmpEl.isSelected()){
                           return false;
                        }
                    }                   
   ...
   public void testFind_ElementInProjectTree(){
        lastTestCase=getCurrentTestMethodName();;
        final String lineName = "testFEPT";
        final String className = "FEPT";
        final String popupPath = "Find|Element In Project Tree";
        boolean enabled = true;                
            final LinkOperator lnk1 = createWorkingLink(lineName, className);
            eventTool.waitNoEvent(1500);
            final SequenceDiagramOperator tmpDia = new SequenceDiagramOperator(DIAGRAM);
   ...
   public void testFind_SourceElement(){
        lastTestCase=getCurrentTestMethodName();;
        final String lineName = "testFSE";
        final String className = "FSE";
        final String popupPath = "Find|Source Element";        
        boolean enabled = true;                
            final LinkOperator lnk1 = createWorkingLink(lineName, className);
            final DiagramElementOperator ln1 = line1;        
            eventTool.waitNoEvent(1500);
            ...
            GenericContextMenuVerifier verifier = new GenericContextMenuVerifier(lnk1, dia){
                protected boolean checkActionResult(){
                    return ln1.isSelected();                    
                }
            };
            boolean result = verifier.verifyElement(popupPath, enabled);
            if (!result){
                fail("testFind_SourceElement failed. Reason unknown");
            }            
    }
   
   
   public void testFind_TargetElement(){
        lastTestCase=getCurrentTestMethodName();;
        final String lineName = "testFTE";
        final String className = "FTE";
        final String popupPath = "Find|Target Element";              
        boolean enabled = true;                
            final LinkOperator lnk1 = createWorkingLink(lineName, className);
            final DiagramElementOperator ln2 = line2;  
            eventTool.waitNoEvent(1500);
            ...
            GenericContextMenuVerifier verifier = new GenericContextMenuVerifier(lnk1, dia){
                protected boolean checkActionResult(){
                    return ln2.isSelected();                    
                }
            };
            boolean result = verifier.verifyElement(popupPath, enabled);
            if (!result){
                fail("testFind_TargetElement failed. Reason unknown");
            }            
    }
   ...
   public void testProperties(){
        ...
        boolean enabled = true;                
            ...
            GenericContextMenuVerifier verifier = new GenericContextMenuVerifier(lnk, dia){
                protected boolean checkActionResult(){
                    try{
                        new JButtonOperator(new JDialogOperator("Message - Properties"), "Close").push();
                        return true;                    
                    }catch(Exception e){
                        return false;
                    }
