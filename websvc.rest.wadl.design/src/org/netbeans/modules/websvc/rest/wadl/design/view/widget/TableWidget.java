...
public class TableWidget extends Widget{
    ...
    private void createTable() {
        ...
        for(int j=0; j<model.getRowCount();j++) {
            ...
            addChild(rowWidget);
            createCellWidget(rowWidget, j, noCols);
        }
    }
    
    private void createCellWidget(Widget rowWidget, int j, int noCols) {
        for (int i = 0; i < noCols; i++) {
            if(model.getTypeAt(j, i) == null)
                rowWidget.addChild(createLabelWidget(i, j));
            else
                rowWidget.addChild(createComboBoxWidget(i, j));
        }
    }
    ...
    private Widget createComboBoxWidget(final int col, final int row) {
        JComboBox cb = new JComboBox(model.getTypeAt(row, col));
        cb.setSelectedItem(model.getValueAt(row, col).toLowerCase());
        cb.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                JComboBox tf = (JComboBox) e.getSource();
                model.setValueAt((String) tf.getSelectedItem(), row, col);
            }
        });
        ComponentWidget cellWidget = new ComponentWidget(getScene(), cb);
        if (col != 0) {
            cellWidget.setBorder(new LineBorder(0, 1, 0, 0, BORDER_COLOR));
        }
        cellWidget.setFont(getScene().getFont().deriveFont(Font.BOLD));
