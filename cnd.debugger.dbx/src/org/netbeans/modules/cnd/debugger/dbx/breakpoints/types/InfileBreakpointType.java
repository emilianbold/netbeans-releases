...
//@BreakpointType.Registration(displayName="#LBL_Infile")
public class InfileBreakpointType extends NativeBreakpointType {

    // interface BreakpointType
    public NativeBreakpoint newInstance(int flags) {
	return new InfileBreakpoint(flags);
    } 

    // interface BreakpointType
    @Override
    public String getTypeDisplayName() {
	return Catalog.get("LBL_Infile");
    }

    // interface NativeBreakpointType
    public BreakpointPanel getCustomizer(NativeBreakpoint editable) {
	if (editable == null)
	    return new InfileBreakpointPanel();
	else
