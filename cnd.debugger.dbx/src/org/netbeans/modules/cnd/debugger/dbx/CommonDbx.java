...
import org.netbeans.modules.cnd.debugger.common2.debugger.remote.RemoteSupport;
...
public abstract class CommonDbx extends GPDbxSurrogate {
    ...
    /*
     * Glue has an annoying habit of making arrays be one bigger than they
     * should be, so compensate as early as we can.
     */
    protected static String[] fixStrings(int count, String in[]) {
	String[] out = new String[count];
	System.arraycopy(in, 0, out, 0, count);
	return out;
    }

    protected static GPDbxLocalItem[] fixLocals(int count,
						      GPDbxLocalItem in[]) {
	GPDbxLocalItem[] out = new GPDbxLocalItem[count];
	System.arraycopy(in, 0, out, 0, count);
	return out;
    }

    protected static GPDbxThread[] fixThreads(int count,
						    GPDbxThread in[]) {
	GPDbxThread[] out = new GPDbxThread[count];
	System.arraycopy(in, 0, out, 0, count);
	return out;
    }

    protected static GPDbxFrame[] fixStack(int count,
						 GPDbxFrame in[]) {
	GPDbxFrame[] out = new GPDbxFrame[count];
	System.arraycopy(in, 0, out, 0, count);
	return out;
    }

    protected static GPDbxError[] fixErrors(int count,
						  GPDbxError in[]) {
	GPDbxError[] out = new GPDbxError[count];
	System.arraycopy(in, 0, out, 0, count);
	return out;
    }

    protected static GPDbxPathMap[] fixPathMap(int count,
						     GPDbxPathMap in[]) {
	GPDbxPathMap[] out = new GPDbxPathMap[count];
	System.arraycopy(in, 0, out, 0, count);
	return out;
    }

    protected static GPDbxEventRecord[] fixEventRecords(int count,
						       GPDbxEventRecord in[]) {
	GPDbxEventRecord[] out = new GPDbxEventRecord[count];
	System.arraycopy(in, 0, out, 0, count);
	return out;
    }

    protected static GPDbxDisplayItem[] fixDisplayItems(int count,
						       GPDbxDisplayItem in[]) {
	GPDbxDisplayItem[] out = new GPDbxDisplayItem[count];
	System.arraycopy(in, 0, out, 0, count);
	return out;
    }

    protected static GPDbxSignalInfoInit[] fixSignalInfoInit(int count,
							  GPDbxSignalInfoInit in[]) {
	GPDbxSignalInfoInit[] out = new GPDbxSignalInfoInit[count];
	System.arraycopy(in, 0, out, 0, count);
	return out;
    }
