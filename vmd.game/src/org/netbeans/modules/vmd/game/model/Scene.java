...
public class Scene implements GlobalRepositoryListener, PropertyChangeListener,
        Previewable, Editable, Identifiable {
    ...
    private void updateLayersBounds() {
        ...
        if (!this.allLayersBounds.equals(newBounds)) {
            ...
            this.propertyChangeSupport.firePropertyChange(PROPERTY_LAYERS_BOUNDS,
                    oldBounds, newBounds);
        }
    }

    public TiledLayer createTiledLayer(String name, ImageResource imageResource,
            int rows, int columns, int tileWidth, int tileHeight) {
        TiledLayer layer = this.gameDesign.createTiledLayer(name, imageResource, rows, columns, tileWidth, tileHeight);
        this.append(layer);
        return layer;
    }

    public Sprite createSprite(String name, ImageResource imageResource,
            int numberFrames, int frameWidth, int frameHeight) {
        Sprite sprite = this.gameDesign.createSprite(name, imageResource,
                numberFrames, frameWidth, frameHeight);
        this.append(sprite);
        return sprite;
    }
    ...
    public void append(Layer layer) {
        if (DEBUG) {
            System.out.println(this + " append " + layer); // NOI18N
        }
    ...
    public void insert(Layer layer, int index) {
        if (DEBUG) {
            System.out.println(this + " insert " + layer); // NOI18N
        }
        if (this.layers.contains(layer)) {
    ...
    public void remove(Layer layer) {
        if (DEBUG) {
            System.out.println(this + " remove " + layer); // NOI18N
        }
        int index = layers.indexOf(layer);
    ...
    public void move(Layer layer, int newIndex) {
        ...
        if (oldIndex == -1) {
            if (DEBUG) {
                System.out.println(this + " cannot move " + layer + " - it is not present"); // NOI18N
            }
            return;
        }
        if (DEBUG) {
            System.out.println(this + " move " + layer + " from " + oldIndex + " to " + newIndex); // NOI18N
        }
    ...
    private void fireLayerPositionModified(Layer layer, int index,
    ...
    public void setLayerPosition(Layer layer, Point position, boolean inTransition) {
        ...
        if (DEBUG) {
            System.out.println(layer + " set position " + position); // NOI18N
        }
...
//		}
    ...
    public void setLayerVisible(Layer layer, boolean visible) {
        if (DEBUG) {
            System.out.println(layer + " set visible " + visible); // NOI18N
        }
    ...
    public void setLayerLocked(Layer layer, boolean locked) {
        if (DEBUG) {
            System.out.println(layer + " set locked " + locked); // NOI18N
        }
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        if (DEBUG) {
            System.out.println("property change from " + evt.getSource() + " new value: " + evt.getNewValue()); // NOI18N
        }
    }
