...
public class MainView implements GlobalRepositoryListener, EditorManager {
	...
	private static void loadPreviews() {
		...
		ImageResource bunnyImgRes = ImageResource.getImageResource(urlBunny, 20, 20);
		Sprite bunny = scene.createSprite("bunny2", bunnyImgRes, 8);
		Sequence bunnyDefSeq = bunny.getDefaultSequence();
		bunnyDefSeq.setFrame(new StaticTile(bunnyImgRes, 1), 0);
		bunnyDefSeq.setFrame(new StaticTile(bunnyImgRes, 2), 1);
		bunnyDefSeq.setFrame(new StaticTile(bunnyImgRes, 3), 2);
		bunnyDefSeq.setFrame(new StaticTile(bunnyImgRes, 4), 3);
		bunnyDefSeq.setFrame(new StaticTile(bunnyImgRes, 5), 4);
		bunnyDefSeq.setFrame(new StaticTile(bunnyImgRes, 6), 5);
		bunnyDefSeq.setFrame(new StaticTile(bunnyImgRes, 7), 6);
		bunnyDefSeq.setFrame(new StaticTile(bunnyImgRes, 8), 7);
		bunnyDefSeq.setFrameMs(100);
		
		Sequence shorter = bunny.createSequence("shorter", 4);
		shorter.setFrame(new StaticTile(bunnyImgRes, 3), 0);
		shorter.setFrame(new StaticTile(bunnyImgRes, 5), 1);
		shorter.setFrame(new StaticTile(bunnyImgRes, 7), 2);
		shorter.setFrame(new StaticTile(bunnyImgRes, 9), 3);
		shorter.setFrameMs(130);
		
		URL urlTrees = MainView.class.getResource("res/color_tiles.png");
		ImageResource treesImgRes = ImageResource.getImageResource(urlTrees, 20, 20);
		...
		pineTree.getDefaultSequence().addFrame(new StaticTile(treesImgRes, 2));
		pineTree.getDefaultSequence().addFrame(new StaticTile(treesImgRes, 3));
		AnimatedTile palmTree = treesImgRes.createAnimatedTile("palmTree2", 4);
		palmTree.getDefaultSequence().addFrame(new StaticTile(treesImgRes, 5));
		...
		ImageResource imgRes = ImageResource.getImageResource(urlTiles, 20, 20);
		
		TiledLayer grass = scene.createTiledLayer("grass2", ImageResource.getImageResource(urlTiles, 20, 20), 10, 20);
		for (int i = 0; i < 10; i++) {
			for (int j = 0; j < 20; j++) {
		...
		TiledLayer water = scene.createTiledLayer("water2", ImageResource.getImageResource(urlTiles, 20, 20), 4, 3);
		...
		TiledLayer things = scene.createTiledLayer("things2", ImageResource.getImageResource(urlTiles, 20, 20), 7, 7);
