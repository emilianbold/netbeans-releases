...
public class CndIndexer extends CustomIndexer {
    ...
    protected void index(Iterable<? extends Indexable> files, Context context) {
        ...
        for (Indexable idx : files) {
            ...
            URI uri = fo.toURI();
            Collection<NativeProject> nativeProjects = CMUtilities.getNativeProjects(fo);
            for (NativeProject p : nativeProjects) {
                ...
                Collection<URI> translationUnitsURI = SPIUtilities.getTranslationUnitsURI(uri);
                for (URI mainFileURI : translationUnitsURI) {
                    CMCompilationDataBase.Entry fileEntry = cdb.getFileEntry(mainFileURI);
                    NativeProjectBridge.getInstance().reIndex(p, indices, fileEntry);
                }
            }
        }
    }
