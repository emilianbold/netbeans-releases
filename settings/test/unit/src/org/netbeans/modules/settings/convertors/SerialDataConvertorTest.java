...
public class SerialDataConvertorTest extends NbTestCase {
    ...
    public void testSaveCookieChanges() throws Exception {
        if (Boolean.getBoolean("ignore.random.failures")) {
         ...
         InstanceDataObject i = InstanceDataObject.create (folder, null, f, null);
         assertNull(i.getCookie(SaveCookie.class));
         Lookup.Result<SaveCookie> scr =  i.getLookup().lookupResult(SaveCookie.class);
         scr.addLookupListener(ml);
         Collection<? extends SaveCookie>  saveCookies = scr.allInstances();
         assertFalse(saveCookies.contains(f));

         f.setRootDirectory(getWorkDir());
    ...
    public void test50177ProblemSimulation () throws Exception {
        ...
        Lookup.Item<Ex50177> item = Lookup.getDefault().lookupItem(new Lookup.Template<Ex50177>(Ex50177.class, null, null));
        assertNotNull(item);
        String id = item.getId();

        Ex50177 exObj = item.getInstance();
        assertNotNull(exObj);
        ...
        assertNotNull(Lookup.getDefault().lookupItem(new Lookup.Template<Ex50177>(null, id, null)));
    }
    ...
    private static void waitUntilIsSaved (InstanceDataObject ido) throws InterruptedException {
            ...
            for (int i = 0; i < 5 && sc != null; i++) {
                Thread.sleep (3000);
                sc = ido.getCookie(SaveCookie.class);
            }
            assertNull(sc);
    }
    ...
    */
    public void testCreateFromTemplateForSettingsFile () throws Exception {
        ...
        InstanceCookie ic = newObj.getCookie(InstanceCookie.class);
...
//        Object ser = new java.awt.Button ();
        ...
        Lookup.Result<?> res = l.lookupResult(ser.getClass());
        ...
        Set<?> origSet = new HashSet<Object>(Arrays.asList(ser));
        assertEquals("wrong lookup result", origSet, new HashSet<Object>(col));
        ...
        ic = ido.getCookie(InstanceCookie.class);
        origSet = new HashSet<Object>(Arrays.asList(ic.instanceCreate()));
...
//            System.err.println(name + " serialized");
    ...
    public void testDeleteSettings() throws Exception {
        ...
        InstanceDataObject ido = InstanceDataObject.create(f, filename, obj, null, false);
        assertNotNull("InstanceDataObject.create cannot return null!", ido);
        ...
        Ser ser = new Ser("bla");
        ido = InstanceDataObject.create(f, filename, ser, null, false);
        assertNotNull("InstanceDataObject.create cannot return null!", ido);
