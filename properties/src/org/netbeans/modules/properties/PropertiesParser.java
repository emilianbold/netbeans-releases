...
/*     int mode = 0;                               
     ...
     while (true) {
       ...
       while (++index == fl.stringValue.length())
         if (++part == lines.size())
           break outerloop;
         else
           index = -1;
           
       System.out.println("POS: (" + part + ", " + index + ")");
       
       char cur = fl.stringValue.charAt(index);
       
       switch (mode) {
         case 0:
           // looking for the start of key
           if (UtilConvert.whiteSpaceChars.indexOf(cur) == -1) {
             keyStartPos = fl.startPosition + index;
             mode = 1;
           }  
           break;
         case 1:
           // looking for separation between key and value
           if (cur == '\\') 
             index++; // another letter is guaranteed on this line
           else 
             if(UtilConvert.keyValueSeparators.indexOf(cur) != -1) {
               separatorPos = fl.startPosition + index;
               mode = 2; 
             }
           break;
     ...
     // Find start of key
     int keyStart;
     fl = (FlaggedLine)lines.get(0);
     for(keyStart = 0; keyStart < fl.stringValue.length(); keyStart++) {
       if (UtilConvert.whiteSpaceChars.indexOf(fl.stringValue.charAt(keyStart)) == -1)
         break;
     }        
     int keyStartPos = fl.startPosition + keyStart;
     
     // Find separation between key and value
     int separatorIndex = 0;
     int part;                            
     findSeparatorIndex: // label the loop
     for (part=0; part<lines.size(); part++) {
       fl = (FlaggedLine)lines.get(part);
       for(separatorIndex = keyStart; separatorIndex < fl.stringValue.length(); separatorIndex++) {
         char currentChar = fl.stringValue.charAt(separatorIndex);
         if (currentChar == '\\')
           separatorIndex++;
         else if(UtilConvert.keyValueSeparators.indexOf(currentChar) != -1)
           break findSeparatorIndex;
       }
     }                       
     int separatorIndexPos = fl.startPosition + separatorIndex;
     
     // Skip over whitespace after key if any
     int valueIndex = 0;
     skipWSAfterKey:
     for (; part<lines.size(); part++) {
       fl = (FlaggedLine)lines.get(part);
       for (valueIndex=separatorIndex + 1; valueIndex < fl.stringValue.length(); valueIndex++)
         if (UtilConvert.whiteSpaceChars.indexOf(fl.stringValue.charAt(valueIndex)) == -1)
           break skipWSAfterKey;
     }

     // Skip over one non whitespace key value separators if any
     if (valueIndex < fl.stringValue.length())
       if (UtilConvert.strictKeyValueSeparators.indexOf(fl.stringValue.charAt(valueIndex)) != -1)
         valueIndex++;
     if (valueIndex == fl.stringValue.length() && (part < lines.size() - 1)) {
       part++;
       fl = (FlaggedLine)lines.get(part);
       valueIndex=;;
     ...
     int valueIndexPos = fl.startPosition + valueIndex;*/
         
/*     findStartKey:
     for (int part=0; part<lines.size(); part++) {
       fl = (FlaggedLine)lines.get(part);
       int keyStart;
       int len = fl.stringValue.length();
       for(keyStart = 0; keyStart < len; keyStart++) {
         if (whiteSpaceChars.indexOf(fl.stringValue.charAt(keyStart)) == -1)
           break findStartKey;
       }
     }*/
