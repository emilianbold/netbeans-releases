        ...
        implements PropertyChangeListener, ChangeListener, MouseWheelListener
{
    ...
    private JScrollPane wrapperScrollPane;
    
    private boolean origWheelScrollingEnabled;
            ...
            if (listeningOnViewport != viewport) {
                ...
                if (listeningOnViewport != null) {
                    ...
                    if (scrollPane instanceof JScrollPane) {
                        // Assume JViewport's parent JScrollPane won't change without viewport change as well
                        wrapperScrollPane = (JScrollPane) scrollPane;
                        origWheelScrollingEnabled = wrapperScrollPane.isWheelScrollingEnabled();
                        wrapperScrollPane.setWheelScrollingEnabled(false); // Handle wheel scrolling by a listener in this class
                        wrapperScrollPane.addMouseWheelListener(this);
                    }
                }
            }
...
//        assert SwingUtilities.isEventDispatchThread() : "Must be called in EDT"; // NOI18N
        if (listeningOnViewport != null) {
            if (wrapperScrollPane != null) {
                wrapperScrollPane.removeMouseWheelListener(this);
                wrapperScrollPane.setWheelScrollingEnabled(origWheelScrollingEnabled);
            }
...
//            renderingHints = (Map<?, ?>) defaultColoring.getAttribute(EditorStyleConstants.RenderingHints);
    ...
    public void mouseWheelMoved(MouseWheelEvent evt) {
        ...
        if (evt.getScrollType() != MouseWheelEvent.WHEEL_UNIT_SCROLL) {
            basicScrollPaneUIMouseWheelMoved(wrapperScrollPane, evt);
//          evt.consume(); // consuming the event has no effect
    ...
    private static void basicScrollPaneUIMouseWheelMoved(JScrollPane scrollpane, MouseWheelEvent e) {
        if (e.getWheelRotation() != 0) {
            ...
            if (e.getScrollType() == MouseWheelEvent.WHEEL_UNIT_SCROLL) {
                ...
                if (Boolean.TRUE == fastWheelScroll
                    ...
                    basicScrollBarUIScrollByUnits(toScroll, direction,
                            units, limitScroll);
                }
            } else if (e.getScrollType()
                    == MouseWheelEvent.WHEEL_BLOCK_SCROLL) {
                basicScrollBarUIScrollByBlock(toScroll, direction);
            }
        }
    }

    private static void basicScrollBarUIScrollByUnits(JScrollBar scrollbar, int direction,
    ...
    private static void basicScrollBarUIScrollByBlock(JScrollBar scrollbar, int direction) {
        // This method is called from BasicScrollPaneUI to implement wheel
