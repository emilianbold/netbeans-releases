...
public class ODSHandler {
    ...
    public ODSHandler (QueryAccessorImpl qaImpl, CloudServer server) {
        this.qaImpl = qaImpl;
        this.server = server;
        ...
        list = new PropertyChangeListener() {
            ...
            public void propertyChange (PropertyChangeEvent evt) {
                if (evt.getPropertyName().equals(DefaultDashboard.PROP_REFRESH_REQUEST) && evt.getSource() instanceof DefaultDashboard) {
                    if (ODSHandler.this.server.getUrl().equals(((DefaultDashboard) evt.getSource()).getServer().getUrl())) {
                        clear();
                    }
                }
            }
    ...
    List<QueryHandle> getQueryHandles (ProjectHandle<ODSProject> project, Query... queries) {
    ...
    void registerProject (ProjectHandle<ODSProject> project, List<QueryHandle> queries) {
        ProjectListener pl;
        synchronized (projectListeners) {
    ...
    void registerRepository (Repository repo, ProjectHandle<ODSProject> project) {
    ...
    private class ProjectListener implements PropertyChangeListener {
        ...
        private ProjectHandle<ODSProject> ph;
