...
class KenaiProjectImpl extends org.netbeans.modules.bugtracking.kenai.spi.KenaiProject {
    ...
    public static String getProjectKey (ODSProject project) {
        return project.getServer().getUrl().toString() + ":" + project.getName();
    }

    private ODSProject project;

    private KenaiProjectImpl (ODSProject project) {
        assert project != null;
        this.project = project;
    }

    public static KenaiProjectImpl getInstance(ODSProject project) {
    ...
    public void fireQueryActivated(Query query) {
        ODSHandler handler = Support.getInstance().getODSHandler(project.getServer());
        if(handler == null) {
            return;
        }
