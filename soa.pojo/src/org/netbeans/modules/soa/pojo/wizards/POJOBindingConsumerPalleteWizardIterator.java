 ...
 * @author sgenipudi
 */
public class POJOBindingConsumerPalleteWizardIterator extends WSDLWizardIterator {
    WizardDescriptor wizard = null;
    private String[] mPojoSteps;
    private int mStepNo = 0;
    private boolean mParentFirstStepPanel = false;
    private Panel<WizardDescriptor> mPOJOPanel = null;
    
    private POJOConsumerPalleteWizardIterator mPwiz = null;
  ...
  //  private String[] mSteps = null;
    ...
    public void initializePOJOWiz() {        
           mPwiz = new POJOConsumerPalleteWizardIterator();
           ...
           mPwiz.hasNext();
           POJOConsumerCreationPanelPalleteWizardPanel cpWiz = mPwiz.getConsumer();
            cpWiz.setWizard(wizard);
            ...
            String stepName = NbBundle.getMessage( this.getClass(), "LBL_POJO_Binding_Consumer_Name"  );
            jc.setName(stepName);
            steps.add(stepName);
 ...
 * */
//            mPwiz.populatePOJODefaults();
            populatePOJODefaults(mPwiz);
    ...
    public void initialize(TemplateWizard wiz){
        wizard = wiz;
        wizard.putProperty(BindingUtils.BINDING_EDITOR_MODE,BindingUtils.Type.OUTBOUND);
        wizard.putProperty(GeneratorUtil.HIDE_ADVANCED, Boolean.TRUE);        
        this.appendStep(NbBundle.getMessage( this.getClass(), "LBL_POJO_Binding_Consumer_Name"  ));// NOI18N
...
/*        if ( this.mPojoSteps == null) {
    ...
    public boolean hasNext() {
        mParentFirstStepPanel = false;
        boolean hasNext =  super.hasNext();
        if ( !hasNext) {
            
            Panel<WizardDescriptor> retrWiz =  mMapIndexWizard.get(Integer.valueOf(mStepNo));
            if ( retrWiz instanceof POJOConsumerCreationPanelPalleteWizardPanel ) {
                return false;
            }
    ...
    public boolean hasPrevious() {
       // String[] steps = getSteps();
        if ( this.mStepNo == 0 ) {
              mParentFirstStepPanel = true;
              return mParentFirstStepPanel;
        }     
        mParentFirstStepPanel = false;
...
//
    ...
    public Set<DataObject> instantiate(TemplateWizard wiz) throws IOException {
        ...
        POJOConsumerPalleteWizardIterator pwz = mPwiz;
        if ( !this.bPOJOPanelVisited) {
            
            pwz = new POJOConsumerPalleteWizardIterator();
            mPwiz = pwz;
            pwz.initialize(wizard);
            pwz.hasNext();
            this.populatePOJODefaults(pwz);
            //Get WSDL Model
/*            WSDLModel tempModel = (WSDLModel) wiz.getProperty(WSDLWizardConstants.TEMP_WSDLMODEL);
 ...
 */ 
    ...
    private void populatePOJODefaults(POJOConsumerPalleteWizardIterator pwz) {
      //  throw new UnsupportedOperationException("Not yet implemented");
            ...
            String pojoClassName = name;
            pojoClassName =GeneratorUtil.findNewFile(newFileDir, pojoClassName);
            POJOConsumerCreationPanel pojoCP = (POJOConsumerCreationPanel) pwz.getConsumer().getComponent();
            pojoCP.populateFromWSDLModel(tempModel, false);
    ...
    public Panel<WizardDescriptor> current() {
        ...
        if (mStepNo > 0 &&  bNextPanelIsPOJOPanel) {
            ...
            mMapIndexWizard.put(Integer.valueOf(mStepNo), mPOJOPanel);            
            bNextPanelIsPOJOPanel = false;
            return mPOJOPanel;
         }
