 ...
 * @author anjeleevich
 */
public class LocalNotificationsChildren extends
        WLMChildren<LocalNotificationsHolder>
{
    public LocalNotificationsChildren(LocalNotificationsHolder component,
            Lookup lookup)
    {
    ...
    protected Collection<? extends WLMComponent> createKeys() {
        ...
        List<TLocalNotification> result = null;

        if (holder != null) {
            result = holder.getLocalNotifications();
        }

        if (result == null) {
            result = new ArrayList<TLocalNotification>(0);
        }
    ...
    protected Node[] createNodes(WLMComponent component) {
        if (component instanceof TLocalNotification) {
            Node node = new LocalNotificationNode(
                    (TLocalNotification) component, getLookup());
    ...
    public static final WLMAcceptableDescendants ACCEPTABLE_DESCENDANTS
            = WLMAcceptableDescendants.create(WLMNodeType.LOCAL_NOTIFICATION);
}
