 ...
 * @author anjeleevich
 */
public class ImportsChildren extends WLMChildren<TTask> {

    public ImportsChildren(TTask component, Lookup lookup) {
        super(component, lookup);
    }

    @Override
    protected Collection<? extends WLMComponent> createKeys() {
        return getWLMComponent().getImports();
    }

    @Override
    protected Node[] createNodes(WLMComponent component) {
        if (component instanceof TImport) {
            TImport importComponent = (TImport) component;
            Lookup lookup = getLookup();
            return new Node[] { new ImportNode(importComponent, lookup) };
        }
    ...
    public static final WLMAcceptableDescendants ACCEPTABLE_DESCENDANTS
            = WLMAcceptableDescendants.create(WLMNodeType.IMPORT);
}
