 ...
 * @author anjeleevich
 */
public class UsersChildren extends WLMChildren<AssignmentHolder> {

    public UsersChildren(AssignmentHolder component, Lookup lookup) {
    ...
    private List<User> getAllUsers() {
        ...
        List<User> includedUsers = null;
        List<User> excludedUsers = null;

        if (assignment != null) {
            includedUsers = assignment.getUsers();
        ...
        if (includedUsers != null) {
            result.addAll(includedUsers);
        }

        if (excludedUsers != null) {
            result.addAll(excludedUsers);
        }
...
//        }
    ...
    protected Node[] createNodes(WLMComponent component) {
        if (component instanceof User) {
            return new Node[] { new UserNode((User) component, getLookup()) };
        }
    ...
    public static final WLMAcceptableDescendants ACCEPTABLE_DESCENDANTS
            = WLMAcceptableDescendants.create(WLMNodeType.USER);
