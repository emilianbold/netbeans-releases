 ...
 * @author anjeleevich
 */
public class ActionChildren extends WLMChildren<TAction> {

    public ActionChildren(TAction component, Lookup lookup) {
        super(component, lookup);
    }

    @Override
    protected Collection<? extends WLMComponent> createKeys() {
        List<TAction> key = new ArrayList<TAction>(1);
        key.add(getWLMComponent());
        return key;
    }
    ...
    protected Node[] createNodes(WLMComponent component) {
        if (component instanceof TAction) {
            ...
            Node loaclNotificaitonNode = new LocalNotificationsNode(
                    action, lookup);
            Node changeVariablesNode = new ChangeVariablesNode(action, lookup);
            
            return new Node[] { changeVariablesNode, loaclNotificaitonNode };
        }
