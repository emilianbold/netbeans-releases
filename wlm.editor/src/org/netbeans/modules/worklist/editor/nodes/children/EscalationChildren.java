 ...
 * @author anjeleevich
 */
public class EscalationChildren extends WLMChildren<TEscalation> {

    public EscalationChildren(TEscalation component, Lookup lookup) {
        super(component, lookup);
    }

    @Override
    protected Collection<? extends WLMComponent> createKeys() {
        List<WLMComponent> result = new ArrayList<WLMComponent>(0);
    ...
    protected Node[] createNodes(WLMComponent component) {
        if (component instanceof TEscalation) {
            Lookup lookup = getLookup();

            Node[] result = new Node[3];
            int i = 0;
            result[i++] = new TimeoutNode(
                    (DeadlineOrDuration) component, lookup);
            result[i++] = new AssignmentNode(
                    (AssignmentHolder) component, lookup);
            result[i++] = new LocalNotificationsNode(
                    (LocalNotificationsHolder) component, lookup);
            return result;
        }
