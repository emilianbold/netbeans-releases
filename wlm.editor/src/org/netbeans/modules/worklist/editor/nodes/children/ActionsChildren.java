 ...
 * @author anjeleevich
 */
public class ActionsChildren extends WLMChildren<TTask> {

    public ActionsChildren(TTask task, Lookup lookup) {
        super(task, lookup);
    }
    ...
    protected Collection<? extends WLMComponent> createKeys() {
        ...
        List<TAction> result = task.getActions();
        if (result == null) {
            result = new ArrayList<TAction>(0);
        }
    ...
    protected Node[] createNodes(WLMComponent component) {
        if (component instanceof TAction) {
            TAction action = (TAction) component;
            Node node = new ActionNode(action, getLookup());
    ...
    public static final WLMAcceptableDescendants ACCEPTABLE_DESCENDANTS
            = WLMAcceptableDescendants.create(
                    WLMNodeType.ACTION,
                    WLMNodeType.LOCAL_NOTIFICATIONS,
                    WLMNodeType.LOCAL_NOTIFICATION,
                    WLMNodeType.CHANGE_VARIABLES,
                    WLMNodeType.COPY);
}
