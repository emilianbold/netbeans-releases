 ...
 * @author anjeleevich
 */
public class GroupNode extends WLMNode<Group> {
    private String cachedXPath = null;
    
    public GroupNode(Group group, Lookup lookup) {
        this(group, Children.LEAF, lookup);
    }

    public GroupNode(Group group, Children children, Lookup lookup) {
        super(group, children, lookup);
        updateDisplayName();
    }

    @Override
    public void updateDisplayName() {
        Group group = getWLMComponent();

        String xpath = group.getContent();
        ...
        if (!StringUtils.equals(xpath, cachedXPath)) {
            ...
            boolean excluded = (group.getParent() instanceof TExcluded);
