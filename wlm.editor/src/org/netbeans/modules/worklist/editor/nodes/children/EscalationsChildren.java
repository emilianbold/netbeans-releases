 ...
 * @author anjeleevich
 */
public class EscalationsChildren extends WLMChildren<TTask> {

    public EscalationsChildren(TTask component, Lookup lookup) {
        super(component, lookup);
    }

    @Override
    protected Collection<? extends WLMComponent> createKeys() {
        List<TEscalation> result = getWLMComponent().getEscalations();
        if (result == null) {
            result = new ArrayList<TEscalation>(0);
        }
        return result;
    }
    ...
    protected Node[] createNodes(WLMComponent component) {
        if (component instanceof TEscalation) {
            return new Node[] { 
                    new EscalationNode((TEscalation) component, getLookup())
            };
        }
