 ...
 * @author anjeleevich
 */
public class TimeoutsChildren extends WLMChildren<TTask> {

    public TimeoutsChildren(TTask component, Lookup lookup) {
    ...
    protected Collection<? extends WLMComponent> createKeys() {
        ...
        List<TTimeout> result = task.getTimeouts();

        if (result == null) {
            result = new ArrayList<TTimeout>(0);
        }
    ...
    protected Node[] createNodes(WLMComponent component) {
        if (component instanceof TTimeout) {
            Node[] result = new Node[1];
            result[0] = new TimeoutNode((TTimeout) component, getLookup());
            return result;
        }
    ...
    public static final WLMAcceptableDescendants ACCEPTABLE_DESCENDANTS
            = WLMAcceptableDescendants.create(WLMNodeType.TIMEOUT);
}
