 ...
 * @author anjeleevich
 */
public class GoToDesignAction extends NodeAction {
    ...
    protected void performAction(Node[] nodes) {
        ...
        SwingUtilities.invokeLater(new GoToDesignRunnable((WLMNode) nodes[0]));
    }
    ...
    public String getName() {
        return NbBundle.getMessage(getClass(), 
                "GO_TO_DESIGN_ACTION_NAME"); // NOI18N
    }
    ...
    private static class GoToDesignRunnable implements Runnable {
        WLMNode node;

        public GoToDesignRunnable(WLMNode node) {
            this.node = node;
        }

        public void run() {
            WorklistDataObject dataObject = node.getLookup()
                    .lookup(WorklistDataObject.class);
            ...
            if (perspectives != null) {
                for (int i = 0; i < perspectives.length; i++) {
                    if (DesignerMultiViewDescription.PREFERRED_ID
