 ...
 * @author anjeleevich
 */
public class NotificationEMailsChildren extends WLMChildren<TNotification> {

    public NotificationEMailsChildren(TNotification component, Lookup lookup) {
        super(component, lookup);
    }

    @Override
    protected Collection<? extends WLMComponent> createKeys() {
        TNotification notificaion = getWLMComponent();
        TEmail email = notificaion.getEmail();

        List<EmailAddress> result = null;

        if (email != null) {
            result = email.getAddresses();
        }

        if (result == null) {
            result = new ArrayList<EmailAddress>(0);
        }
    ...
    protected Node[] createNodes(WLMComponent component) {
        if (component instanceof EmailAddress) {
            EmailAddress address = (EmailAddress) component;
            Node[] result = new Node[1];

            result[0] = new EmailAddressNode(address, getLookup());
    ...
    public static final WLMAcceptableDescendants ACCEPTABLE_DESCENDANTS
            = WLMAcceptableDescendants.create(WLMNodeType.EMAIL_ADDRESS);
}
