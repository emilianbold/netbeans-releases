 ...
 * @author anjeleevich
 */
public class InitVariablesChildren extends WLMChildren<TTask> {

    public InitVariablesChildren(TTask component, Lookup lookup) {
    ...
    protected Collection<? extends WLMComponent> createKeys() {
        ...
        VariableInit initVariables = (init == null) ? null
                : init.getVariableInit();

        List<TCopy> result = null;
        if (initVariables != null) {
            result = initVariables.getCopyList();
        }

        if (result == null) {
            result = new ArrayList<TCopy>(0);
        }
    ...
    protected Node[] createNodes(WLMComponent component) {
        if (component instanceof TCopy) {
            Node copyNode = new CopyNode((TCopy) component, getLookup());
    ...
    public static final WLMAcceptableDescendants ACCEPTABLE_DESCENDANTS
            = WLMAcceptableDescendants.create(WLMNodeType.COPY);
}
