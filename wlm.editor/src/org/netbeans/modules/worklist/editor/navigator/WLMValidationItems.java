 ...
 * @author anjeleevich
 */
public class WLMValidationItems {

    private ResultItem[] data = new ResultItem[4];
    private int size = 0;
    ...
    void add(ResultItem resultItem) {
        if (size == data.length) {
            ResultItem[] newData = new ResultItem[size * 3 / 2 + 1];
            System.arraycopy(this.data, 0, newData, 0, size);
            this.data = newData;
        }
        this.data[size++] = resultItem;
    ...
    public int getAdviceCount() {
        return size - errorCount - warningCount;
    }
