...
public final class WorklistWizardIterator implements WizardDescriptor.InstantiatingIterator {
        ...
        try {
            if (panels == null) {
                if (this.wizard instanceof TemplateWizard) {
                    ...
                    WLMWizardPanel1BottomPanel bottomPanel = new WLMWizardPanel1BottomPanel(folder);
                    // creates simple wizard panel with bottom panel
                    WizardDescriptor.Panel firstPanel = new WLMWizardPanel1(Templates.createSimpleTargetChooser(project, sourceGroups, bottomPanel), bottomPanel);
                    JComponent comp = (JComponent) firstPanel.getComponent();
                    // the bottom panel should listen to changes on file name text field
                    ((WLMWizardPanel1BottomPanel) bottomPanel).setNameTF(findFileNameField(comp, Templates.getTargetName(this.wizard)));

                    panels = new WizardDescriptor.Panel[]{
                                //new WLMWizardPanel1(folder),
                                firstPanel,
                                new WLMWizardPanel2(project),
                                new WLMWizardPanel3(),
                                new WLMWizardPanel4(),
                                new WLMWizardPanel5(),
                                new WLMWizardPanel6()
        ...
        if (this.wizard instanceof TemplateWizard) {
            ...
            if (dObj != null && dObj instanceof WorklistDataObject) {
                ...
                try {
                    ...
                    model.getTask().setTargetNamespace(targetNS);

                    String taskName = (String) wiz.getProperty(WizardConstants.TASK_NAME);

                    WSDLModel wsdlModel = generateWSDLFile(wiz, taskName, dir);

                    createPortType(wiz, wsdlModel);

                    Collection<PortType> portTypes = wsdlModel.getDefinitions().getPortTypes();
                    Collection<Operation> operations = portTypes.iterator().next().getOperations();
                    ...
                    if (users != null) {
                        ...
                        while (itU.hasNext()) {
                            ...
                            tuser.setContent(user.getUserName());
                    ...
                    if (groups != null) {
                        ...
                        while (itG.hasNext()) {
                            ...
                            tgroup.setContent(group.getGroupName());
    ...
    private void createPortType(TemplateWizard wiz, WSDLModel wsdlModel) {
        ElementOrType mInputElementOrType = (ElementOrType) wiz.getProperty(WLMVisualPanel2.INPUT_ELEMENT_OR_TYPE);
        ElementOrType mOutputElementOrType = (ElementOrType) wiz.getProperty(WLMVisualPanel2.OUTPUT_ELEMENT_OR_TYPE);
        ElementOrType mFaultElementOrType = (ElementOrType) wiz.getProperty(WLMVisualPanel2.FAULT_ELEMENT_OR_TYPE);
        WLMWSDLProvider.createPortType(wsdlModel, mInputElementOrType, mOutputElementOrType, mFaultElementOrType);
    }

    /**
     * To generate wsdl file in the newly created wlm project from the temp wsdl file
     * and return the wsdl model of the generated wsdl.
     * 
     * @param wiz
     * @param taskName
     * @param dir
     * @return WSDLModel newly generated wsdl file
     */
    private WSDLModel generateWSDLFile(TemplateWizard wiz, String taskName, FileObject dir) {
        String folderLocation = dir.getPath();
        WSDLModel tempWSDFFileModel = (WSDLModel) wiz.getProperty(WLMVisualPanel2.WSDL_MODEL);
        WLMWSDLProvider.createWSDLFile(taskName, folderLocation, tempWSDFFileModel);
        
        String wsdlFilePath = folderLocation + "/" + taskName + WSDLProvider.WSDL_SUFFIX + "." + WSDLProvider.WSDL_EXT;

        WSDLModel wsdlModel = Utility.createWSDLModel(wsdlFilePath);
        return wsdlModel;
    }
