...
import org.netbeans.modules.asm.base.generated.DisScanner;
import org.netbeans.modules.asm.base.syntax.AntlrScanner;
import org.netbeans.modules.asm.base.syntax.BaseAsmSyntax;

public class DisX86SyntaxProvider implements AsmSyntaxProvider {
   
    private static final ScannerFactory scannerFactory = new ScannerFactory() {
        
        public AntlrScanner createScanner(Reader input, Object state) {
            DisScanner scanner = new DisScanner(input);
    ...
    public AsmSyntax getSyntax(AsmModel model) {
        return new ATTSyntax((AbstractAsmModel)model, scannerFactory);
    }

    private static class ATTSyntax extends BaseAsmSyntax {
        public ATTSyntax(AbstractAsmModel model, ScannerFactory fact) {                       
            super(model, fact);
        }
        ...
        protected void patchAttParser(ATTParser parser) {
            ...
            parser.setFunctionNameDetector(DisFuncNameDetector.getInstance());
        }         
    }

    @Override
    public String toString() {
        return "Dis-X86";  // NOI18N
    }
}
