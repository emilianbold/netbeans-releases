...
#define D(i) ((s[i]-states[ttid][i])*1000/delta)

int64_t        states[int][10];
int64_t          past[int];
int64_t        lastts[int];
int64_t                  s[10];
int64_t                   w[8];
...
/delta && lastts[ttid]/
{
       ...
       wi=(t->t_sobj_ops?t->t_sobj_ops->sobj_type:0);
       w[wi]=D(7);

       printf("%d %d %d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d\n", cpu, ttid, lastts[ttid],
              D(0), D(1), D(2), D(3), D(4), D(5), D(8), D(9), D(6), w[0], w[1], w[2], w[3], w[4], w[5], w[6], w[7]);
