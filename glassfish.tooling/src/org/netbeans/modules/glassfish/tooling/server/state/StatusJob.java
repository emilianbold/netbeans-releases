...
import static org.netbeans.modules.glassfish.tooling.TaskState.COMPLETED;
...
import static org.netbeans.modules.glassfish.tooling.server.state.StatusJobState.NO_CHECK;
import static org.netbeans.modules.glassfish.tooling.server.state.StatusJobState.UNKNOWN;
import static org.netbeans.modules.glassfish.tooling.server.state.StatusJobState.UNKNOWN_PORT;
import static org.netbeans.modules.glassfish.tooling.server.state.StatusJobState.OFFLINE;
import static org.netbeans.modules.glassfish.tooling.server.state.StatusJobState.OFFLINE_PORT;
import static org.netbeans.modules.glassfish.tooling.server.state.StatusJobState.ONLINE;
import static org.netbeans.modules.glassfish.tooling.server.state.StatusJobState.SHUTDOWN;
import static org.netbeans.modules.glassfish.tooling.server.state.StatusJobState.SHUTDOWN_PORT;
import static org.netbeans.modules.glassfish.tooling.server.state.StatusJobState.STARTUP;
import static org.netbeans.modules.glassfish.tooling.server.state.StatusJobState.STARTUP_PORT;
...
public class StatusJob {
     ...
     */
    private static GlassFishStatusCheckResult checkResult(final StatusJob job) {
        return GlassFishStatusCheckResult.and(job.portCheck.evalResult(),
    ...
    private static void portCheckTransition(
            final StatusJob job, final GlassFishStatusTask task) {
    ...
    private static void commandTransition(
        ...
        GlassFishStatusCheckResult status = checkResult(job);
        GlassFishStatus oldState;
        GlassFishStatus newState;
        synchronized (job) {
            StatusJobState oldInternalState = job.state;
    ...
    void stopJob(StatusScheduler scheduler) {
        synchronized (this) {
            GlassFishStatus oldStatus = state.toGlassFishStatus();
            state = NO_CHECK;
            status.setStatus(state.toGlassFishStatus());
            scheduler.remove(this);
            scheduler.reschedule(this);
    ...
    private void startStateImpl(StatusScheduler scheduler,
        ...
        GlassFishStatus oldStatus = status.getStatus();
        state = STARTUP;
        status.setStatus(state.toGlassFishStatus());
        scheduler.remove(this);
        scheduler.reschedule(this);
    ...
    public void notifyNewStatusListeners(
        ...
        int i = 0;
        boolean isElement;
        LinkedList<GlassFishStatusListener> listeners;
