...
//                        log.info("Executing browser to show " + clientUrl);//NOI18N - no localization in maven build now.
        ...
        if (depl) {
            ...
            if (provider != null) {
                if (ExecutionChecker.DEV_NULL.equals(provider.getServerInstanceID())) {
                    ...
                    SelectAppServerPanel panel = new SelectAppServerPanel(!isDefaultGoal, project.getProjectDirectory());
                    ...
                    if (obj == NotifyDescriptor.OK_OPTION) {
                        ...
                        if (!ExecutionChecker.DEV_NULL.equals(instanceId)) {
                            ...
                            if (permanent) {
                                persistServer(instanceId, serverId, panel.getPermanentFolder());
    ...
    private void persistServer(final String iID, final String sID, final FileObject projFolder) {
        ...
        final FileObject fo = projFolder.getFileObject("pom.xml"); //NOI18N
        try {
            fo.getFileSystem().runAtomicAction(new FileSystem.AtomicAction() {
                public void run() throws IOException {
                    FileObject fo2 = FileUtil.createData(projFolder, "profiles.xml");
