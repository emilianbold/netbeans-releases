...
HRESULT Utils::registerInterfaceInGlobal(IUnknown *pUnk, REFIID riid, DWORD *pdwCookie) {
    ...
    return SUCCEEDED(AtlGetGITPtr(&spGIT)) ? 
        spGIT->RegisterInterfaceInGlobal(pUnk, riid, pdwCookie) : E_FAIL;
}

HRESULT Utils::getInterfaceFromGlobal(DWORD dwCookie, REFIID riid, void **ppv) {
    CComPtr<IGlobalInterfaceTable> spGIT;
    return SUCCEEDED(AtlGetGITPtr(&spGIT)) ? 
        spGIT->GetInterfaceFromGlobal(dwCookie, riid, ppv) : E_FAIL;
}

HRESULT Utils::revokeInterfaceFromGlobal(DWORD dwCookie) {
    CComPtr<IGlobalInterfaceTable> spGIT;
    return SUCCEEDED(AtlGetGITPtr(&spGIT)) ? 
